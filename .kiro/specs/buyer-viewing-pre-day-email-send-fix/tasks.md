...
OAuth2リフレッシュトークンの無効化（`invalid_grant`エラー）
...
1. ✅ エラーログの詳細化（Phase 1）
2. ✅ 環境変数の確認ログ改善（Phase 2）
3. ✅ エラーハンドリングの改善（Phase 3）
4. ✅ Google Calendar連携の再設定
5. ✅ メール送信テスト成功
...
- ✅ 買主内覧前日メール送信が正常に動作
- ✅ 500エラーが発生しない
- ✅ Gmail送信とカレンダー連携の両方が修復

### 今後の予防策

- リフレッシュトークンは毎日使用されるため、6ヶ月間の無効化リスクは低い
- `tenant@ifoo-oita.com` のパスワード変更時は、再度Google Calendar連携を実行
