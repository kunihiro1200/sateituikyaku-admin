...
- [x] 2. データベースクエリの最適化
  ...
  - [x] 2.2 deleted_at IS NULL条件のインデックスを作成
    - SQLマイグレーションファイルを作成
    - `CREATE INDEX idx_buyers_deleted_at ON buyers(deleted_at) WHERE deleted_at IS NULL;`を実行
    - _Requirements: 4.2_
  
  - [x] 2.3 Supabaseクライアントの接続プール設定を確認
    - `backend/src/config/supabase.ts`の設定を確認
    - 接続プールが有効化されていることを確認
    - _Requirements: 4.4_

- [x] 3. パフォーマンス監視とロギングの実装
  - [x] 3.1 BuyerService.getSidebarCounts()にログを追加
    - 処理開始時刻を記録
    - 処理完了時に所要時間をログ出力
    - 5秒超過時に警告ログを出力
    - エラー発生時にスタックトレースを出力
  ...
  - [x] 3.2 BuyerService.getSidebarCountsFallback()にログを追加
    - フォールバック処理の開始をログ出力
    - 処理時間をログ出力
...
- [x] 7. 後方互換性の確認
  ...
  - [x] 7.3 ステータス計算ロジックの一貫性確認
    - GASで計算されたステータスとバックエンドで計算されたステータスが一致することを確認
    - _Requirements: 6.3_
  
  - [x] 7.4 買主データ更新時のキャッシュ無効化確認
