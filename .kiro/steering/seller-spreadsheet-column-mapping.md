...
| **BC列（列54）** | **`査定額1（自動計算）v`** | **`valuation_amount_1`** | **NUMBER** | **査定額1（自動計算値、フォールバック）** |
| **BD列（列55）** | **`査定額2（自動計算）v`** | **`valuation_amount_2`** | **NUMBER** | **査定額2（自動計算値、フォールバック）** |
| **BE列（列56）** | **`査定額3（自動計算）v`** | **`valuation_amount_3`** | **NUMBER** | **査定額3（自動計算値、フォールバック）** |
...
| `査定額1（自動計算）v` | `valuation_amount_1` | **BC列（列54）** | NUMBER | 自動計算査定額1（フォールバック） | フォールバック |
| `査定額2（自動計算）v` | `valuation_amount_2` | **BD列（列55）** | NUMBER | 自動計算査定額2（フォールバック） | フォールバック |
| `査定額3（自動計算）v` | `valuation_amount_3` | **BE列（列56）** | NUMBER | 自動計算査定額3（フォールバック） | フォールバック |
...
**優先順位**（全ての種別で共通）：
```
CB,CC,CD列（手動入力査定額） > 自動計算査定額（BC/BD/BE列、列54-56）
```
...
| **CC** | 80 | `査定額2` | **手動入力（最優先）** |
| **CD** | 81 | `査定額3` | **手動入力（最優先）** |
| **BC** | 54 | `査定額1（自動計算）v` | 自動計算（フォールバック） |
| **BD** | 55 | `査定額2（自動計算）v` | 自動計算（フォールバック） |
| **BE** | 56 | `査定額3（自動計算）v` | 自動計算（フォールバック） |

#### データベースカラム

| データベースカラム名 | 対応するスプレッドシートカラム | 列位置 | 説明 |
|-------------------|---------------------------|--------|------|
| `valuation_amount_1` | **優先**: `査定額1`（CB列）<br>**フォールバック**: `査定額1（自動計算）v` | CB列（手動）<br>BC列（自動） | 査定額1（万円単位 → 円単位に自動変換） |
| `valuation_amount_2` | **優先**: `査定額2`（CC列）<br>**フォールバック**: `査定額2（自動計算）v` | CC列（手動）<br>BD列（自動） | 査定額2（万円単位 → 円単位に自動変換） |
| `valuation_amount_3` | **優先**: `査定額3`（CD列）<br>**フォールバック**: `査定額3（自動計算）v` | CD列（手動）<br>BE列（自動） | 査定額3（万円単位 → 円単位に自動変換） |
...
**査定額の特別ルール**（最重要）:
- ✅ **手動入力査定額（CB/CC/CD列、列79-81）を最優先で使用する**
- ✅ **手動入力が空の場合のみ、自動計算査定額（BC/BD/BE列、列54-56）を使用する**
- ✅ **種別に関係なく、この優先順位を適用する**
