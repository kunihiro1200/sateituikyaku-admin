...
### ⚠️ 重要：実際の定期同期はGASが担っている

**バックエンドコードに `EnhancedPeriodicSyncManager` が実装されていますが、Vercelはサーバーレス環境のため、常駐プロセスが存在しません。そのため、このクラスの定期実行は実際には動作しません。**

**実質的な定期同期は、Google Apps Script（GAS）の時間トリガーが担っています。**

### 実装

**GAS関数**: `syncSellerList`

**トリガー設定**:
- **同期間隔**: **10分ごと**（GASの時間トリガー）
- **実行場所**: Google スプレッドシートに紐づいたGASプロジェクト
- **エラー時**: GASのトリガーが継続して次回も実行される
...
### EnhancedPeriodicSyncManager について

`backend/src/services/EnhancedAutoSyncService.ts` に実装されているが、**Vercelサーバーレス環境では動作しない**。

- ❌ **Vercel本番環境**: サーバーレスのため常駐プロセスなし → 定期実行されない
- ✅ **ローカル開発環境**: `npm run dev` で起動した場合のみ動作する（開発・デバッグ用途）

**絶対に間違えないでください**: 本番環境の定期同期はGASの `syncSellerList`（10分トリガー）です。
...
| 項目 | 値 |
|------|-----|
| **トリガー** | GASの時間トリガー（10分ごと） |
| **同期間隔** | **10分ごと**（GAS設定） |
| **実装** | GAS関数 `syncSellerList` |
| **注意** | `EnhancedPeriodicSyncManager` はVercel本番環境では動作しない |
| **エラー時** | GASトリガーが継続して次回も実行 |
...
**確認事項**:
1. GASの `syncSellerList` トリガーが有効になっているか？（GASのトリガー管理画面で確認）
2. GASのエラーログにエラーが出ていないか？
3. バックエンドAPIが正常に動作しているか？

**解決策**:
```
GASのトリガー管理画面を開く
→ syncSellerList の10分トリガーが存在するか確認
→ 存在しない場合は再設定する
```

**注意**: `AUTO_SYNC_ENABLED` 環境変数はVercel本番環境では意味を持たない（`EnhancedPeriodicSyncManager` が動作しないため）
...
**理由**:
- GASの `syncSellerList` が10分ごとに自動実行されるため、手動同期は通常不要
- 緊急時のみ手動同期を実行
...
2. **スプレッドシート → データベース**: **10分ごと**（GASの `syncSellerList` トリガー）

**重要**: `EnhancedPeriodicSyncManager` はVercel本番環境では動作しない（サーバーレスのため）。本番の定期同期はGASが担っている。
...
**更新履歴**:
- 2026年3月21日: Vercelサーバーレス環境では `EnhancedPeriodicSyncManager` が動作しないことを明記。実質的な定期同期はGASの `syncSellerList`（10分トリガー）であることを追記
