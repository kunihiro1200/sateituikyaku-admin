...
重複には**性質の違う2種類**がある。混同して一括削除してはいけない。

| 種別 | 条件 | 対応 |
|---|---|---|
| **エラー重複** | 同一電話番号 **かつ 反響日時も完全一致** | 同じメールの二重処理。1件残して削除 |
| **正常な再問い合わせ** | 同一電話番号 **かつ 反響日時が違う** | 別時期の依頼。**残す**。重複ボタンで確認する |

例：`09023985316` の紺野さんは 2025-06-27 と 2026-08-28 に問い合わせ → 両方残すのが正しい。

### 現状（2026年8月31日時点）

| 分類 | 組数 |
|---|---|
| エラー重複（削除候補） | **0組**（cleanup-duplicates が既に処理済み） |
| 正常な再問い合わせ | 143組 |
| 反響日時が未記録で判別不可 | 646組 |

調査コマンド：`npx ts-node backend/check-duplicate-groups.ts`（読み取りのみ）

### 🚨 一括削除する前に必ず除外すべき2条件

過去に「重複」を機械的に数えると **655件** が削除候補に見えたが、実際は0件だった。
以下を除外しないと**無関係なレコードを大量に削除してしまう**。

**除外1：プレースホルダー電話番号**

電話番号欄に固定文字列が入っているレコードが多数あり、同じ文字列は同じハッシュになるため
別人同士が「同一電話番号」と誤判定される。
...
→ **数字が10桁未満なら電話番号として扱わない**。
`GET /api/sellers/:id/duplicates` はこの除外を実装済み（メールも形式チェックする）。

**除外2：`inquiry_detailed_datetime` が NULL のレコード**

古いレコードは反響詳細日時が未記録。`null` 同士を「同一日時」とみなすと、
別人・別依頼を重複と誤判定する。**NULL は判定不能として扱い、自動削除の対象にしない。**

### 重複表示の仕組み

- API: `GET /api/sellers/:id/duplicates`
  （`phone_number_hash` / `email_hash` / 正規化した `property_address` で照合）
- UI: 通話モードページの `DuplicateIndicatorBadge` → `DuplicateDetailsModal`
- ⚠️ **`phone_number_hash` に依存している**ため、ハッシュが壊れると重複バッジが出なくなる
  （2026年8月：408件でバッジが出ていなかった。ハッシュ修復で復旧）
...
| `is_restored` | `true` | 復元したレコードの目印 |
| `restored_at` | 復元日時 | 記録用 |

- 復元専用の分離スクリプト：`backend/separate-restored-sellers.ts`

### サイドバーの「復元」カテゴリーは削除済み

一度サイドバーに「復元」カテゴリーを実装したが、復元した142件は
cleanup-duplicates が正しく削除した重複レコードだったため**不要と判断して削除した**（2026年8月31日）。

142件は `deleted_at` を保持した論理削除状態で残っているため、
必要になれば `is_restored = true` で取り出せる（カラムは残置）。

**同じものを再実装しないこと。** 削除レコードを見たい要件が出た場合は、
カテゴリーを作る前に「それはエラー重複ではないか」を先に確認する。
