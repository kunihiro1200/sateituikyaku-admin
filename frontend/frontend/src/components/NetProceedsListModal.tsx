...
<div class="a4">
  ...
  <div class="layer">
    ...
    ${((p.taxMode === 'none' || p.taxMode === 'none_mortgage') ? p.rows.slice(0, 9) : p.taxMode === 'known' ? p.rows.slice(0, 12) : p.rows.slice(0, 9)).map((row, i) => {
      ...
      const mortgageLeft  = p.taxMode === 'none_mortgage' ? 96 : 77; // 印紙代の左側（none_mortgageは+10mm右にずらし済み）
      const stampLeft     = p.taxMode === 'none_mortgage' ? 121 : p.taxMode === 'none' ? 111 : p.taxMode === 'known' ? 95 : p.taxMode === 'unknown_mortgage' ? 97 : 74;
