...
export function generatePage4RepeaterHtml(propertyAddress: string, propertyPrice: number | null, propertyType: string | undefined, today: string, propertyNumber: string = ''): string {
  ...
  return `<div style="width:100%;height:100%;padding:10mm 15mm;background:#fff;font-family:${FONT};font-size:9pt;color:#000;box-sizing:border-box;display:flex;flex-direction:column;overflow:hidden;">
    <div style="border:2px solid #000;text-align:center;padding:8px;margin-bottom:4px;">
      <span style="font-size:14pt;font-weight:bold;">資金計画書《概算》（リピーター様）</span>
    </div>
    <div style="text-align:right;font-size:9pt;margin-bottom:2px;">作成日：　${today}</div>
    <div style="font-size:10pt;border-bottom:1px solid #000;padding-bottom:4px;margin-bottom:12px;">${propertyAddress}</div>
    <table style="width:100%;border-collapse:collapse;border:2px solid #000;margin-bottom:16px;">
...
export function generatePage4CashRepeaterHtml(propertyAddress: string, propertyPrice: number | null, propertyType: string | undefined, today: string, propertyNumber: string = ''): string {
  ...
  return `<div style="width:100%;height:100%;padding:10mm 15mm;background:#fff;font-family:${FONT};font-size:9pt;color:#000;box-sizing:border-box;display:flex;flex-direction:column;overflow:hidden;">
    <div style="border:2px solid #000;text-align:center;padding:8px;margin-bottom:4px;">
      <span style="font-size:14pt;font-weight:bold;">資金計画書《概算》（自己資金・リピーター様）</span>
    </div>
    <div style="text-align:right;font-size:9pt;margin-bottom:2px;">作成日：　${today}</div>
    <div style="font-size:10pt;border-bottom:1px solid #000;padding-bottom:4px;margin-bottom:12px;">${propertyAddress}</div>
    <table style="width:100%;border-collapse:collapse;border:2px solid #000;margin-bottom:16px;">
...
// ============================================================
// 全ページ結合（リピーター版）- 仲介手数料2%
// ============================================================
export function generateAllPagesRepeaterHtml(buyer: Record<string,unknown>, propertyDetails: Record<string,unknown>[], today: string): string {
  const pages: string[] = [];
  const buyerNumber = (buyer.buyer_number || '') as string;
  const viewingDateStr = (buyer.viewing_date || today) as string;
  for (const property of propertyDetails) {
...
// ============================================================
// 全ページ結合（自己資金・リピーター版）- 仲介手数料2%・住宅ローン欄なし
// ============================================================
export function generateAllPagesCashRepeaterHtml(buyer: Record<string,unknown>, propertyDetails: Record<string,unknown>[], today: string): string {
  const pages: string[] = [];
  const buyerNumber = (buyer.buyer_number || '') as string;
  const viewingDateStr = (buyer.viewing_date || today) as string;
  for (const property of propertyDetails) {
