...
export const generateCancellationGuidance = (
  ...
  if (site === 'ウ') {
    // イエウール
    message = `${name}様[改行]不動産一括査定サイトにご登録いただいており、個人情報がずっと残ったままになる為、一旦キャンセルされることをオススメ致します。（依頼日より3日以内の申請でキャンセルOK）[改行][改行]キャンセルされる場合は、下記イエウール本社へメールにて以下の内容をお送りください。[改行][改行]（＊注意点としまして[改行]査定額しりたかっただけ→キャンセルNG[改行]査定不要→キャンセルOK）[改行][改行]【送信先】ieul-support@speee.jp[改行]【件名】査定依頼のキャンセルについて[改行]【本文】↓そのまま本文コピペしてください[改行]査定依頼をキャンセルしたいです。査定不要です[改行]物件住所：${propertyAddress}[改行]お名前：${name}[改行][改行]ご不明な点がございましたら、お気軽にお問い合わせください。[改行][改行]㈱いふう[改行]<<当社住所>>`;
  } else if (site === 'す') {
    // すまいステップ
