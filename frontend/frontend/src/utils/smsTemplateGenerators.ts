...
export const generateInitialCancellationGuidance = (
  ...
  if (site === 'ウ') {
    // イエウール専用
    message = `${name}様[改行][改行]この度はイエウールより${propertyAddress}の査定依頼をいただきましてありがとうございます。[改行][改行]お電話させていただきましたが、ご不在のようでしたのでメッセージを送らせていただきました。[改行][改行]もし、査定のご依頼をキャンセルされる場合は、イエウールのサポートセンターまでご連絡をお願いいたします。[改行][改行]【イエウールサポートセンター】[改行]TEL: 05054971590[改行]Mail: ieul-support@speee.jp[改行][改行]またご連絡させていただきます。[改行][改行]㈱いふう[改行]<<当社住所>>[改行]TEL: 097-533-2022`;
  } else if (site === 'L' || site === 'Y') {
    // ライフルホームズ、Yahoo
    message = `${name}様[改行][改行]この度は${propertyAddress}の査定依頼をいただきましてありがとうございます。[改行][改行]お電話させていただきましたが、ご不在のようでしたのでメッセージを送らせていただきました。[改行][改行]もし、査定のご依頼をキャンセルされる場合は、このメッセージに返信する形でお知らせください。[改行][改行]またご連絡させていただきます。[改行][改行]㈱いふう[改行]<<当社住所>>[改行]TEL: 097-533-2022`;
  } else if (site === 'す') {
    // すまいステップ
    message = `${name}様[改行][改行]この度は${propertyAddress}の査定依頼をいただきましてありがとうございます。[改行][改行]お電話させていただきましたが、ご不在のようでしたのでメッセージを送らせていただきました。[改行][改行]もし、査定のご依頼をキャンセルされる場合は、下記のGoogleフォームよりお手続きください。[改行]https://forms.gle/iu3rLdPJ784WJxJW7[改行][改行]またご連絡させていただきます。[改行][改行]㈱いふう[改行]<<当社住所>>[改行]TEL: 097-533-2022`;
  } else if (site === 'H') {
    // HOME4U
    message = `${name}様[改行][改行]この度は${propertyAddress}の査定依頼をいただきましてありがとうございます。[改行][改行]お電話させていただきましたが、ご不在のようでしたのでメッセージを送らせていただきました。[改行][改行]またご連絡させていただきます。[改行][改行]㈱いふう[改行]<<当社住所>>[改行]TEL: 097-533-2022`;
  } else {
    // その他のサイト（基本メッセージ）
    message = `${name}様[改行][改行]この度は${propertyAddress}の査定依頼をいただきましてありがとうございます。[改行][改行]お電話させていただきましたが、ご不在のようでしたのでメッセージを送らせていただきました。[改行][改行]またご連絡させていただきます。[改行][改行]㈱いふう[改行]<<当社住所>>[改行]TEL: 097-533-2022`;
  }
...
export const generateCancellationGuidance = (
  ...
  if (site === 'ウ') {
    // イエウール
    message = `${name}様[改行]不動産一括査定サイトにご登録いただいておりますのでこの後も複数社から電話や訪問が続く可能性があるため、一旦キャンセルされることをオススメ致します。（依頼日より3日以内の申請でキャンセルOK）[改行][改行]キャンセルされる場合は、下記イエウール本社へメールにて以下の内容をお送りください。[改行][改行]（＊注意点としまして[改行]査定額しりたかっただけ→キャンセルNG[改行]査定不要→キャンセルOK[改行][改行]↓そのまま本文コピペしてください）[改行][改行]【送信先】ieul-support@speee.jp[改行]【件名】査定依頼のキャンセルについて[改行]【本文】[改行]査定依頼をキャンセルしたいです。査定不要です[改行]物件住所：${propertyAddress}[改行]お名前：${name}[改行][改行]ご不明な点がございましたら、お気軽にお問い合わせください。[改行][改行]㈱いふう[改行]<<当社住所>>[改行]TEL: 097-533-2022`;
  } else if (site === 'す') {
    // すまいステップ
