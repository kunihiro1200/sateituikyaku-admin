...
export const replacePlaceholders = (
  ...
  try {
    ...
    if (hasFI) {
      ...
      // 「不動産会社のいふうです」→「クジラ不動産です」（不動産会社が重複するため）
      result = result.replace(/不動産会社のいふうです/g, 'クジラ不動産です');
      // 「株式会社 いふう」（スペースあり）→「株式会社くじら不動産」
      result = result.replace(/株式会社\sいふう/g, '株式会社くじら不動産');
      // 「株式会社いふう」（スペースなし）→「株式会社くじら不動産」
