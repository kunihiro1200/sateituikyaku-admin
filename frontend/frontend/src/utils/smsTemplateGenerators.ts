...
export const replacePlaceholders = (
  ...
  try {
    ...
    if (hasFI) {
      ...
      // ステップ2: 「不動産会社のいふう」単体パターン
      result = result.replace(/不動産会社のいふう/g, 'くじら不動産');
      // ステップ3: 残った「株式会社 いふう」「株式会社いふう」を置換
      result = result.replace(/株式会社\s?いふう/g, '株式会社くじら不動産');
      // ステップ4: 「㈱いふう」→「㈱くじら不動産」
      result = result.replace(/㈱いふう/g, '㈱くじら不動産');
      // ステップ5: 万が一残った「不動産会社の株式会社くじら不動産」パターンの後処理
      result = result.replace(/不動産会社の株式会社くじら不動産/g, '株式会社くじら不動産');
    }
