...
export const PROPERTY_STATUS_DEFINITIONS = [
  ...
  { key: 'suumo_publish_notice', label: 'SUUMO登録+公開お知らせメール', color: '#1976d2' },
  ...
  { key: 'exclusive_asou', label: '麻専任公開中', color: '#ff5722' },
  { key: 'exclusive_hisa', label: '久専任公開中', color: '#ff5722' },
...
export const calculatePropertyStatus = (
  ...
  if (listing.property_number && listing.report_date) {
    console.log(`[未報告判定] ${listing.property_number}:`, {
      ...
      will_be_unreported: reportDate && reportDate <= today && isPublished,
    });
  }
  ...
  // 6.5 一般・公開中（AA14824以降）: 公開お知らせメール配信が未 → SUUMO登録+公開お知らせメール
  // ※ 判定開始基準: 物件番号 AA14824 以降。それ未満のAA物件・他プレフィックス物件は対象外
  if (isGeneralPublicTarget(listing)) {
    if (listing.publish_notice_email !== '済') {
      return PROPERTY_STATUS_DEFINITIONS.find(s => s.key === 'suumo_publish_notice')!;
    }
  }
