...
export default function FukuokaTateuriManagePage() {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      const dupCheck = await api.post('/api/tateuri/check-duplicate', {
        source_url: addUrl.trim(),
        region: 'fukuoka',
      });
  ...
  return (
    <div style={{ fontFamily: "'Hiragino Sans', 'Meiryo', sans-serif", background: '#f5f5f5', minHeight: '100vh' }}>
      ...
      {/* ヘッダー */}
      <div style={{ background: '#2196F3', color: '#fff', padding: '14px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <h1 style={{ fontSize: 18, fontWeight: 'bold', margin: 0 }}>🏠 福岡建売専門HP 管理画面</h1>
        <button onClick={() => window.open('/fukuoka-tateuri', '_blank')}
          style={{ background: 'rgba(255,255,255,0.2)', border: '1px solid rgba(255,255,255,0.4)', color: '#fff', padding: '6px 14px', borderRadius: 6, cursor: 'pointer', fontSize: 13 }}>
          🔗 公開サイトを見る
        </button>
      </div>
      ...
      <div style={{ maxWidth: 800, margin: '0 auto', padding: '24px 16px' }}>
        ...
        <div style={sectionStyle}>
          <h2 style={{ fontSize: 16, fontWeight: 'bold', marginBottom: 16, color: '#1565C0' }}>＋ 物件を追加（福岡）</h2>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            ...
            <button
              ...
              style={{ background: '#2196F3', color: '#fff', border: 'none', padding: '10px 20px', borderRadius: 6, cursor: adding ? 'not-allowed' : 'pointer', fontSize: 14, whiteSpace: 'nowrap', opacity: adding ? 0.7 : 1, fontWeight: 'bold' }}
            >
              {adding ? '取得中...' : '追加'}
            </button>
          </div>
                ...
                <div key={p.slug} style={{ display: 'flex', gap: 12, alignItems: 'center', padding: '10px 12px', background: '#fafafa', borderRadius: 8, border: '1px solid #eee' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6, flexShrink: 0 }}>
                    <button
                      ...
                      style={{ background: '#2196F3', color: '#fff', border: 'none', padding: '5px 12px', borderRadius: 4, cursor: 'pointer', fontSize: 12, fontWeight: 'bold' }}
                    >
                      確認
                    </button>
                    <button
