 ...
 */
function isViewingPrepUnconfirmedFrontend(buyer: { viewing_date?: string | null; viewing_prep_calendar_confirmed_at?: string | null; property_number?: string | null }): boolean {
  ...
  useEffect(() => {
    ...
    const fetchBuyers = async () => {
      try {
        ...
        // URLパラメータがある場合はバックエンドに直接渡す
        if (viewingMonth) quickParams.viewingMonth = viewingMonth;
        if (assigneeParam) quickParams.followUpAssignee = assigneeParam;
        
        // 全件データ未取得時でもselectedCalculatedStatusが指定されている場合はAPIにフィルタパラメータを渡す
        if (selectedCalculatedStatus && !viewingMonth && !assigneeParam) {
          // バックエンドのgetBuyersByStatusで英語キーで処理されるカテゴリはそのまま渡す
        ...
        if (viewingMonth || assigneeParam) {
          console.log('[BuyersPage] URL parameters: loaded via API directly');
        }
