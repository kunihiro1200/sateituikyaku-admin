...
export default function NearbyCasesPage() {
  ...
  // ボタンクリック直後（ユーザーのジェスチャー内）に hiddenTableRef の内容を
  // ClipboardItem でHTMLごとコピーする
  const handleCopy = async () => {
    if (!hiddenTableRef.current) return;

    const html = hiddenTableRef.current.innerHTML;

    // ClipboardItem はユーザーのジェスチャー直後でしか動かないため、
    // このハンドラを async にして await しても問題ない（同期的コールスタックに乗る）
    try {
      ...
      // ClipboardItem が使えない場合は execCommand で DOM 選択コピー
  ...
  return (
    <Container maxWidth="xl" sx={{ py: 3 }}>
      ...
      {/*
        =====================================================================
        コピー用 隠しHTMLテーブル
        画面には表示しないが、ClipboardItem.write() でこのdivのHTMLをコピーする。
        copyTargetCases が変わるたびにReactが再レンダリングするので常に最新を保持。
        =====================================================================
      */}
      <div
        ref={hiddenTableRef}
        aria-hidden="true"
        style={{ position: 'absolute', left: '-9999px', top: 0, width: '1px', height: '1px', overflow: 'hidden', opacity: 0 }}
      >
