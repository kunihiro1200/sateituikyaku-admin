...
import {
  ...
  generateSaleTimingSMS,
  generateBestTimeToSellSMS,
  generateCapitalGainsTaxSMS,
  convertLineBreaks,
  replacePlaceholders,
...
export function insertWrongNumberText(body: string, insertionText: string): string {
  const triggers = [
    ...
    '株式会社くじら不動産です。',
    'くじら不動産です。',
    '株式会社くじら不動産（株式会社いふう）です。', // FI変換後の「株式会社いふうです。」
    '株式会社くじら不動産と申します。',              // FI変換後の「株式会社いふうと申します。」
    '不動産会社の株式会社くじら不動産です。',         // FI変換後の「不動産会社の㈱いふうです。」
    'くじら不動産と申します。',                      // 短縮形
  ];
...
const CallModePage = () => {
  ...
  const filteredSellerEmailTemplates = useMemo(() => {
    ...
    const valuationReasonFiltered = siteFiltered.filter(t => {
    ...
    return valuationReasonFiltered
      ...
      .sort((a, b) =>
        getSellerEmailTemplatePriority(a.template.name) - getSellerEmailTemplatePriority(b.template.name)
  ...
  const [aiValuation, setAiValuation] = useState<{ amount1: number; amount2: number; amount3: number; targetFloor: number; targetArea: number } | null>(null);
  ...
  const smsTemplates: SMSTemplate[] = [
    ...
    },
    {
      id: 'sale_timing',
      label: '売却時期',
      generator: generateSaleTimingSMS,
    },
    {
      id: 'best_time_to_sell',
      label: '今が売り時',
      generator: generateBestTimeToSellSMS,
    },
    {
    ...
    },
    {
      ...
      label: '譲渡所得税',
      generator: generateCapitalGainsTaxSMS,
    },
    {
