...
export default function SellerOtherDecisionAnalysisPage() {
  ...
  const handleNext = () => {
    ...
    navigate(`/sellers/${nextQueueItem.sellerId}/other-decision-analysis?${params.toString()}`);
  };
  ...
  return (
    <Box sx={{ maxWidth: 960, mx: 'auto', p: { xs: 2, sm: 3 } }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
        <Button startIcon={<ArrowBackIcon />} onClick={() => navigate(backPath)} variant="outlined" size="small">
          {backLabel}
        </Button>
        {nextQueueItem && (
          <Button
            ...
            variant="outlined"
            size="small"
            sx={{ borderColor: '#c62828', color: '#c62828', '&:hover': { borderColor: '#7f0000', bgcolor: '#fce4ec' } }}
          >
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                  {qa.ai_questions.map((q, idx) => {
                    ...
                    <Box key={q.id}>
                      ...
                      {isSaved && !isEditing ? (
                        <Box sx={{ bgcolor: '#fff', border: '1px solid #ef9a9a', borderRadius: 1, p: 1.5 }}>
                          <Box sx={{ display: 'flex', justifyContent: 'flex-end', mb: 0.5 }}>
                            <Button size="small" variant="text" sx={{ fontSize: '0.7rem', color: '#c62828', minWidth: 0, p: '0 4px' }}
