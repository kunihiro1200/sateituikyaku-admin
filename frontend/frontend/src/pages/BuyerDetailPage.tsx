...
export default function BuyerDetailPage() {
  ...
  // 買主同士の重複（電話番号・メールアドレス一致の他の買主）
  const fetchBuyerDuplicates = async () => {
    try {
      setBuyerDuplicatesLoading(true);
      setBuyerDuplicatesError(null);
      const res = await api.get(`/api/buyers/${buyer_number}/buyer-duplicates`);
      setBuyerDuplicates(res.data.duplicates || []);
    } catch (error) {
      console.error('Failed to fetch buyer duplicates:', error);
      setBuyerDuplicatesError('買主の重複情報の取得に失敗しました');
    } finally {
                    ...
                    <InlineEditableField
                      ...
                      helperText={
                        missingRequiredFields.has('other_company_property')
                          ? '「建物名/価格」に値がある場合、他社物件（住所）は必須です。買主様にここの文言が直接メールで届くので、価格は絶対に書かないでください。詳細な住所のみを書いてください。'
            ...
            >
              ...
              <Grid container spacing={isMobile ? 1 : 2}>
                {section.fields.map((field: any, fieldIndex: number) => {
                  ...
                  // viewing_survey_result フィールドは読み取り専用テキスト表示（inlineEditable: false のため先に処理）
                  ...
                  // viewing_survey_confirmed フィールドは「↑確認済み」ボタン（inlineEditable: false のため先に処理）
