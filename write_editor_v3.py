...
# -*- coding: utf-8 -*-
"""
RichTextCommentEditor.tsx を修正するスクリプト
根本原因: value prop が変わると useEffect が innerHTML を上書きし、
          保存した Range が指すノードが消えてカーソル位置が無効になる。

解決策:
1. useEffect での innerHTML 上書きを「エディタがフォーカスを持っていない時だけ」に限定
2. isFocusedRef で現在フォーカス中かどうかを追跡
3. blur 時にカーソル位置を保存してから isFocusedRef を false にする
4. focus 時に isFocusedRef を true にする
"""
