...
async def scrape_athome_property(url: str) -> dict:
    ...
    async with Stealth().use_async(async_playwright()) as p:
        ...
        exterior_keywords = ["外観", "室内", "リビング", "キッチン", "バス", "トイレ", "洗面", "玄関", "バルコニー", "眺望", "収納", "エントランス", "駐車場", "共有部分", "設備"]
        
        layout_images = []
        exterior_images = []
        
        # commentsから画像のラベルを取得して分類
        for i, (url, label) in enumerate(image_with_labels):
            # alt属性から判定
            is_layout = any(kw in label for kw in layout_keywords)
            is_exterior = any(kw in label for kw in exterior_keywords)
            
            # commentsからも判定（画像の順番に対応するコメントを探す）
            if i < len(result["comments"]):
                comment = result["comments"][i]
                if not is_layout:
                    is_layout = any(kw in comment for kw in layout_keywords)
                if not is_exterior:
                    is_exterior = any(kw in comment for kw in exterior_keywords)
