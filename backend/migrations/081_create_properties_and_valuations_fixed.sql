-- Phase 2: Properties & Valuations - Database Schema (Fixed Version)
-- Migration 081: Create properties and valuations tables
-- 修正版: 既存テーブルを削除してから作成

-- ============================================================================
-- 既存テーブルのクリーンアップ（必要な場合のみ）
-- ============================================================================

-- 既存のvaluationsテーブルを削除（propertiesへの外部キー制約があるため先に削除）
DROP TABLE IF EXISTS valuations CASCADE;

-- 既存のpropertiesテーブルを削除
...
BEGIN
  ...
  RAISE NOTICE '  - properties table: OK (% columns)', v_column_count;
