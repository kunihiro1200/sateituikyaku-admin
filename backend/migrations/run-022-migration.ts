...
async function runMigration() {
  try {
    console.log('🚀 Starting Migration 022: Add site column to sellers table\n');

    // Read SQL file
    const sqlPath = path.join(__dirname, '022_add_site_column.sql');
    ...
    if (verifyError) {
      ...
      console.log('   Please run: npx ts-node migrations/verify-022-migration.ts');
      return false;
    }

    console.log('✅ Site column verified successfully!\n');

    console.log('═'.repeat(60));
    console.log('🎉 Migration 022 completed successfully!');
    console.log('═'.repeat(60));
    console.log('\n📋 Summary of changes:');
    console.log('  ✓ Added site column to sellers table (VARCHAR(100), nullable)');
    console.log('  ✓ Created index idx_sellers_site for analytics queries');
