 ...
 */
router.get('/chat-debug', async (req, res) => {
    try {
 ...
 */
router.get('/name-by-email', async (req, res) => {
    try {
 ...
 */
router.get('/active-initials', async (req, res) => {
    try {
        // スプシから有効なイニシャルを取得
        ...
        try {
            ...
            const activeInitials = (employees || [])
                ...
                // 除外リストに含まれるものを除外
                if (EXCLUDED_INITIALS.includes(val))
                    return false;
                // 日本語のみのイニシャルを除外（例: 「生」）
                if (/^[\u3040-\u30FF\u4E00-\u9FFF]+$/.test(val))
                    return false;
 ...
 */
router.get('/normal-initials', async (req, res) => {
    try {
 ...
 */
router.get('/initials-by-email', async (req, res) => {
    try {
 ...
 */
router.get('/jimu-initials', async (req, res) => {
    try {
        // キャッシュがあれば返す
 ...
 */
router.get('/jimu-staff', async (req, res) => {
    try {
        // キャッシュがあれば返す
        if (jimuStaffCache && Date.now() < jimuStaffCache.expiresAt) {
 ...
 */
router.get('/is-sales', async (req, res) => {
    try {
        const email = req.employee?.email || '';
        if (!email) {
...
router.get('/', async (req, res) => {
    try {
        ...
        if (calendarStatus) {
            filteredEmployees = employeesWithStatus.filter((emp) => emp.calendarStatus.status === calendarStatus);
        }
