 ...
 * 現在のセルマッピングを返す（デバッグ用）
 */
router.get('/debug-cells', (_req, res) => {
...
const JYUCHO_ITEMS = [
    ...
    // ── 駐車場・駐輪場情報 ──
    { key: 'parking_info', label: '駐車場の台数・料金・利用条件等の詳細（記載があれば全て。台数、月額料金、空き状況など）。「第◯条」「第◯項」「◯ページ」などの参照情報は含めないこと', cell: 'M431', type: 'text' },
    { key: 'parking_reference', label: '駐車場情報の参照元（「第◯条」「第◯項」「◯ページ」など）', cell: 'BB431', type: 'text' },
...
7. date型の項目（基準日）は文書に記載されている日付をそのまま西暦で返すこと
   ...
   - 日付の記載が見つからない場合はnull
8. **駐車場情報（parking_info）について**:
   - 「第◯条」「第◯項」「◯ページ」などの参照情報は parking_info には含めないこと
   - 参照情報は parking_reference に分けて抽出すること
   - parking_info には実際の内容（台数、料金、利用条件など）のみを記載すること
9. 見つからない場合はnull
10. 必ず以下のJSON形式のみで応答すること（説明文・コードブロック記号は不要）
...
router.post('/write', async (req, res) => {
    try {
        const { spreadsheetUrl, results } = req.body;
        ...
        for (const r of results) {
            ...
            if (!master)
                continue;
            if (r.content === null || r.content === undefined || r.content === '')
                continue;
            if (master.type === 'date' && master.dateCells) {
                // 日付型: 西暦→令和変換して年・月・日セルに分割書き込み
                const parsed = parseDateToReiwa(r.content);
                const dc = master.dateCells;
                if (parsed.year)
                    dateWrites.push({ cell: dc.year, value: parsed.year });
                if (parsed.month)
                    dateWrites.push({ cell: dc.month, value: parsed.month });
                if (parsed.day)
                    dateWrites.push({ cell: dc.day, value: parsed.day });
            }
            else if (master.cell) {
        ...
        for (const item of writableItems) {
            ...
            if (item.type === 'boolean') {
                const boolVal = item.content === 'true' || item.content === true;
...
router.post('/save', async (req, res) => {
    try {
        const { propertyNumber, results } = req.body;
