...
const PanoramaUrlService_1 = require("../services/PanoramaUrlService");
...
const propertyImageService = new PropertyImageService_1.PropertyImageService(60, // cacheTTLMinutes（画像キャッシュ）
...
const panoramaUrlService = new PanoramaUrlService_1.PanoramaUrlService();
// InquirySyncServiceのインスタンス化（遅延初期化 + 動的インポート）
let inquirySyncService = null;
// InquirySyncServiceを取得（必要な時だけ初期化）
const getInquirySyncService = async () => {
    if (!inquirySyncService) {
        const { InquirySyncService } = await Promise.resolve().then(() => __importStar(require('../services/InquirySyncService')));
...
const inquiryRateLimiter = (0, rateLimiter_1.createRateLimiter)({
...
const inquirySchema = zod_1.z.object({
    ...
    phone: zod_1.z.string().min(1, '電話番号を入力してください').max(20, '電話番号は20文字以内で入力してください'),
    message: zod_1.z.string().min(1, 'メッセージを入力してください').max(1000, 'メッセージは1000文字以内で入力してください'),
    propertyId: zod_1.z.string().uuid('無効な物件IDです').optional()
});
// 公開物件一覧取得
router.get('/properties', async (req, res) => {
    try {
        const { limit = '20', offset = '0', propertyType, types, // 新しいパラメータ名
        minPrice, maxPrice, areas, location, minAge, maxAge, propertyNumber, showPublicOnly, // 公開中のみ表示フィルター
        withCoordinates, // 座標がある物件のみ取得（地図表示用）
         } = req.query;
        // パラメータのバリデーション
        const parsedLimit = Math.min(parseInt(limit, 10), 2000); // 最大2000件
...
// 決済日取得（物件番号で取得）
router.get('/properties/:propertyNumber/settlement-date', async (req, res) => {
    try {
...
// BQ列データ取得（物件番号で取得）
router.get('/properties/:propertyNumber/about', async (req, res) => {
    try {
        const { propertyNumber } = req.params;
        // BQ列データを取得
...
// 概算書PDF生成（物件番号で生成）
router.post('/properties/:propertyNumber/estimate-pdf', async (req, res) => {
    try {
        const { propertyNumber } = req.params;
        // 概算書PDFを生成
...
// おすすめコメント取得
router.get('/properties/:id/recommended-comment', async (req, res) => {
    try {
        const { id } = req.params;
        // 物件情報を取得
        const property = await propertyListingService.getPublicPropertyById(id);
        ...
        const result = await recommendedCommentService.getRecommendedComment(property.property_number, property.property_type, id);
        // キャッシュヘッダーを設定（5分間）
        res.set('Cache-Control', 'public, max-age=300');
        res.json(result);
    }
...
// お気に入り文言取得
router.get('/properties/:id/favorite-comment', async (req, res) => {
    try {
        const { id } = req.params;
        // お気に入り文言を取得
...
router.get('/properties/:id/comments-diagnostic', async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const recommendedComment = await recommendedCommentService.getRecommendedComment(property.property_number, property.property_type, id);
        ...
        if (!spreadsheetUrl || !spreadsheetUrl.includes('/spreadsheets/d/')) {
            ...
            spreadsheetUrl = workTask?.spreadsheet_url;
        }
        // セル位置/範囲の決定
        const cellPositionMap = {
            '土地': 'B53',
            '戸建て': 'B142',
            '戸建': 'B142',
            'マンション': 'B150',
...
// Athomeデータ取得
router.get('/properties/:id/athome', async (req, res) => {
    try {
        const { id } = req.params;
        // 物件情報を取得
        const property = await propertyListingService.getPublicPropertyById(id);
...
router.options('/images/:fileId', (req, res) => {
    res.set('Access-Control-Allow-Origin', '*');
    res.set('Access-Control-Allow-Methods', 'GET, OPTIONS');
    res.set('Access-Control-Allow-Headers', 'Content-Type');
    res.status(204).send();
});
router.options('/images/:fileId/thumbnail', (req, res) => {
    res.set('Access-Control-Allow-Origin', '*');
    res.set('Access-Control-Allow-Methods', 'GET, OPTIONS');
    res.set('Access-Control-Allow-Headers', 'Content-Type');
    res.status(204).send();
...
// 画像プロキシ（CORS対策）- サムネイル
router.get('/images/:fileId/thumbnail', async (req, res) => {
    try {
        const { fileId } = req.params;
        // CORSヘッダーを最初に設定
        res.set('Access-Control-Allow-Origin', '*');
...
// 画像削除（認証必須）
router.delete('/properties/:propertyId/images/:imageId', auth_1.authenticate, async (req, res) => {
    try {
        ...
        // 画像を削除
        const result = await propertyImageService.deleteImage(imageId, propertyId, storageUrl, deletedBy, ipAddress);
        if (result.success) {
            res.status(200).json({
                success: true,
...
// 問い合わせ送信
router.post('/inquiries', inquiryRateLimiter, async (req, res) => {
    try {
...
// 物件番号検索（社内用・認証必須）
router.get('/internal/properties/search', auth_1.authenticate, async (req, res) => {
    try {
        ...
        // 検索実行
        const results = await propertyListingService.searchByPropertyNumber(trimmedNumber, isExactMatch);
        res.json({
            properties: results,
...
// 格納先URL自動取得（認証必須）
router.post('/properties/:propertyNumber/retrieve-storage-url', auth_1.authenticate, async (req, res) => {
    try {
        const { propertyNumber } = req.params;
...
// 格納先URL手動更新（認証必須）
router.put('/properties/:propertyNumber/storage-url', auth_1.authenticate, async (req, res) => {
    try {
        const { propertyNumber } = req.params;
...
// パノラマURL取得
router.get('/properties/:propertyNumber/panorama-url', async (req, res) => {
    try {
        const { propertyNumber } = req.params;
...
// 環境変数診断エンドポイント（開発用）
router.get('/debug/env-check', async (_req, res) => {
    try {
        const envCheck = {
...
// データベース接続テスト（開発用）
router.get('/debug/db-test/:propertyNumber', async (req, res) => {
    try {
        const { propertyNumber } = req.params;
        const { PropertyDetailsService } = await Promise.resolve().then(() => __importStar(require('../services/PropertyDetailsService')));
...
// 画像キャッシュクリア（特定物件）
router.post('/properties/:identifier/clear-image-cache', async (req, res) => {
    try {
        const { identifier } = req.params;
...
// 全物件の画像キャッシュをクリア
router.post('/clear-all-image-cache', async (_req, res) => {
    try {
...
// 画像・基本情報を更新（軽量版）
router.post('/properties/:identifier/refresh-essential', async (req, res) => {
    try {
        const { identifier } = req.params;
...
// 全て更新（完全版）
router.post('/properties/:identifier/refresh-all', async (req, res) => {
    try {
        const { identifier } = req.params;
        ...
        const [favoriteComment, recommendedComment, athomeData] = await Promise.all([
            ...
            athomeDataService.getAthomeData(property.property_number, property.property_type, storageUrl).catch(err => {
...
router.post('/properties/:identifier/update-storage-url', async (req, res) => {
    try {
        const { identifier } = req.params;
        const { storageUrl } = req.body;
