 ...
 * 査定メールを送信
 */
router.post('/:sellerId/send-valuation-email', async (req, res) => {
    try {
 ...
 * 追客メールを送信
 */
router.post('/:sellerId/send-follow-up-email', [
    (0, express_validator_1.body)('content').notEmpty().withMessage('Email content is required'),
], async (req, res) => {
    try {
        ...
        // メールを送信
        const result = await emailService.sendFollowUpEmail(seller, content, req.employee.email);
        if (!result.success) {
            return res.status(502).json({
                error: {
                    code: 'EMAIL_SEND_ERROR',
 ...
 * テンプレートメールを送信
 */
router.post('/:sellerId/send-template-email', [
    (0, express_validator_1.body)('templateId').notEmpty().withMessage('Template ID is required'),
    (0, express_validator_1.body)('to').optional().isEmail().withMessage('Invalid email address'),
    ...
    (0, express_validator_1.body)('htmlBody').optional().isString().withMessage('HTML body must be a string'),
    (0, express_validator_1.body)('from').optional().isEmail().withMessage('Invalid from email address'),
    (0, express_validator_1.body)('attachments').optional().isArray().withMessage('Attachments must be an array'),
    (0, express_validator_1.body)('replyTo').optional().isEmail().withMessage('Invalid replyTo email address'),
], async (req, res) => {
    try {
        // デバッグログ: リクエストボディを記録
        ...
        if (attachments && Array.isArray(attachments) && attachments.length > 0) {
            ...
            const emailAttachmentsRaw = await Promise.all(attachments.map(async (img) => {
                // ローカルファイル（Base64データ）
                if (img.base64Data) {
                    return {
            ...
            if (replyTo) {
                ...
                result = await emailService.sendTemplateEmail(sellerWithUpdatedEmail, subject, content || '', req.employee.email, req.employee.id, htmlBody, // オプション: カスタムHTMLボディ（貼り付けた画像を含む場合）
        ...
        const EMAIL_TEMPLATE_ASSIGNEE_MAP = {
        ...
        if (!senderInitials && req.employee?.email) {
            try {
                const { EmployeeUtils } = await Promise.resolve().then(() => __importStar(require('../utils/employeeUtils')));
                ...
                if (matched?.initials)
                    senderInitials = matched.initials;
        ...
        if (assigneeKey && senderInitials && sellerId) {
            try {
                await sellerService.updateSeller(sellerId, { [assigneeKey]: senderInitials });
...
router.post('/with-images', [
    ...
    (0, express_validator_1.body)('sellerNumber').notEmpty().withMessage('Seller number is required'),
    (0, express_validator_1.body)('to').isEmail().withMessage('Valid email address is required'),
    (0, express_validator_1.body)('subject').notEmpty().withMessage('Subject is required'),
    (0, express_validator_1.body)('body').notEmpty().withMessage('Email body is required'),
    (0, express_validator_1.body)('selectedImages').optional().isObject().withMessage('Selected images must be an object'),
], async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
 ...
 * 売主の画像一覧を取得
 */
router.get('/images/:sellerNumber', async (req, res) => {
    try {
        ...
        // 画像識別サービスで分類
        const { ImageIdentifierService } = await Promise.resolve().then(() => __importStar(require('../services/ImageIdentifierService')));
 ...
 * 画像プレビューを取得
 */
router.get('/images/preview/:fileId', async (req, res) => {
    try {
 ...
 */
router.post('/send-distribution', [
    (0, express_validator_1.body)('senderAddress').isEmail().withMessage('Valid sender email address is required'),
    (0, express_validator_1.body)('recipients').isArray({ min: 1 }).withMessage('At least one recipient is required'),
    (0, express_validator_1.body)('subject').notEmpty().withMessage('Subject is required'),
    (0, express_validator_1.body)('body').notEmpty().withMessage('Email body is required'),
    (0, express_validator_1.body)('propertyNumber').optional().isString().withMessage('Property number must be a string'),
    ...
    try {
        ...
        if (attachments && Array.isArray(attachments) && attachments.length > 0) {
            const { GoogleDriveService } = await Promise.resolve().then(() => __importStar(require('../services/GoogleDriveService')));
            const driveService = new GoogleDriveService();
            const attachmentsRaw = await Promise.all(attachments.map(async (img) => {
                // ローカルファイル（Base64データ）
                if (img.base64Data) {
                    return {
                        filename: img.name || 'attachment.jpg',
            ...
            processedAttachments = attachmentsRaw.filter((a) => a !== null);
        }
 ...
 */
router.post('/by-seller-number/:sellerNumber/send-template-email', [
    (0, express_validator_1.body)('templateId').notEmpty().withMessage('Template ID is required'),
    (0, express_validator_1.body)('to').optional().isEmail().withMessage('Invalid email address'),
    (0, express_validator_1.body)('subject').notEmpty().withMessage('Subject is required'),
    (0, express_validator_1.body)('content').optional().isString().withMessage('Content must be a string'),
    ...
    try {
        ...
        if (sellerError || !sellerRow) {
            ...
            seller = await sellerService.decryptSeller(sellerRow);
        }
        const recipientEmail = to || seller.email;
        if (!recipientEmail) {
            return res.status(400).json({
        ...
        const sellerWithEmail = { ...seller, email: recipientEmail };
        if (attachments && Array.isArray(attachments) && attachments.length > 0) {
            const { GoogleDriveService } = await Promise.resolve().then(() => __importStar(require('../services/GoogleDriveService')));
            const driveService = new GoogleDriveService();
            const emailAttachmentsRaw = await Promise.all(attachments.map(async (img) => {
            ...
            const emailAttachments = emailAttachmentsRaw.filter(Boolean);
            result = await emailService.sendEmailWithCcAndAttachments({
                to: recipientEmail,
                subject,
                body: htmlBody || content || '',
            ...
            if (replyTo) {
                ...
                // 既存フロー（変更なし）
                result = await emailService.sendTemplateEmail(sellerWithEmail, subject, content || '', senderEmail, req.employee?.id || 'system', htmlBody, senderEmail);
            }
        }
        return res.json({
            success: true,
