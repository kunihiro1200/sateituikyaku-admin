...
const templateService = new EmailTemplateService_1.EmailTemplateService();
const staffService = new StaffManagementService_1.StaffManagementService();
...
router.get('/debug', async (req, res) => {
    ...
    try {
        ...
        // C列（区分）の全ユニーク値を確認
        const categories = [...new Set(rows.map((r) => (r[0] || '').toString().trim()))];
        debug.categories = categories;
        debug.firstRows = rows.slice(0, 10).map((r) => ({
            C: r[0], D: r[1], C_charCodes: (r[0] || '').split('').map((c) => c.charCodeAt(0))
        }));
        // 「買主」でフィルタした結果
        const buyerRows = rows.filter((r) => (r[0] || '').toString().trim() === '買主');
...
router.post('/property/merge', async (req, res) => {
    try {
        ...
        let sellerName = '';
        let sellerEmail = '';
        const resolveSellerName = (sellerNameValue, ownerInfo) => {
        ...
        // プレースホルダー置換
        const mergedSubject = templateService.mergePropertyTemplate(template.subject, property, sellerNameClean, staffInfo);
        const mergedBody = templateService.mergePropertyTemplate(template.body, property, sellerNameClean, staffInfo);
...
router.post('/:templateId/mergeMultiple', async (req, res) => {
    try {
        ...
        if (allProperties.length === 0) {
            ...
            mergedContent.subject = templateService.mergeAngleBracketPlaceholders(mergedContent.subject, buyer, [], staffInfoForMerge);
            mergedContent.body = templateService.mergeAngleBracketPlaceholders(mergedContent.body, buyer, [], staffInfoForMerge);
            // FI物件判定（物件なしの場合もpropertyIdsで判定）
            const hasFIPropertyNoMatch = propertyIds.some((id) => id.toUpperCase().includes('FI'));
            if (hasFIPropertyNoMatch) {
                ...
                mergedContent.body = mergedContent.body.replace(/〒870-0044\n?大分市舞鶴町1丁目3-30/g, '福岡市中央区舞鶴3丁目1－10');
                mergedContent.body = mergedContent.body.replace(/〒870-0044大分市舞鶴町1丁目3-30/g, '福岡市中央区舞鶴3丁目1－10');
            }
        ...
        // <<>> プレースホルダー用データ
        const propertyDataForPlaceholders = allProperties.map((p) => ({
        ...
        // {{}} 形式用データ（後方互換）
        const legacyProperties = allProperties.map((p) => ({
            propertyNumber: p.property_number || '',
        ...
        mergedContent.subject = templateService.mergeAngleBracketPlaceholders(mergedContent.subject, buyer, propertyDataForPlaceholders, staffInfoForMerge);
        mergedContent.body = templateService.mergeAngleBracketPlaceholders(mergedContent.body, buyer, propertyDataForPlaceholders, staffInfoForMerge);
        // FI物件判定: 問合せ物件番号に「FI」が含まれる場合、建売専門サイトリンクを削除し住所を置換
        const hasFIProperty = propertyIds.some((id) => id.toUpperCase().includes('FI'));
        if (hasFIProperty) {
