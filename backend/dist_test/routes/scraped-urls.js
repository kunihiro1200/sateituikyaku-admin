...
const ScrapedUrlService_1 = require("../services/ScrapedUrlService");
const router = (0, express_1.Router)();
const scrapedUrlService = new ScrapedUrlService_1.ScrapedUrlService();
/**
 * URLの重複チェック
 ...
 */
router.post('/check-duplicate-by-property-number', async (req, res) => {
    try {
        ...
        const result = await scrapedUrlService.checkDuplicateByPropertyNumber(propertyNumber, sourceSite || 'athome');
        return res.json({
            success: true,
            data: result,
        });
...
router.post('/', async (req, res) => {
    try {
        const { url, referenceUrl, scrapedResultUrl, propertyNumber, sourceSite, title, price, address, } = req.body;
        if (!url || !sourceSite) {
            return res.status(400).json({
                success: false,
 ...
 */
router.put('/:url/mark-as-posted', async (req, res) => {
    try {
...
router.get('/unposted', async (req, res) => {
    try {
        ...
        const urls = await scrapedUrlService.getUnpostedUrls(sourceSite);
        return res.json({
            success: true,
            data: urls,
        });
...
router.get('/history', async (req, res) => {
    try {
        ...
        const history = await scrapedUrlService.getScrapingHistory(limit ? parseInt(limit) : 50, sourceSite);
        return res.json({
            success: true,
            data: history,
        });
