...
const CATEGORY_EXCLUDE_FILTERS = {
 ...
 */
router.get('/places', auth_1.authenticate, async (req, res) => {
    try {
        ...
        await Promise.all(PLACE_CATEGORIES.map(async (category) => {
            try {
                ...
                if (response.data.status === 'OK' && response.data.results?.length > 0) {
                    ...
                    if (nameFilter) {
                        filtered = filtered.filter((p) => nameFilter.test(p.name || ''));
                    }
                    if (excludeFilter) {
                        filtered = filtered.filter((p) => !excludeFilter.test(p.name || ''));
                    }
