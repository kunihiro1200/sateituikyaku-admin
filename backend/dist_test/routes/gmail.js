...
router.post('/send', upload.array('attachments'), async (req, res) => {
    ...
    try {
        ...
        const sendWithTimeout = Promise.race([
            ...
            new Promise((_, reject) => setTimeout(() => reject(new Error('メール送信がタイムアウトしました（30秒）。ネットワーク接続を確認してください。')), 30000)),
        ]);
        ...
        if (propertyIds && propertyIds.length > 0) {
            try {
                ...
                const { data: listings } = await supabase
                    ...
                    .in('id', propertyIds);
                propertyNumbers = (listings || []).map((l) => l.property_number).filter(Boolean);
            }
            catch (err) {
        ...
        const employeeId = req.employee?.id || COMPANY_ACCOUNT_UUID;
