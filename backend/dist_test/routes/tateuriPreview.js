...
router.get('/', async (req, res) => {
    try {
        const region = req.query.region || 'oita';
        ...
        const isValidCoordForRegion = (lat, lng) => {
            if (lat == null || lng == null)
                return false;
            if (lat < coordRange.latMin || lat > coordRange.latMax || lng < coordRange.lngMin || lng > coordRange.lngMax)
                return false;
            const latStr = lat.toString();
            const lngStr = lng.toString();
            if (/\.\d*(\d)\1{4,}/.test(latStr))
                return false;
            if (/\.\d*(\d)\1{4,}/.test(lngStr))
                return false;
            return true;
        };
        const geocodingService = new GeocodingService_1.GeocodingService();
...
async function scrapeSuumoAndSave(url, region, res) {
    try {
        ...
        // SUUMOのHTMLを取得
        const htmlRes = await axios_1.default.get(url, {
            headers: {
        ...
        const extractField = (label) => {
            ...
            const pattern = new RegExp(escaped + '[\\s\\S]{0,200}?<\\/th>[\\s\\S]{0,50}?<td[^>]*>([\\s\\S]{1,300}?)<\\/td>', 'i');
        ...
        // 「支払シミュレーション」以降を除去
        if (price)
            price = price.replace(/支払.*$/, '').replace(/□.*$/, '').trim();
        // フォールバック: 「万円」パターン
        if (!price) {
        ...
        if (!address) {
            ...
            if (m)
                address = stripTags(m[1]).replace(/\[.*?\]/g, '').trim();
        }
        // カーナビ住所をフォールバックに
        if (!address) {
        ...
        if (!access) {
            ...
            if (trainMatch)
                access = trainMatch[1].trim();
        }
        // --- アピールコメント（特徴ピックアップ） ---
        let appeal_comment = null;
        ...
        if (!appeal_comment) {
            ...
            if (m2)
                appeal_comment = m2[1].trim();
        }
        // --- 提供元情報（会社名・電話番号） ---
        let provider_name = null;
        let provider_phone = null;
        ...
        if (latKw)
            lat = parseFloat(latKw[1]);
        if (lngKw)
            lng = parseFloat(lngKw[1]);
        ...
        if (!lat || !lng) {
            ...
            if (dataLat)
                lat = parseFloat(dataLat[1]);
            if (dataLng)
                lng = parseFloat(dataLng[1]);
        }
        ...
        for (const match of dataSrcMatches) {
            ...
            // 除外する画像（明らかに不要なもの）
            const shouldExclude = imgUrl.includes('/icon/') ||
        ...
        if (images.length === 0 && propertyNumber) {
            ...
            for (let i = 1; i <= 15; i++) {
                images.push(`https://img.suumo.com/jj/bukken/ichiran/JJ010FJ001.jpg?ar=011&bs=011&rn=${rn}&pn=${propertyNumber}&sp=0&no=${i}&ts=1`);
            }
        }
...
router.get('/test/send-notification', async (req, res) => {
    try {
        console.log('[Test] メール通知テスト開始');
        const { TateuriPriceCheckService } = await Promise.resolve().then(() => __importStar(require('../services/TateuriPriceCheckService')));
        ...
        await service.sendNotification(testPriceDowns, testSoldOuts);
...
router.get('/cron/price-check', async (req, res) => {
    try {
        ...
        // テスト用：件数制限パラメータ
        const limit = parseInt(req.query.limit) || 0; // 0 = 全件
