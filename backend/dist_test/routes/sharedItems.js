...
const SharedItemsService_1 = require("../services/SharedItemsService");
const EmailService_1 = require("../services/EmailService");
const multer_1 = __importDefault(require("multer"));
const supabase_js_1 = require("@supabase/supabase-js");
const router = (0, express_1.Router)();
const sharedItemsService = new SharedItemsService_1.SharedItemsService();
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const newStaffNotShared = updates['共有できていない'] || updates['staff_not_shared'] || '';
        const newConfirmationDate = updates['確認日'] || updates['confirmation_date'] || '';
        // 確認日が空欄の場合のみメール送信を検討
        if (!newConfirmationDate) {
            try {
                ...
                const previousStaffNotShared = currentItem?.['共有できていない'] || currentItem?.staff_not_shared || '';
                const previousNames = previousStaffNotShared
                    ? previousStaffNotShared.split(',').map((s) => s.trim()).filter(Boolean)
                    : [];
                const newNames = newStaffNotShared
                    ? newStaffNotShared.split(',').map((s) => s.trim()).filter(Boolean)
                    : [];
 ...
 */
router.post('/:id/staff-confirmation', async (req, res) => {
    try {
        await ensureInitialized();
 ...
 */
router.post('/:id/mark-confirmed', async (req, res) => {
    try {
        await ensureInitialized();
        const { staffName } = req.body;
        if (!staffName) {
