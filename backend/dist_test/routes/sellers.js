...
const PropertyDistributionAreaCalculator_1 = require("../services/PropertyDistributionAreaCalculator");
const CityNameExtractor_1 = require("../services/CityNameExtractor");
...
const EStatService_1 = require("../services/EStatService");
const router = (0, express_1.Router)();
const sellerService = new SellerService_supabase_1.SellerService();
// シングルトンインスタンス（キャッシュを維持するため）
const distributionCalculator = new PropertyDistributionAreaCalculator_1.PropertyDistributionAreaCalculator();
const cityExtractor = new CityNameExtractor_1.CityNameExtractor();
/**
 * SpreadsheetSyncServiceを初期化して返す（Vercelサーバーレス対応）
 * SyncQueueが使えない環境でも直接スプシ同期できるようにする
 */
async function createSpreadsheetSyncService() {
    try {
        ...
        return new SpreadsheetSyncService_1.SpreadsheetSyncService(sheetsClient, supabase);
    }
    catch (err) {
...
function getDuplicatesCache(sellerId) {
...
function setDuplicatesCache(sellerId, data) {
    duplicatesCache.set(sellerId, { data, expiresAt: Date.now() + DUPLICATES_CACHE_TTL_MS });
}
function invalidateDuplicatesCache(sellerId) {
    duplicatesCache.delete(sellerId);
}
// 認証不要のエンドポイントを先に定義
/**
...
router.get('/sidebar-counts/update', async (req, res) => {
    try {
        const { SellerSidebarCountsUpdateService } = await Promise.resolve().then(() => __importStar(require('../services/SellerSidebarCountsUpdateService')));
        const supabase = (0, supabase_js_1.createClient)(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_KEY);
...
router.post('/sidebar-counts/update', async (req, res) => {
    try {
...
router.post('/backfill-call-log', async (req, res) => {
    try {
        const fromDate = req.body?.from || '2026-04-13T00:00:00+09:00';
        ...
        const keyIdx = headers.findIndex((h) => h === '売主追客ログID');
        ...
        for (let i = 1; i < rawData.length; i++) {
            ...
            if (key)
                existingKeys.add(String(key).trim());
        }
        ...
        const sellerIds = [...new Set((activities || []).map((a) => a.seller_id))];
...
router.post('/', [
    ...
    (0, express_validator_1.body)('property').notEmpty().withMessage('Property information is required'),
    (0, express_validator_1.body)('property.address').notEmpty().withMessage('Property address is required'),
    (0, express_validator_1.body)('property.prefecture').optional().isString().withMessage('Prefecture must be a string'),
    (0, express_validator_1.body)('property.city').optional().isString().withMessage('City must be a string'),
    (0, express_validator_1.body)('property.propertyType').notEmpty().withMessage('Property type is required'),
    // Phase 1 validations (all optional for now)
    (0, express_validator_1.body)('inquirySource').optional().isString().withMessage('Inquiry source must be a string'),
    (0, express_validator_1.body)('inquiryYear').optional().isInt({ min: 2000, max: 2100 }).withMessage('Inquiry year must be a valid year'),
    (0, express_validator_1.body)('inquiryDate').optional().isISO8601().withMessage('Inquiry date must be a valid date'),
    (0, express_validator_1.body)('inquiryDatetime').optional().isISO8601().withMessage('Inquiry datetime must be a valid datetime'),
    (0, express_validator_1.body)('confidenceLevel').optional().isIn(['high', 'medium', 'low']).withMessage('Invalid confidence level'),
    (0, express_validator_1.body)('firstCallerInitials').optional().isString().withMessage('First caller initials must be a string'),
    (0, express_validator_1.body)('sellerNumber').optional().matches(/^AA\d+$/).withMessage('Seller number must be in format AA{number}'),
], async (req, res) => {
    try {
...
router.get('/', [
    ...
    (0, express_validator_1.query)('pageSize').optional().isInt({ min: 1, max: 9999 }).withMessage('Page size must be between 1 and 9999'),
    // Phase 1 filter validations
    (0, express_validator_1.query)('inquirySource').optional().isString().withMessage('Inquiry source must be a string'),
    (0, express_validator_1.query)('inquiryYearFrom').optional().isInt({ min: 2000 }).withMessage('Inquiry year from must be a valid year'),
    (0, express_validator_1.query)('inquiryYearTo').optional().isInt({ min: 2000 }).withMessage('Inquiry year to must be a valid year'),
    (0, express_validator_1.query)('isUnreachable').optional().isBoolean().withMessage('Is unreachable must be a boolean'),
    (0, express_validator_1.query)('confidenceLevel').optional().isIn(['high', 'medium', 'low']).withMessage('Invalid confidence level'),
    (0, express_validator_1.query)('firstCaller').optional().isString().withMessage('First caller must be a string'),
    (0, express_validator_1.query)('duplicateConfirmed').optional().isBoolean().withMessage('Duplicate confirmed must be a boolean'),
    (0, express_validator_1.query)('valuationNotRequired').optional().isBoolean().withMessage('Valuation not required must be a boolean'),
    (0, express_validator_1.query)('inquirySite').optional().isString().withMessage('Inquiry site must be a string'),
    (0, express_validator_1.query)('propertyType').optional().isString().withMessage('Property type must be a string'),
    (0, express_validator_1.query)('statusFilter').optional().isString().withMessage('Status filter must be a string'),
    // サイドバーカテゴリフィルター（visitAssigned:xxx, todayCallAssigned:xxx の動的カテゴリも許可）
    (0, express_validator_1.query)('statusCategory').optional().isString().withMessage('Invalid status category'),
], async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
        ...
        const params = {
            ...
            nextCallDateFrom: req.query.nextCallDateFrom ? new Date(req.query.nextCallDateFrom) : undefined,
            nextCallDateTo: req.query.nextCallDateTo ? new Date(req.query.nextCallDateTo) : undefined,
            sortBy: req.query.sortBy || 'created_at',
            sortOrder: req.query.sortOrder || 'desc',
            // Phase 1 filters
            inquirySource: req.query.inquirySource,
            inquiryYearFrom: req.query.inquiryYearFrom ? parseInt(req.query.inquiryYearFrom) : undefined,
            inquiryYearTo: req.query.inquiryYearTo ? parseInt(req.query.inquiryYearTo) : undefined,
            ...
            firstCaller: req.query.firstCaller,
            ...
            inquirySite: req.query.inquirySite,
 ...
 * サイドバーの担当者別カテゴリー表示用
 */
router.get('/assignee-initials', async (req, res) => {
    try {
 ...
 * "AA" + (n + 1) 形式で返す
 */
router.get('/next-seller-number', async (req, res) => {
    try {
        const sheetsClient = new GoogleSheetsClient_1.GoogleSheetsClient({
 ...
 */
router.get('/call-ranking', async (req, res) => {
    try {
        // JSTで当月の開始日・終了日を計算
        const now = new Date();
        ...
        for (const row of data || []) {
            const initial = row.first_call_person;
 ...
 */
router.get('/call-ranking-yearly', async (req, res) => {
    try {
 ...
 */
router.get('/call-tracking-ranking', async (req, res) => {
    try {
        // JSTで当月の開始日・終了日を計算
        const now = new Date();
        const jstOffset = 9 * 60 * 60 * 1000;
        ...
        const { GoogleSheetsClient } = await Promise.resolve().then(() => __importStar(require('../services/GoogleSheetsClient')));
        const { sheetsRateLimiter } = await Promise.resolve().then(() => __importStar(require('../services/RateLimiter')));
        const sheetsClient = new GoogleSheetsClient({
            spreadsheetId: '1wKBRLWbT6pSKa9IlTDabjhjTnfs_GxX6Rn6M6kbio1I',
            sheetName: '売主追客ログ',
            serviceAccountKeyPath: process.env.GOOGLE_SERVICE_ACCOUNT_KEY_PATH,
        ...
        // ヘッダー行からインデックスを動的に取得
        const headers = rawData[0];
        const dateColIdx = headers.findIndex((h) => h === '日付');
        const firstHalfColIdx = headers.findIndex((h) => h === '担当（前半）');
        const secondHalfColIdx = headers.findIndex((h) => h === '担当（後半）');
...
router.get('/performance-metrics', [
    (0, express_validator_1.query)('month').notEmpty().withMessage('Month is required').matches(/^\d{4}-\d{2}$/).withMessage('Month must be in YYYY-MM format'),
], async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
        ...
        const { month } = req.query;
        const performanceMetricsService = new PerformanceMetricsService_1.PerformanceMetricsService();
        // 拡張メトリクス（月平均を含む）を返す
        const metrics = await performanceMetricsService.calculateEnhancedMetrics(month);
        res.json(metrics);
    }
    catch (error) {
 ...
 */
router.get('/by-number/:sellerNumber', async (req, res) => {
    try {
        ...
        const { data: sellerRaw } = await supabase
            ...
            .eq('seller_number', sellerNumber)
            .single();
        let latitude = sellerRaw?.latitude ?? null;
        let longitude = sellerRaw?.longitude ?? null;
        // 座標がない場合、バックエンドでジオコーディングして取得・保存
        if ((latitude == null || longitude == null) && seller.propertyAddress) {
            try {
                const apiKey = process.env.GOOGLE_MAPS_API_KEY;
                if (apiKey) {
                    const { GeocodingService } = await Promise.resolve().then(() => __importStar(require('../services/GeocodingService')));
 ...
 */
router.patch('/by-number/:sellerNumber', async (req, res) => {
    try {
        const { sellerNumber } = req.params;
        ...
        if (phone_number_hash) {
            ...
            if (phoneMatches) {
                ...
                for (const seller of phoneMatches) {
                    let decryptedName = '';
                    try {
                        decryptedName = seller.name ? decrypt(seller.name) : '';
                    }
                    catch { /* skip */ }
...
router.put('/:id', async (req, res) => {
    try {
        ...
        if (appointmentDate && assignedTo) {
            ...
            const seller = await sellerService.updateSellerWithAppointment(req.params.id, req.body, req.employee.id);
            res.json(seller);
            // スプレッドシートに非同期で同期（レスポンスをブロックしない）
            createSpreadsheetSyncService().then(syncService => {
                if (syncService) {
                    syncService.syncToSpreadsheet(req.params.id).catch(e => console.error('⚠️ [SpreadsheetSync] Sync error (appointment):', e));
                }
            });
            // サイドバーカウントを非同期で即時更新（レスポンスをブロックしない）
            Promise.resolve().then(() => __importStar(require('../services/SellerSidebarCountsUpdateService'))).then(({ SellerSidebarCountsUpdateService }) => {
                ...
                updateService.updateSellerSidebarCounts().catch((e) => console.error('⚠️ [SidebarCounts] Update error:', e));
            }).catch((e) => console.error('⚠️ [SidebarCounts] Import error:', e));
        }
        else {
            // 通常の更新
            ...
            createSpreadsheetSyncService().then(syncService => {
                if (syncService) {
                    syncService.syncToSpreadsheet(req.params.id).catch(e => console.error('⚠️ [SpreadsheetSync] Sync error:', e));
                }
            });
            ...
            if (req.body.firstCallPerson !== undefined) {
                ...
                createSpreadsheetSyncService().then(syncService => {
                    if (syncService) {
                        ...
                        }).catch(e => console.error('⚠️ [SpreadsheetSync] firstCallPerson sync error (best-effort):', e));
                    }
                }).catch(e => console.error('⚠️ [SpreadsheetSync] Failed to initialize sync service for firstCallPerson:', e));
            }
            // サイドバーカウントを非同期で即時更新（レスポンスをブロックしない）
            Promise.resolve().then(() => __importStar(require('../services/SellerSidebarCountsUpdateService'))).then(({ SellerSidebarCountsUpdateService }) => {
                const { createClient } = require('@supabase/supabase-js');
 ...
 */
router.post('/:id/clear-unreachable', async (req, res) => {
    try {
        const { id } = req.params;
 ...
 */
router.post('/:id/confirm-duplicate', async (req, res) => {
    try {
        const { id } = req.params;
 ...
 */
router.get('/:id/duplicate-history', async (req, res) => {
    try {
        const { id } = req.params;
...
router.get('/check-duplicate', [
    (0, express_validator_1.query)('phone').notEmpty().withMessage('Phone number is required'),
    (0, express_validator_1.query)('email').optional().isEmail().withMessage('Invalid email format'),
    (0, express_validator_1.query)('excludeId').optional().isUUID().withMessage('Exclude ID must be a valid UUID'),
], async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
        ...
        const matches = await sellerService.checkDuplicates(phone, email, excludeId);
        res.json({
 ...
 * 査定額メールを送信
 */
router.post('/:id/send-valuation-email', async (req, res) => {
    try {
        const { id } = req.params;
        // キャッシュをクリアして最新の売主情報を取得
        const { CacheHelper } = await Promise.resolve().then(() => __importStar(require('../utils/cache')));
        ...
        // メール送信
        const { EmailService } = await Promise.resolve().then(() => __importStar(require('../services/EmailService.supabase')));
        const emailService = new EmailService();
        const result = await emailService.sendValuationEmail(seller, valuationData, req.employee.email, req.employee.id);
        if (!result.success) {
            return res.status(500).json({
                error: {
 ...
 * 売主の追客ログ履歴を取得
 */
router.get('/:sellerNumber/follow-up-logs/history', [
    (0, express_validator_1.query)('refresh').optional().isBoolean().withMessage('Refresh must be a boolean'),
], async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
        ...
        const { followUpLogHistoryService } = await Promise.resolve().then(() => __importStar(require('../services/FollowUpLogHistoryService')));
        // 履歴ログを取得
 ...
 */
router.get('/:id/nearby-buyers', async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const buyerService = new BuyerService_1.BuyerService();
 ...
 */
router.get('/:id/inquiry-url', async (req, res) => {
    try {
        const { id } = req.params;
        const seller = await sellerService.getSeller(id);
        if (!seller) {
        ...
        const sheetsInstance = client.sheets;
 ...
 */
router.post('/:id/area-report', async (req, res) => {
    try {
        const { id } = req.params;
        const seller = await sellerService.getSeller(id);
        if (!seller)
        ...
        const areaRatio = (0, EStatService_1.estimateAreaRatio)(city, town);
        const [realPopData, realTrxData, realPriceData] = await Promise.all([
            (0, EStatService_1.fetchPopulationData)(city, areaRatio),
            (0, EStatService_1.fetchTransactionData)(city, areaRatio),
            (0, EStatService_1.fetchPriceData)(city),
        ]);
...
JSONのみ返してください。`;
        ...
        const hhRows = hh.map((r) => `<tr><td style="${td}">${r.type}</td><td style="${td}">${r.city}</td><td style="${tda}">${r.area}</td></tr>`).join('');
        const trx = data.transactions || [];
        const trRows = trx.map((r, i) => {
        ...
        const summary = (data.summary || []).map((s) => `<li style="margin-bottom:8px;"><strong style="color:#c62828;">${s}</strong></li>`).join('');
        const tbl = (rows) => `<table style="width:100%;border-collapse:collapse;margin-bottom:8px;"><thead><tr style="background:#e3f2fd;"><th style="${th}width:20%">年</th><th style="${th}width:40%">${CL}</th><th style="${th}width:40%;background:#fff9c4">${AL}</th></tr></thead><tbody>${rows}</tbody></table>`;
        ...
        const cmt = (t) => `<p style="font-size:11px;color:#555;background:#f5f5f5;padding:6px 10px;border-radius:4px;">📊 <strong>分析：</strong>${t}</p>`;
        const sec = (n, t) => `<h2 style="font-size:15px;color:#1a237e;border-left:4px solid #1a237e;padding-left:8px;margin-bottom:10px;">${n} ${t}</h2>`;
 ...
 */
router.get('/:id/nearby-properties', auth_1.authenticate, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const rawAddress = seller.propertyAddress || seller.property_address || seller.address || '';
        if (!rawAddress) {
        ...
        const { data: sellerRaw } = await supabase
            ...
            .eq('id', id)
            .single();
        let centerLat = sellerRaw?.latitude || null;
        let centerLng = sellerRaw?.longitude || null;
        // DBに座標がなければジオコーディング
        if (!centerLat || !centerLng) {
            const { GeocodingService } = await Promise.resolve().then(() => __importStar(require('../services/GeocodingService')));
            const geocodingService = new GeocodingService();
            const sellerPrefix = (seller.sellerNumber || '').slice(0, 2);
        ...
        // 売主の種別を取得
        const sellerPropertyType = seller.propertyType || seller.property_type || '';
        // 種別グループを判定
        ...
        const matchesAllowedType = (rowType) => {
            if (allowedTypesNP.length === 0)
                return true;
            const rt = rowType.trim();
            return allowedTypesNP.some(t => rt === t || rt.startsWith(t) || t.startsWith(rt));
        };
        // Excelシリアル値→日付文字列変換
        const excelSerialToDateStr = (value) => {
            if (!value)
                return '';
 ...
 *   - 土/土地 → 物件スプシの種別が土/土地
 */
router.get('/:id/sales-history', auth_1.authenticate, async (req, res) => {
    try {
        const { id } = req.params;
        // 売主情報を取得
        const seller = await sellerService.getSeller(id);
        ...
        const extractSearchKeyword = (addr) => {
            const normalized = normalizeAddress(addr);
            // 市・区・町・村・郡 で終わる行政区画を検出
            const cityMatch = normalized.match(/^(.+?[市区町村郡])/);
            if (!cityMatch)
                return normalized.slice(0, 5); // フォールバック
...
router.post('/manual-sync', async (_req, res) => {
    ...
    try {
        ...
        const step1Res = await axios_1.default.get(STEP1_URL, { timeout: 300000 });
        ...
        const step2Res = await axios_1.default.post(`${BACKEND_URL}/api/sync/trigger?additionOnly=true`, {}, {
            headers: {
