 ...
 */
router.post('/transfer-to-spreadsheet', async (req, res) => {
    try {
        const { propertyNumber, spreadsheetUrl, results } = req.body;
        if (!propertyNumber) {
            return res.status(400).json({ error: 'propertyNumber が必要です' });
        }
        if (!spreadsheetUrl) {
        ...
        const extractContentOnly = (text, maxLength) => {
            let content = text;
        ...
        const determineUserRange = (content) => {
        ...
        // ヘルパー関数: 支払先を判定
        const determinePayee = (content) => {
            if (content.includes('管理組合'))
                return '管理組合';
            if (content.includes('管理会社'))
                return '管理会社';
            if (content.includes('理事会'))
                return '理事会';
            return '管理組合'; // デフォルト
        };
        // ヘルパー関数: 用途制限の種類を判定
        const determineUsageRestriction = (content) => {
