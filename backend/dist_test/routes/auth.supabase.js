...
router.post('/callback', async (req, res) => {
    try {
        ...
        const { data: { user }, error } = await supabase_1.supabaseClient.auth.setSession({
            access_token,
        ...
        const employee = await authService.getOrCreateEmployee(user.id, user.email, user.user_metadata);
        if (isDev) {
 ...
 */
router.get('/my-initials', async (req, res) => {
    try {
        const authHeader = req.headers.authorization;
        if (!authHeader || !authHeader.startsWith('Bearer ')) {
        ...
        if (!email)
            return res.json({ initials: null });
        // DBのinitialsカラムを確認
        const dbInitials = employee?.initials;
        if (dbInitials)
