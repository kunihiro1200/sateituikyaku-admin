...
const TokiExtractService_1 = require("../services/TokiExtractService");
const router = (0, express_1.Router)();
const workTaskService = new WorkTaskService_1.WorkTaskService();
const tokiExtractService = new TokiExtractService_1.TokiExtractService();
/**
 * POST /api/toki-extract/:propertyNumber/extract
 * 謄本PDFを読み取り、抽出結果をプレビューとして返す（スプシへの書き込みはしない）
 */
router.post('/:propertyNumber/extract', async (req, res) => {
    try {
        ...
        const storageUrl = workTask.storage_url ?? null;
        const spreadsheetUrl = workTask.spreadsheet_url ?? null;
        const mediationType = workTask.mediation_type ?? null;
        const propertyType = workTask.property_type ?? null;
        // バリデーション
        if (!storageUrl) {
 ...
 */
router.post('/:propertyNumber/write', async (req, res) => {
    try {
        const { propertyNumber } = req.params;
 ...
 */
router.post('/:propertyNumber/extract-keiyaku', async (req, res) => {
    try {
        const { propertyNumber } = req.params;
        // 業務データを取得
        const workTask = await workTaskService.getByPropertyNumber(propertyNumber);
 ...
 */
router.post('/:propertyNumber/write-keiyaku', async (req, res) => {
    try {
        const { propertyNumber } = req.params;
        const { extractResult, sheetName, spreadsheetUrl } = req.body;
        if (!extractResult || !sheetName || !spreadsheetUrl) {
 ...
 */
router.get('/:propertyNumber/list-kodate-pdfs', async (req, res) => {
    try {
        const { propertyNumber } = req.params;
        const workTask = await workTaskService.getByPropertyNumber(propertyNumber);
        if (!workTask)
            return res.status(404).json({ error: '業務データが見つかりません' });
        const storageUrl = workTask.storage_url ?? null;
        const spreadsheetUrl = workTask.spreadsheet_url ?? null;
        const mediationType = workTask.mediation_type ?? null;
        const propertyType = workTask.property_type ?? null;
 ...
 */
router.post('/:propertyNumber/extract-kodate-single', async (req, res) => {
    try {
        const { propertyNumber } = req.params;
        const { fileId, fileName, pdfType } = req.body;
        if (!fileId)
 ...
 */
router.post('/:propertyNumber/extract-kodate', async (req, res) => {
    try {
        const { propertyNumber } = req.params;
        // 業務データを取得
        const workTask = await workTaskService.getByPropertyNumber(propertyNumber);
 ...
 */
router.post('/:propertyNumber/write-kodate', async (req, res) => {
    try {
        const { propertyNumber } = req.params;
        const { extractResult, sheetName, spreadsheetUrl } = req.body;
        if (!extractResult || !sheetName || !spreadsheetUrl) {
 ...
 */
router.get('/:propertyNumber/list-kodate-keiyaku-pdfs', async (req, res) => {
    try {
        const { propertyNumber } = req.params;
        const workTask = await workTaskService.getByPropertyNumber(propertyNumber);
        if (!workTask)
 ...
 */
router.post('/:propertyNumber/extract-kodate-keiyaku-single', async (req, res) => {
    try {
        const { propertyNumber } = req.params;
 ...
 */
router.post('/:propertyNumber/extract-kodate-keiyaku', async (req, res) => {
    try {
        const { propertyNumber } = req.params;
        const workTask = await workTaskService.getByPropertyNumber(propertyNumber);
        if (!workTask) {
 ...
 */
router.post('/:propertyNumber/write-kodate-keiyaku', async (req, res) => {
    try {
        const { propertyNumber } = req.params;
        const { extractResult, sheetName, spreadsheetUrl } = req.body;
        if (!extractResult || !sheetName || !spreadsheetUrl) {
 ...
 */
router.get('/:propertyNumber/list-tochi-pdfs', async (req, res) => {
    try {
        const { propertyNumber } = req.params;
        const workTask = await workTaskService.getByPropertyNumber(propertyNumber);
        if (!workTask) {
        ...
        if (!isTochi)
            return res.status(400).json({ error: 'この機能は土地（種別：土）のみ対応しています' });
 ...
 */
router.post('/:propertyNumber/extract-tochi-single', async (req, res) => {
    try {
        const { propertyNumber } = req.params;
        const { fileId, fileName } = req.body;
        if (!fileId || !fileName) {
 ...
 */
router.post('/:propertyNumber/write-tochi', async (req, res) => {
    try {
        const { propertyNumber } = req.params;
        const { extractResult, sheetName, spreadsheetUrl } = req.body;
        if (!extractResult || !sheetName || !spreadsheetUrl) {
