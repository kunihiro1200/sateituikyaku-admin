 ...
 */
router.post('/call-memos', auth_1.authenticate, async (req, res) => {
    try {
 ...
 */
router.get('/seller/:sellerId', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const spreadsheetComments = [];
        const sellerComments = seller.comments;
        if (sellerComments) {
 ...
 */
router.post('/comment-highlights', auth_1.authenticate, async (req, res) => {
    try {
 ...
 */
router.post('/enhance-email', auth_1.authenticate, async (req, res) => {
    try {
        const { currentBody, previousBodies, mode } = req.body;
        const enhanceMode = mode === 'rewrite' ? 'rewrite' : 'light';
 ...
 */
router.post('/house-maker-info', auth_1.authenticate, async (req, res) => {
    try {
        const { commentText } = req.body;
        if (!commentText || typeof commentText !== 'string' || commentText.trim().length === 0) {
        ...
        const extractCompletion = await axios_1.default.post('https://api.openai.com/v1/chat/completions', {
            model: 'gpt-4o-mini',
            messages: [
                {
                    role: 'system',
...
ハウスメーカーが見つからない場合: {"makerName": null}`,
        ...
        const contentCompletion = await axios_1.default.post('https://api.openai.com/v1/chat/completions', {
            model: 'gpt-4o',
            messages: [
                {
                    role: 'system',
