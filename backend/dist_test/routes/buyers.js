...
const BuyerSyncService_1 = require("../services/BuyerSyncService");
const EmailHistoryService_1 = require("../services/EmailHistoryService");
const RelatedBuyerService_1 = require("../services/RelatedBuyerService");
const uuidValidator_1 = require("../middleware/uuidValidator");
...
const buyerSyncService = new BuyerSyncService_1.BuyerSyncService();
const emailHistoryService = new EmailHistoryService_1.EmailHistoryService();
const buyerLinkageCache = new BuyerLinkageCache_1.BuyerLinkageCache();
// 一覧取得
router.get('/', async (req, res) => {
    try {
        const { page = '1', limit = '50', search, status, assignee, dateFrom, dateTo, sortBy = 'reception_date', sortOrder = 'desc', withStatus, calculatedStatus, statusCategory, viewingMonth, followUpAssignee, } = req.query;
        // calculatedStatus 指定の場合はステータスフィルタリング（withStatusの有無に関わらず）
        if (calculatedStatus) {
            const result = await buyerService.getBuyersByStatus(calculatedStatus, {
...
router.get('/validate-property-number', async (req, res) => {
...
// 買付率統計取得（/:id よりも前に定義）
router.get('/purchase-rate-statistics', auth_1.authenticate, async (_req, res) => {
    try {
...
router.get('/status-categories', async (_req, res) => {
    try {
...
router.get('/status-categories-only', async (_req, res) => {
    try {
...
router.get('/status-categories-with-buyers', async (_req, res) => {
    try {
...
function authenticateOrApiKey(req, res, next) {
    // まずAPI Keyの存在を確認
...
router.put('/batch', authenticateOrApiKey, async (req, res) => {
    try {
...
router.patch('/pinrich-500man-bulk-update', async (req, res) => {
    try {
...
// router.use(authenticate)よりも前に定義する必要がある
router.put('/:id', authenticateOrApiKey, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const syncResult = await buyerService.updateWithSync(buyerNumber, sanitizedData, userId, userEmail, { force: force === 'true' });
        ...
        await (0, BuyerService_1.invalidateBuyerStatusCache)();
...
// 認証不要（公開エンドポイント）
router.post('/update-sidebar-counts', async (_req, res) => {
    try {
...
router.get('/viewing-double-booking-check', async (req, res) => {
    try {
        const { propertyNumber, viewingDate, currentBuyerNumber } = req.query;
...
router.post('/backfill-property-info', authenticateOrApiKey, async (req, res) => {
    try {
        console.log('[POST /buyers/backfill-property-info] Starting backfill...');
        const supabase = buyerService.supabase;
        ...
        let updated = 0;
        let skipped = 0;
        await buyerService.initSyncServices();
        const writeService = buyerService.writeService;
...
router.post('/repair-property-numbers', authenticateOrApiKey, async (req, res) => {
    try {
...
router.post('/radius-search', auth_1.authenticate, async (req, res) => {
    try {
...
router.post('/restore-to-sheet', authenticateOrApiKey, async (req, res) => {
    try {
        const buyerNumbers = req.body?.buyerNumbers || [];
        const dryRun = req.body?.dryRun === true || req.body?.dryRun === 'true';
...
router.post('/get-sheet-buyer-numbers', authenticateOrApiKey, async (_req, res) => {
    try {
        const { google } = await Promise.resolve().then(() => __importStar(require('googleapis')));
        const pathMod = await Promise.resolve().then(() => __importStar(require('path')));
        const spreadsheetId = process.env.GOOGLE_SHEETS_BUYER_SPREADSHEET_ID;
...
router.get('/next-buyer-number', async (_req, res) => {
    try {
        const buyerNumber = await buyerService.generateBuyerNumber();
...
router.get('/search', async (req, res) => {
    try {
        ...
        const results = await buyerService.search(q, parseInt(limit, 10));
        res.json(results);
    }
    catch (error) {
...
// 過去買主番号取得
router.get('/:id/past-buyer-numbers', async (req, res) => {
    try {
        const { id } = req.params;
...
// 完全な問い合わせ履歴取得（旧形式）
router.get('/:id/inquiry-history-legacy', async (req, res) => {
    try {
        const { id } = req.params;
...
// 問い合わせ履歴取得（買主詳細ページ用）
router.get('/:id/inquiry-history', async (req, res) => {
    try {
        const { id } = req.params;
        // UUIDかどうかで判定
        const isUuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(id);
...
// メール送信履歴を保存
router.post('/:buyerId/email-history', async (req, res) => {
    try {
...
// メール送信履歴を取得
router.get('/:buyerId/email-history', async (req, res) => {
    try {
        const { buyerId } = req.params;
...
router.get('/:id/related', async (req, res) => {
    ...
    try {
        ...
        // 関連買主を検索
        const relatedBuyers = await RelatedBuyerService_1.relatedBuyerService.findRelatedBuyers(buyerId);
...
// 統合問合せ履歴を取得
router.get('/:id/unified-inquiry-history', (0, uuidValidator_1.uuidValidationMiddleware)('id'), async (req, res) => {
    const startTime = Date.now();
    const { id } = req.params;
    try {
        ...
        // 統合問合せ履歴を取得
        const inquiries = await RelatedBuyerService_1.relatedBuyerService.getUnifiedInquiryHistory(allBuyerIds);
        // 買主番号のリストを作成
        const buyerNumbers = [
...
// 競合解決エンドポイント
router.post('/:id/conflict', async (req, res) => {
    try {
        const { id } = req.params;
        ...
        if (resolution === 'db_wins') {
            ...
            // force=trueで更新を実行
            const result = await buyerService.updateWithSync(id, updateData, userId, userEmail, { force: true });
            return res.json({
                success: true,
                buyer: result.buyer,
...
// 近隣物件取得（類似物件）
router.get('/:buyerNumber/nearby-properties', async (req, res) => {
    try {
        ...
        const result = await buyerService.getNearbyProperties(propertyNumber);
        res.json(result);
    }
    catch (error) {
...
// 他社物件新着配信用の買主取得
router.get('/other-company-distribution', auth_1.authenticate, async (req, res) => {
    try {
...
router.post('/scrape-property', auth_1.authenticate, async (req, res) => {
    try {
        const { url } = req.body;
        if (!url) {
...
router.get('/cron/scrape-server-ping', async (req, res) => {
    try {
        const scrapeApiUrl = process.env.SCRAPE_API_URL || 'https://sateituikyaku-scrape-server-production.up.railway.app';
    ...
    try {
        ...
        if (buyerBeforeDelete && buyerBeforeDelete.latest_status?.includes('買') && buyerBeforeDelete.property_number) {
            ...
            const { data: remainingBuyers } = await supabase
                ...
                .is('deleted_at', null);
            const hasOtherPurchaseBuyer = (remainingBuyers || []).some((b) => b.latest_status?.includes('買'));
...
// 買主を物理削除（完全削除）
router.delete('/:id/permanent', auth_1.authenticate, async (req, res) => {
    try {
        const { id } = req.params;
...
router.post('/:buyerNumber/call-history', async (req, res) => {
    try {
        const { buyerNumber } = req.params;
        ...
        await activityLogService.logActivity({
            employeeId: req.employee?.id || SYSTEM_EMPLOYEE_ID,
            action: 'phone_call',
            targetType: 'buyer',
...
router.post('/:buyerNumber/sms-history', async (req, res) => {
    try {
        const { buyerNumber } = req.params;
        ...
        const activityLogService = new ActivityLogService();
        await activityLogService.logActivity({
            employeeId: req.user?.id || SYSTEM_EMPLOYEE_ID,
            action: 'sms',
            targetType: 'buyer',
            targetId: buyerNumber,
            metadata: {
...
router.post('/:buyer_number/send-offer-chat', async (req, res) => {
    try {
        ...
        const OFFER_WEBHOOK_URL = 'https://chat.googleapis.com/v1/spaces/AAAA6iEDkiU/messages?key=AIzaSyDdI0hCZtE6vySjMm-WEfRq3CPzqKqqsHI&token=azlyf21pENCpLLUdJPjnRNXOzsIAP550xebOMVxYRMQ';
...
router.post('/:buyer_number/send-confirmation', async (req, res) => {
    try {
        const { buyer_number } = req.params;
...
${detailUrl}`;
        ...
        try {
            ...
            const syncResult = await buyerService.updateWithSync(buyer_number, { confirmation_to_assignee: confirmationText }, 'system', 'system@example.com', { force: true });
