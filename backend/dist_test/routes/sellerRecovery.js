...
const EnhancedAutoSyncService_1 = require("../services/EnhancedAutoSyncService");
 ...
 */
router.post('/:sellerNumber/recover', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const userEmail = req.user?.email || req.employee?.email || 'unknown';
        const recoveryUser = recoveredBy || userEmail;
        console.log(`🔄 Recovery request for ${sellerNumber} by ${recoveryUser}`);
        // Initialize sync service
        const syncService = new EnhancedAutoSyncService_1.EnhancedAutoSyncService(supabaseUrl, supabaseServiceKey);
 ...
 */
router.get('/:sellerNumber/recovery-status', auth_1.authenticate, async (req, res) => {
    try {
        const { sellerNumber } = req.params;
        // Validate seller number format
        if (!sellerNumber || !sellerNumber.match(/^AA\d+$/)) {
