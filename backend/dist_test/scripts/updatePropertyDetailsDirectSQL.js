...
async function updatePropertyDetailsDirectSQL() {
    ...
    try {
        ...
        // テスト用に1件だけ取得
        const result = await client.query(`SELECT id, property_number, property_type, storage_location 
       FROM property_listings 
       WHERE id = $1 
       LIMIT 1`, ['65909b23-beb3-445e-aa6f-4116e2c02ef9']);
        ...
        for (const property of properties) {
            try {
                ...
                const [propertyAbout, recommendedComments, athomeData, favoriteComment] = await Promise.all([
                    ...
                    // おすすめコメント
                    recommendedCommentService.getRecommendedComment(property.property_number, property.property_type, property.id).then(result => result.comments).catch(err => {
                        console.error(`  recommended_comments取得エラー: ${err.message}`);
                        return [];
                    }),
                    // Athome情報
                    athomeDataService.getAthomeData(property.property_number, property.property_type, property.storage_location).then(result => result.data).catch(err => {
                ...
                // 直接SQLでデータベースに保存
                await client.query(`UPDATE property_listings
           SET 
             property_about = $1,
