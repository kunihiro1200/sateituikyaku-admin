...
exports.FiscalYearUtils = void 0;
class FiscalYearUtils {
     ...
     * @returns 年度の開始日と終了日
     */
    static getFiscalYearPeriod(date) {
     ...
     */
    static getFiscalYearMonthRange(year, month) {
        if (month < 1 || month > 12) {
     ...
     */
    static getPreviousFiscalYearRange(year, month) {
        if (month < 1 || month > 12) {
            throw new Error('Month must be between 1 and 12');
        }
        // 現在の年度期間を取得
        ...
        // 前年度の開始日（1年前の10月1日）
        const previousStartDate = new Date(currentFiscalPeriod.startDate.getFullYear() - 1, this.FISCAL_YEAR_START_MONTH, 1);
        // 前年度の同じ月の末日
     ...
     */
    static getMonthsElapsedInFiscalYear(year, month) {
        if (month < 1 || month > 12) {
            throw new Error('Month must be between 1 and 12');
        }
        const targetDate = new Date(year, month - 1, 1);
...
exports.FiscalYearUtils = FiscalYearUtils;
/**
 * 会社の年度開始月（10月 = 9、0-indexed）
 */
FiscalYearUtils.FISCAL_YEAR_START_MONTH = 9; // October (0-indexed)
