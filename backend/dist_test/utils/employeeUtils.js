 ...
 * 従業員関連のユーティリティ関数
 */
class EmployeeUtils extends BaseRepository_1.BaseRepository {
    /**
     * 有効な社員でメールアドレスが存在するものを取得
     * @returns 有効な社員の配列
     */
    async getActiveEmployeesWithEmail() {
        try {
     ...
     */
    async getEmployeeIdByInitials(initials) {
     ...
     */
    async getEmployeeByInitials(initials) {
        ...
        try {
            ...
            if (directMatches && directMatches.length > 0) {
                if (directMatches.length > 1) {
                    const names = directMatches.map((e) => e.name).join(', ');
            ...
            if (matchedEmployees.length === 0) {
                ...
                if (nameMatches.length === 1) {
                    ...
                    const names = nameMatches.map((e) => e.name).join(', ');
     ...
     */
    async validateEmployeeForCalendar(employeeId) {
...
exports.EmployeeUtils = EmployeeUtils;
