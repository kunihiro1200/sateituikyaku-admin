...
exports.isDaysFromToday = isDaysFromToday;
exports.isTodayOrPast = isTodayOrPast;
exports.isWithinDaysAgo = isWithinDaysAgo;
 ...
 */
function isDaysFromToday(date, days) {
    if (!date)
        return false;
    const targetDate = parseDateLocal(date);
    if (isNaN(targetDate.getTime()))
 ...
 */
function isWithinDaysAgo(date, minDaysAgo, maxDaysAgo) {
    if (!date)
        return false;
    const targetDate = parseDateLocal(date);
    if (isNaN(targetDate.getTime()))
 ...
 */
function isAfterOrEqual(date, compareDate) {
    if (!date)
        return false;
    const targetDate = parseDateLocal(date);
    if (isNaN(targetDate.getTime()))
