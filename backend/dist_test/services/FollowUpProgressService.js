...
exports.followUpProgressService = exports.FollowUpProgressService = void 0;
...
class FollowUpProgressService {
     ...
     */
    async markUnreachable(sellerId, reason) {
        try {
            const updateData = {
                unreachable: true,
            ...
            const { data, error } = await supabase
                ...
                .update(updateData)
                .eq('id', sellerId)
                .select(`unreachable, second_call_after_unreachable, next_call_date, confidence,
     ...
     * @returns Updated follow-up information
     */
    async scheduleNextCall(sellerId, nextCallDate, notes) {
        try {
            const updateData = {
     ...
     * @returns Updated follow-up information
     */
    async updateContactMethod(sellerId, method) {
        try {
     ...
     * @returns Updated follow-up information
     */
    async updatePreferredContactTime(sellerId, time) {
        try {
            const { data, error } = await supabase
                .from('sellers')
     ...
     * @returns Updated follow-up information
     */
    async recordFirstCall(sellerId, initials, person) {
        try {
            const { data, error } = await supabase
                .from('sellers')
                .update({
     ...
     */
    async getFollowUpProgress(sellerId) {
        try {
            const { data, error } = await supabase
                .from('sellers')
                .select(`unreachable, second_call_after_unreachable, next_call_date, confidence,
     ...
     */
    async getSellersRequiringFollowUp(assigneeId, date = new Date()) {
        try {
            let query = supabase
                .from('sellers')
                .select(`id, seller_number, name, phone_number, next_call_date, confidence,
     ...
     */
    async getUnreachableSellers(assigneeId) {
        try {
            let query = supabase
                .from('sellers')
                .select(`id, seller_number, name, phone_number, unreachable, 
     ...
     */
    mapToFollowUpProgressInfo(data) {
        return {
...
exports.FollowUpProgressService = FollowUpProgressService;
// Export singleton instance
exports.followUpProgressService = new FollowUpProgressService();
