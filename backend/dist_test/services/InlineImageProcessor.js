...
class InlineImageProcessor {
    ...
    processHtmlWithImages(html) {
        ...
        if (estimatedTotalSize > InlineImageProcessor.MAX_TOTAL_EMAIL_SIZE) {
            throw new Error(`合計画像サイズが10MBを超えています: ${(estimatedTotalSize / 1024 / 1024).toFixed(2)}MB / 10MB`);
        }
     ...
     */
    replaceDataUrlsWithCids(html, images) {
        let processedHtml = html;
     ...
     */
    generateHtmlWithInlineImages(body, images) {
        ...
        if (hasEmbeddedImages) {
            ...
            processedBody = processedBody.replace(/<img([^>]*)src="data:image\/[^"]+"([^>]*)>/gi, (match, before, after) => {
...
exports.InlineImageProcessor = InlineImageProcessor;
InlineImageProcessor.DATAURL_SIZE_THRESHOLD = 2 * 1024 * 1024; // 2MB
InlineImageProcessor.MAX_INDIVIDUAL_IMAGE_SIZE = 5 * 1024 * 1024; // 5MB
InlineImageProcessor.MAX_TOTAL_EMAIL_SIZE = 10 * 1024 * 1024; // 10MB
InlineImageProcessor.BASE64_OVERHEAD = 1.33; // Base64 is ~33% larger
