...
exports.EnhancedPeriodicSyncManager = exports.EnhancedAutoSyncService = void 0;
exports.getEnhancedAutoSyncService = getEnhancedAutoSyncService;
exports.getEnhancedPeriodicSyncManager = getEnhancedPeriodicSyncManager;
...
class EnhancedAutoSyncService {
    constructor(supabaseUrl, supabaseKey) {
        ...
        this.buyerSheetsClient = null;
        this.buyerColumnMapper = null;
        this.isBuyerInitialized = false;
        // スプレッドシートキャッシュ（Google Sheets APIクォータ対策）
        this.spreadsheetCache = null;
        this.spreadsheetCacheExpiry = 0;
        this.SPREADSHEET_CACHE_TTL = 5 * 60 * 1000; // 5分間キャッシュ（Google Sheets APIクォータ対策）
        // 買主スプレッドシートキャッシュ
     ...
     */
    async getSpreadsheetData(forceRefresh = false) {
        const now = Date.now();
     ...
     */
    async getAllDbSellerNumbers() {
        const allSellerNumbers = new Set();
        const pageSize = 1000;
        let offset = 0;
        let hasMore = true;
        while (hasMore) {
     ...
     * ページネーション対応で全件取得
     */
    async getAllActiveDbSellerNumbers() {
        const allSellerNumbers = new Set();
        const pageSize = 1000;
        let offset = 0;
        let hasMore = true;
     ...
     */
    async validateDeletion(sellerNumber) {
        try {
            // 売主情報を取得
     ...
     */
    async executeSoftDelete(sellerNumber) {
        try {
     ...
     * 削除された売主を一括同期
     */
    async syncDeletedSellers(sellerNumbers) {
        const startedAt = new Date();
        const deletedSellerNumbers = [];
        const manualReviewSellerNumbers = [];
        const errors = [];
     ...
     * @returns 復元結果
     */
    async recoverDeletedSeller(sellerNumber, recoveredBy = 'manual') {
        try {
     ...
     * 不足している売主を同期
     */
    async syncMissingSellers(sellerNumbers) {
        ...
        const allRows = await this.getSpreadsheetData();
        const rowsBySellerNumber = new Map();
        for (const row of allRows) {
            const sellerNumber = row['売主番号'];
            if (sellerNumber) {
     ...
     * 既存売主のデータを更新
     */
    async syncUpdatedSellers(sellerNumbers) {
        const startTime = new Date();
        const errors = [];
    ...
    async detectUpdatedSellers() {
        ...
        // スプレッドシートから全データを取得（キャッシュ対応）
        const allRows = await this.getSpreadsheetData();
        const sheetDataBySellerNumber = new Map();
        for (const row of allRows) {
            const sellerNumber = row['売主番号'];
            // 🚨 修正: 任意の2文字アルファベットプレフィックス + 数字の形式に対応（AA12903、FI123、BB456等）
            if (sellerNumber && typeof sellerNumber === 'string' && /^[A-Z]{2}\d+$/.test(sellerNumber)) {
     ...
     */
    formatInquiryDate(inquiryYear, inquiryDate) {
     ...
     */
    combineVisitDateAndTime(visitDate, visitTime) {
     ...
     * 単一の売主を更新
     */
    async updateSingleSeller(sellerNumber, row) {
        ...
        if (mappedData.name && mappedData.name.trim() !== '') {
            updateData.name = (0, encryption_1.encrypt)(mappedData.name);
        }
        if (mappedData.address && mappedData.address.trim() !== '') {
            updateData.address = (0, encryption_1.encrypt)(mappedData.address);
        }
        if (mappedData.phone_number && mappedData.phone_number.trim() !== '') {
            updateData.phone_number = (0, encryption_1.encrypt)(mappedData.phone_number);
            // 重複検出用ハッシュ（SHA-256）
        ...
        if (mappedData.email && mappedData.email.trim() !== '') {
            updateData.email = (0, encryption_1.encrypt)(mappedData.email);
            // 重複検出用ハッシュ（SHA-256）
        ...
        // 除外日を計算（inquiry_site または inquiry_date が存在する場合）
        {
            ...
            if (siteForCalc && dateForCalc) {
                const exclusionDate = ExclusionDateCalculator_1.ExclusionDateCalculator.calculateExclusionDate(dateForCalc, siteForCalc);
     ...
     * 単一の売主を同期
     */
    async syncSingleSeller(sellerNumber, row) {
        const mappedData = this.columnMapper.mapToDatabase(row);
        // 査定額を取得（手入力優先、なければ自動計算）
        // 🚨 重要: 種別（土地/戸建て/マンション）に関係なく、手動入力査定額を最優先で使用
        const valuation1 = row['査定額1'] || row['査定額1（自動計算）v'];
     ...
     */
    async ensurePropertyCreated(sellerId, sellerNumber, row) {
        const propertyAddress = row['物件所在地'] || '';
        ...
        const result = await this.propertySyncHandler.syncProperty(sellerId, propertyData, propertyNumber);
        if (!result.success) {
     ...
     */
    async logPropertyCreationError(sellerNumber, errorMessage) {
        try {
            await this.supabase
                .from('sync_logs')
                .insert({
    ...
    async syncPropertyListingUpdates() {
        ...
        try {
            ...
            const { PropertyListingSyncService } = await Promise.resolve().then(() => __importStar(require('./PropertyListingSyncService')));
     ...
     */
    async syncHiddenPropertyListings() {
        const startTime = Date.now();
        try {
            ...
            const toDelete = dbPropertyNumbers.filter(pn => !spreadsheetPropertyNumbers.has(pn));
    ...
    async syncMissingPropertyDetails() {
        ...
        try {
            ...
            const propertyDetailsNumbers = new Set();
            const emptyCommentsPropertyNumbers = new Set(); // コメントデータが空の物件
            offset = 0;
            hasMore = true;
            while (hasMore) {
            ...
            for (let i = 0; i < missingPropertyNumbers.length; i += BATCH_SIZE) {
                ...
                for (const propertyNumber of batch) {
                    try {
                        ...
                        const syncSuccess = await athomeSheetSyncService.syncPropertyComments(propertyNumber, property.property_type);
     ...
     * 更新同期と削除同期も含む
     */
    async runFullSync(triggeredBy = 'scheduled', clearCache = false) {
        const startTime = new Date();
    ...
    async initializeBuyer() {
        ...
        try {
            ...
            this.buyerSheetsClient = new GoogleSheetsClient_1.GoogleSheetsClient(sheetsConfig);
            await this.buyerSheetsClient.authenticate();
            // BuyerColumnMapper を動的にインポート
            const { BuyerColumnMapper } = await Promise.resolve().then(() => __importStar(require('./BuyerColumnMapper')));
     ...
     */
    async getBuyerSpreadsheetData(forceRefresh = false) {
        const now = Date.now();
     ...
     */
    async getAllDbBuyerNumbers() {
        const allBuyerNumbers = new Set();
        const pageSize = 1000;
        let offset = 0;
        let hasMore = true;
        while (hasMore) {
     ...
     * 更新が必要な買主を検出
     */
    async detectUpdatedBuyers() {
        if (!this.isBuyerInitialized || !this.buyerSheetsClient) {
            await this.initializeBuyer();
        }
        console.log('🔍 Detecting updated buyers (full field comparison)...');
        const allRows = await this.getBuyerSpreadsheetData();
        const sheetDataByBuyerNumber = new Map();
        for (const row of allRows) {
            const buyerNumber = row['買主番号'];
            if (buyerNumber !== null && buyerNumber !== undefined && buyerNumber !== '') {
                const buyerNumberStr = String(buyerNumber).trim();
        ...
        while (hasMore) {
            ...
            if (!dbBuyers || dbBuyers.length === 0) {
                ...
                for (const dbBuyer of dbBuyers) {
                    ...
                    const mappedSheetData = this.buyerColumnMapper.mapSpreadsheetToDatabase(Object.keys(sheetRow), Object.values(sheetRow));
                    let needsUpdate = false;
                    for (const [dbField, sheetValue] of Object.entries(mappedSheetData)) {
                        if (skipFields.has(dbField))
                            continue;
     ...
     * 不足している買主を同期
     */
    async syncMissingBuyers(buyerNumbers) {
        const startTime = new Date();
        const errors = [];
        ...
        const allRows = await this.getBuyerSpreadsheetData();
        const rowsByBuyerNumber = new Map();
        for (const row of allRows) {
            const buyerNumber = row['買主番号'];
            if (buyerNumber) {
     ...
     * 既存買主のデータを更新
     */
    async syncUpdatedBuyers(buyerNumbers) {
        const startTime = new Date();
        const errors = [];
     ...
     * 単一買主の新規同期
     */
    async syncSingleBuyer(buyerNumber, row) {
        const mappedData = this.buyerColumnMapper.mapSpreadsheetToDatabase(Object.keys(row), Object.values(row));
     ...
     */
    async updateSingleBuyer(buyerNumber, row) {
        const mappedData = this.buyerColumnMapper.mapSpreadsheetToDatabase(Object.keys(row), Object.values(row));
        const updateData = {
            ...mappedData,
            updated_at: new Date().toISOString(),
     ...
     */
    async getAllActiveBuyerNumbers() {
        const allBuyerNumbers = new Set();
        const pageSize = 1000;
        let offset = 0;
        let hasMore = true;
     ...
     * 削除前のバリデーション（買主用）
     */
    async validateBuyerDeletion(buyerNumber) {
        try {
     ...
     */
    async executeBuyerHardDelete(buyerNumber) {
        try {
     ...
     * 削除された買主を一括同期
     */
    async syncDeletedBuyers(buyerNumbers) {
        const startedAt = new Date();
        const deletedBuyerNumbers = [];
        const manualReviewBuyerNumbers = [];
        const errors = [];
     ...
     * 削除された買主を復元
     */
    async recoverDeletedBuyer(buyerNumber, recoveredBy = 'manual') {
        try {
...
exports.EnhancedAutoSyncService = EnhancedAutoSyncService;
// シングルトンインスタンス
let enhancedAutoSyncServiceInstance = null;
function getEnhancedAutoSyncService() {
    if (!enhancedAutoSyncServiceInstance) {
        enhancedAutoSyncServiceInstance = new EnhancedAutoSyncService(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_KEY);
    }
 ...
 */
class EnhancedPeriodicSyncManager {
    ...
    async runSync() {
        try {
            const { getSyncHealthChecker } = await Promise.resolve().then(() => __importStar(require('./SyncHealthChecker')));
...
exports.EnhancedPeriodicSyncManager = EnhancedPeriodicSyncManager;
// 強化版定期同期マネージャーのシングルトン
let enhancedPeriodicSyncManagerInstance = null;
function getEnhancedPeriodicSyncManager(intervalMinutes) {
    if (!enhancedPeriodicSyncManagerInstance) {
        enhancedPeriodicSyncManagerInstance = new EnhancedPeriodicSyncManager(intervalMinutes || parseInt(process.env.AUTO_SYNC_INTERVAL_MINUTES || '5', 10));
    }
