...
class ConflictResolver {
    constructor(writeService, columnMapper) {
     ...
     * @returns 競合チェック結果
     */
    async checkConflict(buyerNumber, dbValues, expectedSpreadsheetValues, lastSyncedAt) {
        const conflicts = [];
        // スプレッドシートの現在の行データを取得
     ...
     * @returns 書き込み結果
     */
    async forceOverwrite(buyerNumber, dbValues) {
        try {
     ...
     * @returns 解決結果
     */
    async resolveConflict(buyerNumber, conflicts, resolution) {
        try {
     ...
     * @returns 競合があるかどうか
     */
    async checkFieldConflict(buyerNumber, dbFieldName, expectedValue) {
