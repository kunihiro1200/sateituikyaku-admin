...
exports.SyncMetricsCollector = void 0;
...
class SyncMetricsCollector {
     ...
     * 同期メトリクスを記録
     */
    async recordSyncMetrics(metrics) {
        const records = [];
        // 成功率
        ...
        // サーキットブレーカー状態
        const cbStateValue = metrics.circuitBreakerState === 'closed' ? 0 :
        ...
        for (const metric of metricsData || []) {
            if (metric.metric_type === 'sync_errors_by_type' && metric.metadata?.error_type) {
                ...
                if (errorType in errorsByType) {
                    errorsByType[errorType] += metric.metric_value;
                }
                else {
...
exports.SyncMetricsCollector = SyncMetricsCollector;
