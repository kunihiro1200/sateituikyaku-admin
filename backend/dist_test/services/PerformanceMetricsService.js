...
class PerformanceMetricsService extends BaseRepository_1.BaseRepository {
    /**
     * 拡張されたメトリクス計算（月平均を含む）
     */
    async calculateEnhancedMetrics(month) {
        const [year, monthNum] = month.split('-').map(Number);
        // 現在月のメトリクスを並列計算
        const [visitAppraisalRateResult, exclusiveContracts, competitorLossUnvisited, competitorLossVisited] = await Promise.all([
        ...
        const [visitAppraisalRateAvg, exclusiveContractsRateAvg, competitorLossUnvisitedAvg, competitorLossVisitedAvg, competitorLossUnvisitedPrevYearAvg, competitorLossVisitedPrevYearAvg, exclusiveByRepWithAvg, competitorLossVisitedByRepWithAvg,] = await Promise.all([
            this.calculateFiscalYearMonthlyAverage(year, monthNum, async (y, m) => {
            ...
            this.calculatePreviousYearMonthlyAverage(year, monthNum, async (y, m) => {
                const result = await this.calculateCompetitorLossUnvisited(y, m);
                return result.rate;
            }),
            this.calculatePreviousYearMonthlyAverage(year, monthNum, async (y, m) => {
            ...
            this.calculateRepresentativeMonthlyAverages(year, monthNum, exclusiveContracts.byRepresentative, (y, m) => this.calculateExclusiveContracts(y, m)),
            this.calculateRepresentativeMonthlyAverages(year, monthNum, competitorLossVisited.byRepresentative, (y, m) => this.calculateCompetitorLossVisited(y, m)),
        ]);
        return {
            month,
            visitAppraisalRate: {
     ...
     * 年度内の月平均を計算
     */
    async calculateFiscalYearMonthlyAverage(year, month, calculateMonthMetric) {
        const monthRange = FiscalYearUtils_1.FiscalYearUtils.getFiscalYearMonthRange(year, month);
     ...
     * 前年度同期間の月平均を計算
     */
    async calculatePreviousYearMonthlyAverage(year, month, calculateMonthMetric) {
        const prevYearRange = FiscalYearUtils_1.FiscalYearUtils.getPreviousFiscalYearRange(year, month);
        const monthsElapsed = FiscalYearUtils_1.FiscalYearUtils.getMonthsElapsedInFiscalYear(year, month);
     ...
     * 担当者別の月平均を計算
     */
    async calculateRepresentativeMonthlyAverages(year, month, currentRepMetrics, calculateMonthMetric) {
        const monthRange = FiscalYearUtils_1.FiscalYearUtils.getFiscalYearMonthRange(year, month);
        const monthsElapsed = monthRange.monthsElapsed;
        const startYear = monthRange.startMonth.getFullYear();
        const startMonth = monthRange.startMonth.getMonth() + 1; // 1-indexed
     ...
     */
    async calculateVisitAppraisalCount(year, month) {
     ...
     * 訪問査定取得割合を計算
     */
    async calculateVisitAppraisalRate(year, month) {
     ...
     */
    async calculateExclusiveContracts(year, month) {
        // Use UTC dates to avoid timezone issues
        const startDate = new Date(Date.UTC(year, month - 1, 1)).toISOString();
        const endDate = new Date(Date.UTC(year, month, 0, 23, 59, 59, 999)).toISOString();
     ...
     */
    async calculateCompetitorLossUnvisited(year, month) {
        // Use UTC dates to avoid timezone issues
        const startDate = new Date(Date.UTC(year, month - 1, 1)).toISOString();
        const endDate = new Date(Date.UTC(year, month, 0, 23, 59, 59, 999)).toISOString();
        // 未訪問で他決になった件数
     ...
     * 結果: 1 ÷ 8 ≈ 12.5%
     */
    async calculateCompetitorLossVisited(year, month) {
        // Use UTC dates to avoid timezone issues
        const startDate = new Date(Date.UTC(year, month - 1, 1)).toISOString();
        const endDate = new Date(Date.UTC(year, month, 0, 23, 59, 59, 999)).toISOString();
        // 営担別の訪問済み他決件数を取得（分子）
