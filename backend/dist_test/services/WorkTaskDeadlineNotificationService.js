...
exports.WorkTaskDeadlineNotificationService = void 0;
const supabase_js_1 = require("@supabase/supabase-js");
const EmailService_supabase_1 = require("./EmailService.supabase");
// ============================================================
// 日付ヘルパー（バックエンド用）
// ============================================================
const isNotBlank = (v) => v !== null && v !== undefined && v !== '';
const isBlank = (v) => !isNotBlank(v);
const parseDate = (s) => {
    if (!s)
        return null;
    const d = new Date(s);
...
function calcStatus(task) {
    ...
    if (isNotBlank(task.sales_contract_deadline) &&
    ...
    if (isBlank(task.site_registration_requestor) &&
        isBlank(task.on_hold) &&
    ...
    if (dateLte(task.settlement_date, t) &&
    ...
    if (isNotBlank(task.settlement_completed_chat) &&
        (isBlank(task.accounting_confirmed) || task.accounting_confirmed === '未') &&
        isNotBlank(task.sales_contract_deadline)) {
        return '入金確認未';
    }
    if (isBlank(task.ledger_created) &&
        isBlank(task.on_hold) &&
        isNotBlank(task.settlement_date) &&
    ...
    if (isBlank(task.site_registration_confirm_request_date) &&
        isBlank(task.sales_contract_deadline) &&
        isNotBlank(task.site_registration_deadline) &&
 ...
 */
function isDeadlineToday(status) {
 ...
 */
class WorkTaskDeadlineNotificationService {
    constructor() {
...
exports.WorkTaskDeadlineNotificationService = WorkTaskDeadlineNotificationService;
