...
class EmailTemplateService {
    ...
    async getTemplates() {
        try {
            const client = new GoogleSheetsClient_1.GoogleSheetsClient({
     ...
     * 物件詳細画面のEmail送信ボタン向け
     */
    async getPropertyNonReportTemplates() {
     ...
     */
    mergePlaceholders(template, context) {
    ...
    createDataMap(context) {
        ...
        if (context.property) {
            ...
            for (const [key, value] of Object.entries(context.property)) {
                if (!['propertyNumber', 'propertyAddress', 'price'].includes(key))
                    dataMap[key] = value;
            }
        }
    ...
    validatePlaceholders(template, context) {
        const dataMap = this.createDataMap(context);
        const missing = [];
    ...
    mergeMultipleProperties(template, buyer, properties) {
        let subject = template.subject;
        let body = template.body;
        const buyerMap = { buyerName: buyer.buyerName, email: buyer.email, ...buyer };
     ...
     */
    mergeAngleBracketPlaceholders(text, buyer, properties, staffInfo) {
        let result = text;
        ...
        if (properties.length === 0) {
            ...
            if (latestViewingDate0) {
                ...
                if (dateMatch0)
                    formattedDate0 = `${parseInt(dateMatch0[2])}月${parseInt(dateMatch0[3])}日`;
            }
            let formattedTime0 = viewingTime0;
            if (viewingTime0) {
                const timeMatch0 = viewingTime0.match(/(\d{1,2}):(\d{2})/);
                if (timeMatch0)
                    formattedTime0 = `${parseInt(timeMatch0[1])}時${timeMatch0[2]}分`;
            }
     ...
     */
    mergePropertyTemplate(text, property, sellerName, staffInfo) {
        let result = text;
        // <<●所有者情報>> → 売主氏名 + 様
