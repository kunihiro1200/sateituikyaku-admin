...
class AreaMapConfigService {
    ...
    async loadAreaMaps() {
        ...
        try {
                    ...
                    try {
                        ...
                        if (coords) {
                            ...
                            this.supabase
                                ...
                                .catch((saveError) => this.logWarning('Failed to save coordinates to DB (non-critical)', {
                                areaNumber: config.areaNumber,
            ...
            const configs = results.filter((c) => c !== null);
            // キャッシュに登録
     ...
     * 特定のエリア番号の座標を取得
     */
    async getCoordinatesForArea(areaNumber) {
        try {
     ...
     * エリア番号の設定を取得
     */
    async getAreaConfig(areaNumber) {
        try {
            const configs = await this.loadAreaMaps();
            const config = configs.find(c => c.areaNumber === areaNumber);
            if (!config) {
    ...
    async healthCheck() {
        try {
            ...
            const expectedAreas = ['①', '②', '③', '④', '⑥', '⑦', '⑧', '⑨', '⑩', '⑪', '⑫', '⑬', '⑭', '⑮', '㊵', '㊶'];
            const missingAreas = expectedAreas.filter(area => !configs.some(c => c.areaNumber === area));
            ...
            const configsWithoutCoords = configs.filter(c => !c.cityName && !c.coordinates);
            if (configsWithoutCoords.length > 0) {
                errors.push(`Configurations without coordinates: ${configsWithoutCoords.map(c => c.areaNumber).join(', ')}`);
            }
            return {
...
exports.AreaMapConfigService = AreaMapConfigService;
