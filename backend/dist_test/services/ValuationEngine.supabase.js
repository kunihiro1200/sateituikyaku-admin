...
exports.ValuationEngine = void 0;
const BaseRepository_1 = require("../repositories/BaseRepository");
const types_1 = require("../types");
const cache_1 = require("../utils/cache");
class ValuationEngine extends BaseRepository_1.BaseRepository {
    constructor() {
        super(...arguments);
        // 基準価格（平方メートルあたり、円）
        this.BASE_PRICES = {
        ...
        // 築年数による減価率（年あたり）
        this.DEPRECIATION_RATE = 0.015; // 1.5%/年
        // 異常値の閾値（平均からの乖離率）
        this.ANOMALY_THRESHOLD = 3.0;
    }
    /**
     * 査定額を計算（査定額1、2、3を算出）
     */
    async calculateValuation(sellerId, property) {
        // 戸建てまたは土地の場合のみ自動計算
        if (property.propertyType !== types_1.PropertyType.DETACHED_HOUSE &&
            property.propertyType !== types_1.PropertyType.LAND) {
        ...
        // 立地による補正（都道府県・市区町村）
        const locationAdjusted = this.applyLocationAdjustment(depreciatedPrice, property.prefecture, property.city);
        // 設備による補正
        ...
        // 計算根拠
        const calculationBasis = this.generateCalculationBasis(property, basePrice, depreciatedPrice, locationAdjusted, valuation1, valuation2, valuation3);
        // データベースに保存
        const valuationData = {
            seller_id: sellerId,
        ...
        // キャッシュをクリア
        const cacheKey = cache_1.CacheHelper.generateKey('valuations', sellerId);
     ...
     * 査定履歴を取得
     */
    async getValuationHistory(sellerId) {
        // キャッシュをチェック
        const cacheKey = cache_1.CacheHelper.generateKey('valuations', sellerId);
        ...
        // キャッシュに保存
        await cache_1.CacheHelper.set(cacheKey, result, cache_1.CACHE_TTL.VALUATION);
        return result;
    }
    /**
     * 基本価格を計算
     */
    calculateBasePrice(property) {
     ...
     * 築年数による減価を適用
     */
    applyDepreciation(price, buildYear) {
        if (!buildYear) {
            return price;
        }
     ...
     * 立地による補正
     */
    applyLocationAdjustment(price, prefecture, city) {
     ...
     * 設備による補正
     */
    applyFacilityAdjustment(price, property) {
        let adjustmentFactor = 1.0;
        // 駐車場あり
        if (property.parking) {
     ...
     * 警告メッセージを生成
     */
    generateWarnings(price, property, isAnomalous) {
        const warnings = [];
        if (isAnomalous) {
     ...
     * 計算根拠を生成
     */
    generateCalculationBasis(property, basePrice, depreciatedPrice, locationAdjusted, valuation1, valuation2, valuation3) {
        const lines = [];
    ...
    getPropertyTypeName(type) {
        const names = {
            [types_1.PropertyType.DETACHED_HOUSE]: '戸建て',
            [types_1.PropertyType.APARTMENT]: 'マンション',
            [types_1.PropertyType.LAND]: '土地',
            [types_1.PropertyType.COMMERCIAL]: '商業用',
        };
     ...
     * @returns 3つの査定額
     */
    async calculateMultipleValuations(sellerId, property, fixedAssetTaxRoadPrice) {
        // マンションの場合は手入力を想定するため、基本計算のみ提供
        if (property.propertyType === types_1.PropertyType.APARTMENT) {
            const basePrice = this.calculateBasePrice(property);
            const depreciatedPrice = this.applyDepreciation(basePrice, property.buildYear);
            const locationAdjusted = this.applyLocationAdjustment(depreciatedPrice, property.prefecture, property.city);
            const facilityAdjusted = this.applyFacilityAdjustment(locationAdjusted, property);
     ...
     */
    async updateSellerValuations(sellerId, valuations) {
        const updateData = {};
     ...
     */
    validateValuationOrder(amount1, amount2, amount3) {
     ...
     */
    generateValuationPdfUrl(sellerId, sellerNumber) {
     ...
     */
    calculateFromRoadPrice(landArea, roadPrice) {
...
exports.ValuationEngine = ValuationEngine;
