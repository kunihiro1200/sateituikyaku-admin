...
exports.BuyerSyncService = exports.SyncErrorType = void 0;
// 買主リストの同期サービス
const googleapis_1 = require("googleapis");
const supabase_js_1 = require("@supabase/supabase-js");
const path = __importStar(require("path"));
const BuyerColumnMapper_1 = require("./BuyerColumnMapper");
const BuyerSyncLogger_1 = require("./BuyerSyncLogger");
const SyncMonitoringService_1 = require("./SyncMonitoringService");
const SPREADSHEET_ID = '1tI_iXaiLuWBggs5y0RH7qzkbHs9wnLLdRekAmjkhcLY';
const SHEET_NAME = '買主リスト';
const BATCH_SIZE = 50; // Reduced from 100 for better progress reporting
var SyncErrorType;
(function (SyncErrorType) {
    SyncErrorType["NETWORK_ERROR"] = "network_error";
    SyncErrorType["DATABASE_ERROR"] = "database_error";
    SyncErrorType["VALIDATION_ERROR"] = "validation_error";
    SyncErrorType["SCHEMA_ERROR"] = "schema_error";
    SyncErrorType["UNKNOWN_ERROR"] = "unknown_error";
})(SyncErrorType || (exports.SyncErrorType = SyncErrorType = {}));
class BuyerSyncService {
    constructor() {
        ...
        this.syncLogger = new BuyerSyncLogger_1.BuyerSyncLogger();
        this.syncMonitoring = new SyncMonitoringService_1.SyncMonitoringService();
    }
    /**
     * 同期中かどうかを確認
     */
     ...
     * 全データを同期（進捗レポート付き）
     */
    async syncAllWithProgress(progressCallback) {
        if (this.isSyncing) {
    ...
    async processBatch(headers, rows, startRowNumber) {
        const result = { created: 0, updated: 0, failed: 0, skipped: 0, errors: [] };
     ...
     * フィールドレベル追跡付きで買主を同期
     */
    async syncBuyerWithFieldTracking(buyerNumber, headers, row) {
        const fieldResults = [];
        try {
     ...
     * フィールドカバレッジを検証
     */
    async validateFieldCoverage(headers) {
        const mappedFields = this.columnMapper.getMappedFields();
        ...
        const unmappedColumns = headers.filter(header => !mappedColumns.includes(header));
     ...
     * 特定フィールドのみを同期
     */
    async syncSpecificFields(buyerNumber, fieldNames) {
        const fieldResults = [];
        try {
...
exports.BuyerSyncService = BuyerSyncService;
