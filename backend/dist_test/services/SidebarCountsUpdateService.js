...
exports.SidebarCountsUpdateService = void 0;
 ...
 */
class SidebarCountsUpdateService {
    constructor(supabaseClient) {
        this.supabase = supabaseClient || (0, supabase_js_1.createClient)(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY);
    }
    /**
     * 買主サイドバーカウントを更新
     *
     ...
     */
    async updateBuyerSidebarCounts(buyerNumberOrId, oldBuyerData) {
        try {
            ...
            // Redisキャッシュを無効化
            await cache_1.CacheHelper.del('buyers:sidebar-counts');
     ...
     */
    async updateSellerSidebarCounts(sellerNumberOrId) {
        try {
     ...
     * 買主のカテゴリーを判定
     */
    determineBuyerCategories(buyer) {
        const categories = [];
        // ✅ タイムゾーン修正: getTodayJST()を使用してJST日付を取得
        const todayStr = (0, dateHelpers_1.getTodayJST)();
        ...
        if (buyer.viewing_date && buyer.broker_inquiry !== '業者問合せ' && !buyer.notification_sender) {
            const dayOfWeek = (0, dateHelpers_1.getDayOfWeek)(buyer.viewing_date);
            const isThursday = dayOfWeek === '木曜日';
            // 木曜内覧の場合は2日前（火曜）、それ以外は1日前
            const isViewingDayBefore = isThursday
                ? (0, dateHelpers_1.isDaysFromToday)(buyer.viewing_date, 2)
                : (0, dateHelpers_1.isTomorrow)(buyer.viewing_date);
        ...
        // 問合せメール未対応
        const isInquiryEmailUnanswered = buyer.inquiry_email_phone === '未' ||
        ...
        if (isPinrichBlankOrUnregistered &&
        ...
        const conditionA_GV = (buyer.viewing_type_general && String(buyer.viewing_type_general).trim() &&
        ...
        const conditionB_GV = (buyer.post_viewing_seller_contact === '未' &&
            buyer.atbb_status && String(buyer.atbb_status).includes('公開中'));
     ...
     * 売主のカテゴリーを判定
     */
    determineSellerCategories(seller) {
        const categories = [];
        // ✅ タイムゾーン修正: getTodayJST()を使用してJST日付を取得
        const todayStr = (0, dateHelpers_1.getTodayJST)();
        // 訪問日前日（dateHelpers.tsの関数を使用してタイムゾーン問題を回避）
        if (seller.visit_assignee && seller.visit_date) {
            const dayOfWeek = (0, dateHelpers_1.getDayOfWeek)(seller.visit_date);
            const isThursday = dayOfWeek === '木曜日';
            // 木曜訪問の場合は2日前（火曜）、それ以外は1日前
            const isVisitDayBefore = isThursday
                ? (0, dateHelpers_1.isDaysFromToday)(seller.visit_date, 2)
                : (0, dateHelpers_1.isTomorrow)(seller.visit_date);
            if (isVisitDayBefore) {
     ...
     * カテゴリーの差分を計算
     */
    calculateCategoryDiff(oldCategories, newCategories) {
        const added = newCategories.filter(newCat => !oldCategories.some(oldCat => oldCat.category === newCat.category && oldCat.assignee === newCat.assignee));
        const removed = oldCategories.filter(oldCat => !newCategories.some(newCat => newCat.category === oldCat.category && newCat.assignee === oldCat.assignee));
     ...
     */
    async getBuyerCategories(buyerId) {
        try {
     ...
     * 買主サイドバーカウントを増減
     */
    async incrementBuyerSidebarCount(category, assignee, delta) {
        try {
            const key = assignee || '';
            // 既存のレコードを取得
     ...
     * 売主サイドバーカウントを増減
     */
    async incrementSellerSidebarCount(category, assignee, delta) {
        try {
            const key = assignee || '';
            // 既存のレコードを取得
            const { data: existing, error: fetchError } = await this.supabase
...
exports.SidebarCountsUpdateService = SidebarCountsUpdateService;
