...
exports.valuationCalculatorService = exports.ValuationCalculatorService = void 0;
const BaseRepository_1 = require("../repositories/BaseRepository");
/**
 * 査定額計算サービス
 * 複雑な査定額計算ロジックを実装
 */
class ValuationCalculatorService extends BaseRepository_1.BaseRepository {
    /**
     * 査定額1を計算（詳細な計算式）
     */
    async calculateValuationAmount1(seller, property) {
        try {
            // 必要なデータの取得
            ...
            // 建築価格の計算
            const constructionPrice = this.calculateConstructionPrice(constructionUnitPrice, buildingArea, buildingAge, structure);
     ...
     * 建築単価を取得
     */
    async getConstructionUnitPrice(buildYear, structure) {
     ...
     */
    getHardcodedConstructionPrice(year, structure) {
     ...
     * 構造に応じた価格を取得
     */
    getPriceByStructure(data, structure) {
        switch (structure) {
            case '木造':
            case '不明':
            case '未確認':
     ...
     * 2024年の最新価格を使用
     */
    getDefaultConstructionPrice(structure) {
        switch (structure) {
            case '木造':
            case '不明':
            case '未確認':
     ...
     */
    calculateConstructionPrice(unitPrice, buildingArea, buildingAge, structure) {
        if (buildingArea === 0) {
            return 0;
        }
        // 基準価格
     ...
     */
    calculateLandPrice(landArea, fixedAssetTaxRoadPrice) {
     ...
     * 査定額1に基づいて加算額を計算
     */
    async calculateValuationAmount2(seller, valuationAmount1) {
        try {
            // 査定額1を万円単位に変換
     ...
     * 査定額1に基づいて加算額を計算
     */
    async calculateValuationAmount3(seller, valuationAmount1) {
        try {
            // 査定額1を万円単位に変換
            const amount1InManYen = valuationAmount1 / 10000;
            // 段階的な加算額を決定
...
exports.ValuationCalculatorService = ValuationCalculatorService;
exports.valuationCalculatorService = new ValuationCalculatorService();
