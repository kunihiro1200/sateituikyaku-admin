...
exports.PanoramaUrlService = void 0;
...
class PanoramaUrlService {
     ...
     */
    async getPanoramaUrl(propertyNumber) {
        try {
            // キャッシュチェック
     ...
     * @returns パノラマURL（存在しない場合はnull）
     */
    async getPanoramaUrlFromSheet(spreadsheetId) {
        try {
            ...
            const sheets = googleapis_1.google.sheets({ version: 'v4', auth: sheetsClient['auth'] });
            // シート名のパターン（末尾スペースを含む）
     ...
     */
    async searchSpreadsheetInDrive(propertyNumber) {
        try {
            // サービスアカウント認証
     ...
     * データをキャッシュに保存
     */
    setCachedData(propertyNumber, url) {
...
exports.PanoramaUrlService = PanoramaUrlService;
