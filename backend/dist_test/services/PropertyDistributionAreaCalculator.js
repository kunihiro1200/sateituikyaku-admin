...
exports.PropertyDistributionAreaCalculator = void 0;
// 物件配信エリア計算サービス
...
const cityAreaMapping_1 = require("../utils/cityAreaMapping");
...
class PropertyDistributionAreaCalculator {
     ...
     * @returns 計算結果
     */
    async calculateDistributionAreas(googleMapUrl, city, address, preloadedCoords) {
        const radiusAreas = [];
        const cityWideAreas = [];
        // 1. 市名に基づいて市全域エリアを追加
        if (city) {
            ...
            if (normalizedCity.includes('大分')) {
                ...
                // さらに住所から詳細エリア（①〜⑧）も追加
                if (address) {
                    const oitaAreas = (0, cityAreaMapping_1.getOitaCityAreas)(address);
            ...
            if (normalizedCity.includes('別府')) {
                ...
                // さらに住所から詳細エリア（⑨〜⑮、㊷、㊸）も追加
                if (address) {
                    const beppuAreas = (0, cityAreaMapping_1.getBeppuCityAreas)(address);
        ...
        let resolvedCoords = preloadedCoords || null;
            ...
            if (true) {
                ...
                for (const config of areaConfigs) {
                    ...
                    if (!config.coordinates)
                        continue;
                    const distance = this.geolocationService.calculateDistance(coords, config.coordinates);
                    // 設定された半径内のエリアを追加
     ...
     * @returns 有効かどうか
     */
    validateAreaNumbers(areaNumbers) {
     ...
     * @returns 計算結果とデバッグ情報
     */
    async calculateWithDebugInfo(googleMapUrl, city, address) {
        const debugInfo = {
            propertyCoords: null,
            cityField: city || null,
            areaConfigs: [],
     ...
     * @returns 検証結果
     */
    async verifyRadiusCalculation(propertyCoords, expectedAreas) {
        const areaConfigs = await this.areaMapConfigService.loadAreaMaps();
        const actualAreas = [];
        for (const config of areaConfigs) {
            if (!config.coordinates)
     ...
     * @returns テスト結果
     */
    async testWithKnownLocation(address, expectedAreas) {
...
exports.PropertyDistributionAreaCalculator = PropertyDistributionAreaCalculator;
