...
exports.pinrichService = exports.PinrichService = void 0;
...
class PinrichService {
     ...
     */
    async updatePinrichStatus(sellerId, status) {
        try {
     ...
     * @returns Updated Pinrich information
     */
    async updateNumberOfCompanies(sellerId, count) {
        try {
            if (count < 0) {
     ...
     */
    async getSellersByPinrichStatus(status) {
        try {
            // Validate Pinrich status
            if (!this.validatePinrichStatus(status)) {
                throw new Error(`Invalid Pinrich status: ${status}`);
            ...
            const { data, error } = await supabase
                .from('sellers')
                .select(`id, seller_number, name, phone_number, pinrich_status, 
     ...
     */
    async getPinrichStatistics(startDate, endDate) {
        try {
            let query = supabase
                .from('sellers')
                .select('pinrich_status, number_of_companies')
     ...
     * @returns Updated Pinrich information
     */
    async closePinrichDistribution(sellerId, reason) {
        try {
            const updateData = {
                pinrich_status: 'クローズ',
            };
     ...
     * @returns Updated Pinrich information
     */
    async startPinrichDistribution(sellerId, numberOfCompanies) {
        try {
            const updateData = {
                pinrich_status: '配信中',
            };
     ...
     */
    async logPinrichStatusChange(sellerId, status, reason) {
        try {
...
exports.PinrichService = PinrichService;
// Export singleton instance
exports.pinrichService = new PinrichService();
