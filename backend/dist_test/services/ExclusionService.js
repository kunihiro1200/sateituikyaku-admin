...
exports.exclusionService = exports.ExclusionService = void 0;
...
class ExclusionService {
     ...
     */
    async addToExclusionList(sellerId, exclusionData) {
        try {
     ...
     * @returns void
     */
    async removeFromExclusionList(sellerId, reason) {
        try {
    ...
    async getExcludedSellers(filters) {
        try {
            let query = supabase
                .from('sellers')
                .select(`id, seller_number, name, phone_number, exclusion_site, 
     ...
     */
    async getCommonExclusionCriteria() {
        try {
            const { data, error } = await supabase
                .from('sellers')
     ...
     */
    async bulkExclude(sellerIds, exclusionData) {
        try {
            const { data, error } = await supabase
                .from('sellers')
                .update({
     ...
     */
    async logExclusionAction(sellerId, action, data) {
        try {
     ...
     */
    async calculateAndSetExclusionDate(sellerId, inquiryDate, site) {
        try {
     ...
     */
    async getSellersWithUpcomingExclusion(daysAhead = 7) {
        try {
            ...
            const { data, error } = await supabase
                .from('sellers')
                .select(`id, seller_number, name, phone_number, exclusion_date, 
     ...
     */
    async sendExclusionDateNotification(sellerId) {
        try {
            // Get seller and assigned employee information
            const { data: seller, error: sellerError } = await supabase
                .from('sellers')
                .select(`id, seller_number, name, exclusion_date, exclusion_action,
           assigned_to, employees:assigned_to (id, name, email, chat_webhook_url)`)
            ...
            const employee = seller.employees;
            if (!employee) {
     ...
     */
    async processDailyExclusionChecks() {
        try {
...
exports.exclusionService = new ExclusionService();
