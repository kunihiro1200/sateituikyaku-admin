...
exports.SharedItemsService = void 0;
...
class SharedItemsService {
    ...
    async create(item) {
        try {
            ...
            const normalizedItem = this.normalizeKeys(item);
    ...
    async update(id, updates) {
        try {
            ...
            await this.sheetsClient.updateRowPartial(rowIndex, updates);
            return { ...updates, id };
        }
        catch (error) {
    ...
    sortItems(items) {
        return items.sort((a, b) => {
            // 共有日が空のアイテムを先に表示
            if (!a.sharing_date && b.sharing_date)
                return -1;
            if (a.sharing_date && !b.sharing_date)
                return 1;
            // 両方とも共有日が空の場合、順序を維持
     ...
     */
    async addStaffConfirmation(itemId, staffName) {
        try {
            // IDから行番号を検索
     ...
     * 「確認日」に日付を設定
     */
    async markStaffConfirmed(itemId, staffName) {
        try {
            // IDから行番号を検索
            const rowIndex = await this.sheetsClient.findRowByColumn('ID', itemId);
            if (!rowIndex) {
     ...
     * 行データをSharedItemに変換
     */
    mapRowToItem(row, rowIndex) {
        // 日付フィールドを変換
        const formattedRow = { ...row };
        // 日付フィールドを変換
        if (formattedRow['日付']) {
            formattedRow['日付'] = this.formatDate(formattedRow['日付']);
        }
        if (formattedRow['共有日']) {
            formattedRow['共有日'] = this.formatDate(formattedRow['共有日']);
        }
        if (formattedRow['確認日']) {
            formattedRow['確認日'] = this.formatDate(formattedRow['確認日']);
        }
        // A列のIDを使用（存在しない場合は行番号をフォールバック）
        const id = formattedRow['ID'] ? String(formattedRow['ID']) : rowIndex.toString();
        return {
            id,
            sharing_location: formattedRow['共有場'] || '',
            sharing_date: formattedRow['共有日'] || null,
            staff_not_shared: formattedRow['共有できていない'] || null,
            confirmation_date: formattedRow['確認日'] || null,
            ...formattedRow,
        };
    }
}
exports.SharedItemsService = SharedItemsService;
/**
 * 英語キー → 日本語キー（スプレッドシートヘッダー）マッピング
 */
SharedItemsService.FIELD_KEY_MAP = {
    'id': 'ID',
    'date': '日付',
    'input_person': '入力者',
