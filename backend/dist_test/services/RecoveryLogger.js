...
exports.RecoveryLogger = exports.RecoveryErrorType = void 0;
// 復旧プロセスのログ管理サービス
const supabase_js_1 = require("@supabase/supabase-js");
const uuid_1 = require("uuid");
var RecoveryErrorType;
(function (RecoveryErrorType) {
    RecoveryErrorType["VALIDATION_ERROR"] = "validation_error";
    RecoveryErrorType["DATABASE_ERROR"] = "database_error";
    RecoveryErrorType["DUPLICATE_ERROR"] = "duplicate_error";
    RecoveryErrorType["UNKNOWN_ERROR"] = "unknown_error";
})(RecoveryErrorType || (exports.RecoveryErrorType = RecoveryErrorType = {}));
/**
 * 復旧プロセスのログを管理するサービス
 */
class RecoveryLogger {
     ...
     * 復旧開始をログに記録
     */
    async logRecoveryStart(totalRows) {
     ...
     * 復旧進捗をログに記録
     */
    async logRecoveryProgress(logId, processedRows) {
     ...
     * 復旧完了をログに記録
     */
    async logRecoveryComplete(logId, result) {
        const log = this.logs.get(logId);
        if (!log) {
            console.warn(`[Recovery ${logId}] ログが見つかりません`);
            return;
     ...
     * 特定の復旧ログを取得
     */
    getRecoveryLog(logId) {
        return this.logs.get(logId) || null;
    }
    /**
     * エラーログをファイルに出力
     */
    async exportErrorLog(logId, filePath) {
        const log = this.logs.get(logId);
        if (!log || !log.result) {
...
exports.RecoveryLogger = RecoveryLogger;
