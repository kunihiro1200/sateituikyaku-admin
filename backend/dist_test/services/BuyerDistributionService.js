...
exports.BuyerDistributionService = void 0;
// 買主配信フィルタリングサービス
...
class BuyerDistributionService {
     ...
     */
    async getQualifiedBuyers(criteria) {
        try {
            // 物件情報を取得
            ...
            // フィルタリング
            qualifiedBuyers = qualifiedBuyers.filter(buyer => this.hasCircleOneInArea(buyer.desired_area) &&
            ...
            // メールアドレスを抽出（重複排除）
            const emails = Array.from(new Set(qualifiedBuyers
     ...
     * @returns 「①」を含む場合true
     */
    hasCircleOneInArea(desiredArea) {
        if (!desiredArea) {
            return false;
        }
     ...
     */
    isDistributionRequired(distributionType) {
        if (!distributionType) {
            return false;
        }
...
exports.BuyerDistributionService = BuyerDistributionService;
