...
exports.invalidateSellerCache = invalidateSellerCache;
exports.invalidateListSellersCache = invalidateListSellersCache;
...
const CalendarService_supabase_1 = require("./CalendarService.supabase");
const ExclusionDateCalculator_1 = require("./ExclusionDateCalculator");
const supabase_js_1 = require("@supabase/supabase-js");
const sellerCategoryFilters_1 = require("../utils/sellerCategoryFilters");
// モジュールレベルのインメモリキャッシュ（プロセス内で持続、Redis 不要）
// Vercel サーバーレスでは同一プロセス内のリクエスト間でキャッシュが共有される
let _employeeInitialsMap = null;
...
async function refreshEmployeeCache() {
    if (_employeeInitialsRefreshing)
        return; // 重複リフレッシュ防止
...
function setSellerCache(sellerId, data) {
...
const _listSellersCache = new Map();
const LIST_SELLERS_CACHE_TTL_MS = 0; // 0 = キャッシュ無効
function getListSellersCache(cacheKey) {
...
function setListSellersCache(cacheKey, data) {
    _listSellersCache.set(cacheKey, { data, expiresAt: Date.now() + LIST_SELLERS_CACHE_TTL_MS });
}
function invalidateListSellersCache() {
    _listSellersCache.clear();
}
class SellerService extends BaseRepository_1.BaseRepository {
    /**
     * 同期キューを設定（インスタンス）
     */
     ...
     */
    static setSharedSyncQueue(syncQueue) {
     ...
     * 売主を登録（Phase 1拡張版）
     */
    async createSeller(data, employeeId) {
        // Phase 1: 重複チェック
        const encryptedPhone = (0, encryption_1.encrypt)(data.phoneNumber);
        const encryptedEmail = data.email ? (0, encryption_1.encrypt)(data.email) : undefined;
        const duplicateMatches = await DuplicateDetectionService_1.duplicateDetectionService.instance.checkDuplicates(encryptedPhone, encryptedEmail);
        ...
        if (!sellerNumber) {
            try {
                sellerNumber = await SellerNumberService_1.sellerNumberService.generateWithRetry();
            }
            catch (error) {
        ...
        // 除外日を計算
        const exclusionDate = ExclusionDateCalculator_1.ExclusionDateCalculator.calculateExclusionDate(data.inquiryDate, data.site);
        // 個人情報を暗号化（address は暗号化対象外）
        const encryptedData = {
            name: (0, encryption_1.encrypt)(data.name),
            ...
            status: data.status || types_1.SellerStatus.FOLLOWING_UP,
            // Phase 1 fields
            seller_number: sellerNumber,
            inquiry_source: data.inquirySource,
            inquiry_year: data.inquiryYear,
            inquiry_date: data.inquiryDate,
            inquiry_detailed_datetime: data.inquiryDetailedDateTime || data.inquiryDatetime || null,
            confidence_level: data.confidence || data.confidenceLevel || null,
            ...
            visit_assignee: data.visitAssignee || null,
            visit_notes: data.visitNotes || null,
            // ステータス・コメント
            comments: data.comments || null,
            assigned_to: data.assignedTo || null,
            // 査定情報
            valuation_amount_1: data.valuationAmount1 ? parseFloat(String(data.valuationAmount1)) : null,
            valuation_amount_2: data.valuationAmount2 ? parseFloat(String(data.valuationAmount2)) : null,
            valuation_amount_3: data.valuationAmount3 ? parseFloat(String(data.valuationAmount3)) : null,
            valuation_method: data.valuationMethod || null,
            valuation_assignee: data.valuationAssignee || null,
            // サイト
            inquiry_site: data.site || null,
        };
        // 売主を作成
        ...
        if (duplicateMatches.length > 0) {
            for (const match of duplicateMatches) {
                try {
                    await DuplicateDetectionService_1.duplicateDetectionService.instance.recordDuplicateHistory(seller.id, match.sellerId, match.matchType);
                }
                catch (error) {
        ...
        const mappedPropertyType = propertyTypeMap[data.property.propertyType] || data.property.propertyType || '戸建て';
        ...
        try {
            ...
            await sellerListClient.appendRow({
                ...
                // 追加フィールド
                '次電日': data.nextCallDate || '',
                '訪問日 \nY/M/D': data.visitDate || '',
                '訪問時間': data.visitTime || '',
                '営担': data.visitAssignee || '',
                '訪問メモ': data.visitNotes || '',
                '確度': data.confidence || data.confidenceLevel || '',
                '状況（当社）': data.status || '',
                'コメント': data.comments || '',
                '査定額1': data.valuationAmount1 ? String(data.valuationAmount1) : '',
                '査定額2': data.valuationAmount2 ? String(data.valuationAmount2) : '',
                '査定額3': data.valuationAmount3 ? String(data.valuationAmount3) : '',
                '査定方法': data.valuationMethod || '',
                '査定担当': data.valuationAssignee || '',
                '連絡方法': data.contactMethod || '',
                '連絡取りやすい日、時間帯': data.preferredContactTime || '',
                '1番電話': data.firstCallPerson || '',
            });
        ...
        await cache_1.CacheHelper.del('sellers:sidebar-counts');
        // スプレッドシートに同期（非同期）
     ...
     */
    async getSeller(sellerId, includeDeleted = false) {
        const _t0 = Date.now();
        ...
        // 除外日を計算
        const exclusionDate = ExclusionDateCalculator_1.ExclusionDateCalculator.calculateExclusionDate(decryptedSeller.inquiryDate, decryptedSeller.site);
        ...
        if (!propertyError && properties && properties.length > 0) {
            ...
            const isValidAddress = (addr) => !!addr && addr.trim() !== '' && addr.trim() !== '未入力';
            const resolvedAddress = isValidAddress(property.property_address) ? property.property_address :
    ...
    async updateSeller(sellerId, data) {
        ...
        if (data.name !== undefined) {
            updates.name = (0, encryption_1.encrypt)(data.name);
        }
        if (data.address !== undefined) {
            updates.address = data.address;
        }
        if (data.phoneNumber !== undefined) {
            updates.phone_number = (0, encryption_1.encrypt)(data.phoneNumber);
            updates.phone_number_hash = data.phoneNumber ? crypto.createHash('sha256').update(data.phoneNumber).digest('hex') : null;
        }
        if (data.email !== undefined) {
            updates.email = data.email ? (0, encryption_1.encrypt)(data.email) : null;
        ...
        if (data.visitAssignee !== undefined) {
            updates.visit_assignee = data.visitAssignee;
        }
        // visitDate を直接指定した場合（訪問予約フォームから、TIMESTAMP型として保存）
        if (data.visitDate !== undefined) {
            updates.visit_date = data.visitDate; // TIMESTAMP形式（YYYY-MM-DD HH:mm:ss）
        }
        // visitTime を直接指定した場合（訪問予約フォームから）
        if (data.visitTime !== undefined) {
            updates.visit_time = data.visitTime; // HH:mm 形式
        }
        if (data.visitAcquisitionDate !== undefined) {
            updates.visit_acquisition_date = data.visitAcquisitionDate; // YYYY-MM-DD形式のまま保存
        }
        ...
        if (data.visitValuationAcquirer !== undefined) {
            updates.visit_valuation_acquirer = data.visitValuationAcquirer;
        }
        ...
        if (data.valuationAssignee !== undefined) {
            updates.valuation_assignee = data.valuationAssignee;
        }
        if (data.valuationMethod !== undefined) {
            updates.valuation_method = data.valuationMethod;
        }
        if (data.landAreaVerified !== undefined) {
            updates.land_area_verified = data.landAreaVerified;
        }
        if (data.buildingAreaVerified !== undefined) {
            updates.building_area_verified = data.buildingAreaVerified;
        }
        // 競合情報フィールド
        ...
        if (data.otherDecisionCountermeasure !== undefined) {
            updates.other_decision_countermeasure = data.otherDecisionCountermeasure;
        }
        if (data.contractYearMonth !== undefined) {
            updates.contract_year_month = data.contractYearMonth;
        }
        if (data.exclusiveOtherDecisionMeeting !== undefined) {
            updates.exclusive_other_decision_meeting = data.exclusiveOtherDecisionMeeting;
        }
        ...
        if (data.exclusionAction !== undefined) {
            // 空文字列の場合はnullに変換（除外アクション解除）
            updates.exclusion_action = data.exclusionAction === '' ? null : data.exclusionAction;
        }
        // 内覧前伝達事項
        ...
        // 郵送ステータスフィールド
        if (data.mailingStatus !== undefined) {
            updates.mailing_status = data.mailingStatus;
        }
        if (data.mailSentDate !== undefined) {
            updates.mail_sent_date = data.mailSentDate;
        }
        // コメントフィールド
        ...
        if (data.inquiryDate !== undefined || data.site !== undefined) {
            ...
            const exclusionDate = ExclusionDateCalculator_1.ExclusionDateCalculator.calculateExclusionDate(inquiryDateForCalc, siteForCalc);
        ...
        if (data.unreachableSmsAssignee !== undefined) {
            updates.unreachable_sms_assignee = data.unreachableSmsAssignee;
        }
        if (data.valuationSmsAssignee !== undefined) {
            updates.valuation_sms_assignee = data.valuationSmsAssignee;
        }
        if (data.valuationReasonEmailAssignee !== undefined) {
            updates.valuation_reason_email_assignee = data.valuationReasonEmailAssignee;
        }
        if (data.valuationReason !== undefined) {
            updates.valuation_reason = data.valuationReason;
        }
        if (data.cancelNoticeAssignee !== undefined) {
            updates.cancel_notice_assignee = data.cancelNoticeAssignee;
        }
        if (data.longTermEmailAssignee !== undefined) {
            updates.long_term_email_assignee = data.longTermEmailAssignee;
        }
        if (data.callReminderEmailAssignee !== undefined) {
            updates.call_reminder_email_assignee = data.callReminderEmailAssignee;
        }
        if (data.visitReminderAssignee !== undefined) {
            updates.visit_reminder_assignee = data.visitReminderAssignee;
        }
        if (data.unreachableStatus !== undefined) {
            updates.unreachable_status = data.unreachableStatus;
        }
        if (data.pinrichStatus !== undefined) {
            updates.pinrich_status = data.pinrichStatus;
        }
        ...
        if (error) {
            ...
            if (isUuidError) {
                ...
                await cache_1.CacheHelper.del(cache_1.CacheHelper.generateKey('seller', found.id));
                await cache_1.CacheHelper.delPattern('sellers:list:*');
                await cache_1.CacheHelper.del('sellers:sidebar-counts');
        ...
        console.log('✅ Updated seller from DB:', {
            id: seller.id,
            phone_number: seller.phone_number ? `${seller.phone_number.substring(0, 20)}...` : 'empty',
            raw_phone_number_length: seller.phone_number?.length || 0,
        });
        // データベースから再度取得して確認
        ...
        await cache_1.CacheHelper.del(cache_1.CacheHelper.generateKey('seller', sellerId));
        await cache_1.CacheHelper.delPattern('sellers:list:*');
        ...
        if (this.shouldUpdateSellerSidebarCounts(updates)) {
            ...
            Promise.resolve().then(() => __importStar(require('./SellerSidebarCountsUpdateService'))).then(({ SellerSidebarCountsUpdateService }) => {
                const sidebarService = new SellerSidebarCountsUpdateService(this.supabase);
                sidebarService.updateAffectedCategories(updatedFields).catch((err) => {
                    console.error('⚠️ Failed to update affected sidebar categories:', err);
                });
            }).catch((err) => console.error('⚠️ [SidebarCounts] Import error:', err));
        }
     ...
     */
    syncDirectToSpreadsheet(sellerId) {
        (async () => {
            try {
                const { SpreadsheetSyncService } = await Promise.resolve().then(() => __importStar(require('./SpreadsheetSyncService')));
                const { GoogleSheetsClient } = await Promise.resolve().then(() => __importStar(require('./GoogleSheetsClient')));
     ...
     */
    shouldUpdateSellerSidebarCounts(updates) {
     ...
     */
    async updateSellerWithAppointment(sellerId, data, creatorEmployeeId) {
            ...
            try {
                ...
                if (typeof assigneeIdentifier === 'string' && assigneeIdentifier.length <= 3) {
                    ...
                    if (values && values.length > 0) {
                        ...
                        const staffRow = values.find((row, index) => index > 0 && row[initialsIndex] === assigneeIdentifier);
                        if (staffRow) {
                ...
                if (existingAppointments && existingAppointments.length > 0) {
                    ...
                    if (existingAppointment.assigned_employee_id !== assignedEmployee.id) {
                        ...
                        }, updatedSeller.name, updatedSeller.phoneNumber, property.address, assignedEmployee.email // 営担のメールアドレスを渡す
                        );
                    }
                    else {
                        // 担当者が同じ場合は、既存の予約を更新
    ...
    async listSellers(params) {
        const { page = 1, pageSize = 50, status, assignedTo, nextCallDateFrom, nextCallDateTo, sortBy = 'inquiry_date', sortOrder = 'desc', includeDeleted = false, // デフォルトで削除済みを除外
        ...
        // キャッシュキーを生成
        const cacheKey = cache_1.CacheHelper.generateKey('sellers:list', page, pageSize, status || 'all', assignedTo || 'all', sortBy, sortOrder, includeDeleted ? 'with-deleted' : 'active-only', statusCategory || 'all');
            ...
            switch (statusCategory) {
                ...
                case 'todayCall':
                    ...
                    while (true) {
                        ...
                        if (data.length < tcPageSize)
                            break;
                        tcPage++;
                    }
                    // JSでフィルタ
                    const todayCallIds = (todayCallCandidates || []).filter((s) => {
                        // FI売主（福岡）は福岡専用カテゴリーに表示するため除外
                        const sellerNum = (s.seller_number || '').toString();
                        if (sellerNum.toUpperCase().startsWith('FI'))
                            return false;
                ...
                case 'todayCallWithInfo':
                    ...
                    // 🚨 重要: 「他決→追客」も含める（getSidebarCountsFallback()と同じロジック）
                    // 全件取得してJSでフィルタ（ORクエリが複雑なため）
                    let todayCallWithInfoCandidates = [];
                    let tcwiPage = 0;
                    ...
                    while (true) {
                        ...
                        if (data.length < tcwiPageSize)
                            break;
                        tcwiPage++;
                    }
                    // JSでフィルタ
                    const todayCallWithInfoIds = (todayCallWithInfoCandidates || []).filter((s) => {
                        // FI売主（福岡）は福岡専用カテゴリーに表示するため除外
                        const sellerNum = (s.seller_number || '').toString();
                        if (sellerNum.toUpperCase().startsWith('FI'))
                            return false;
                ...
                case 'unvaluated': {
                    ...
                    const unvaluatedIds = unvaluatedCandidates.filter(s => {
                        ...
                        if (!hasNoValuation || isNotRequired)
                            return false;
                ...
                case 'pinrichChangeRequired': {
                    ...
                    while (true) {
                        ...
                        if (pageData.length < pinrichPageSize)
                            break;
                        pinrichPage++;
                    }
                ...
                default: {
                    ...
                    const dynamicCategory = statusCategory;
        ...
        const sellerIds = decryptedSellers.map((s) => s.id).filter(Boolean);
        const idToSellerNumber = {};
        for (const s of decryptedSellers) {
            if (s.id && s.sellerNumber)
                idToSellerNumber[s.id] = s.sellerNumber;
        }
        ...
        if (isLabelFilter) {
            const targetLabel = statusCategory.startsWith('fi:todayCallWithInfo:')
                ? statusCategory.replace('fi:todayCallWithInfo:', '')
                : statusCategory.replace('todayCallWithInfo:', '');
            const isValidVal = (v) => !!(v && v.trim() !== '' && v.trim().toLowerCase() !== 'null');
            finalSellers = decryptedSellers.filter((s) => {
                const parts = [];
                ...
                if (isValidVal(preferred))
                    parts.push(preferred.trim());
                if (isValidVal(contact))
                    parts.push(contact.trim());
        ...
        if (typeof statusCategory === 'string' && statusCategory.startsWith('fi:')) {
            ...
            finalSellers = decryptedSellers.filter((s) => {
                ...
                const normalized = {
                    status: s.status || '',
                    next_call_date: (s.nextCallDate || s.next_call_date || '').substring(0, 10) || null,
                    visit_assignee: s.visitAssigneeInitials || s.visit_assignee_initials || null,
                    phone_contact_person: s.phoneContactPerson || s.phone_contact_person || null,
                    preferred_contact_time: s.preferredContactTime || s.preferred_contact_time || null,
                    contact_method: s.contactMethod || s.contact_method || null,
                    unreachable_status: s.unreachableStatus || s.unreachable_status || null,
                    confidence_level: s.confidenceLevel || s.confidence_level || s.confidence || null,
                    // inquiryDateはDateオブジェクト（decryptSeller経由）の場合はUTC基準で文字列化
                    inquiry_date: inquiryDateRaw instanceof Date
                        ...
                        : (typeof inquiryDateRaw === 'string' ? inquiryDateRaw.substring(0, 10) : null),
                    seller_number: s.sellerNumber || s.seller_number || null,
                    valuation_amount_1: s.valuationAmount1 || s.valuation_amount_1 || null,
                    valuation_amount_2: s.valuationAmount2 || s.valuation_amount_2 || null,
                    valuation_amount_3: s.valuationAmount3 || s.valuation_amount_3 || null,
                    valuation_method: s.valuationMethod || s.valuation_method || null,
                };
                if (fiSubCat === 'todayCallNotStarted') {
                    return (0, sellerCategoryFilters_1.isTodayCallNotStarted)(normalized, todayJSTStr);
                }
                else if (fiSubCat === 'todayCall') {
                    return (0, sellerCategoryFilters_1.isTodayCallNoInfo)(normalized, todayJSTStr);
                }
                else if (fiSubCat === 'todayCallWithInfo' || fiSubCat.startsWith('todayCallWithInfo:')) {
                    if (!(0, sellerCategoryFilters_1.isTodayCallWithInfo)(normalized, todayJSTStr))
                        return false;
                    if (fiSubCat.startsWith('todayCallWithInfo:')) {
                        const targetLabel = fiSubCat.replace('todayCallWithInfo:', '');
                        return (0, sellerCategoryFilters_1.getTodayCallWithInfoLabel)(normalized) === targetLabel;
                    }
                    return true;
                }
                else if (fiSubCat === 'unvaluated') {
                    return (0, sellerCategoryFilters_1.isUnvaluated)(normalized, todayJSTStr);
                }
                return true;
            });
        }
        // lastCalledAt を各売主に付与（キーは sellerNumber）
        const sellersWithCallDate = finalSellers.map((seller) => ({
            ...seller,
        ...
        const hasEncryptedName = sellersWithCallDate.some((s) => {
            const name = s.name || '';
        ...
        if (hasEncryptedName) {
            ...
            await cache_1.CacheHelper.set(cacheKey, result, cache_1.CACHE_TTL.SELLER_LIST);
        }
        return result;
    }
    /**
     ...
     */
    async _attachLastCalledAt(sellers) {
        if (sellers.length === 0)
            return sellers;
    ...
    async searchSellers(query, includeDeleted = false) {
        ...
        if (lowerQuery.match(/^\d{7,}$/)) {
            ...
            if (hashError)
                throw new Error(`Failed to search by phone hash: ${hashError.message}`);
        ...
        if (lowerQuery.match(/^[^\s@]+@[^\s@]+\.[^\s@]+$/)) {
            ...
            const { data: hashSellers, error: hashError } = await hashQuery;
            if (hashError)
                throw new Error(`Failed to search by email hash: ${hashError.message}`);
            if (hashSellers && hashSellers.length > 0) {
                const decryptedSellers = await Promise.all(hashSellers.map(s => this.decryptSeller(s)));
                return await this._attachLastCalledAt(decryptedSellers);
            }
        ...
        // 復号化後に部分一致検索
        const results = decryptedSellers.filter((seller) => (seller.name && seller.name.toLowerCase().includes(lowerQuery)) ||
    ...
    async decryptSeller(seller) {
        ...
        try {
            ...
            const decrypted = {
                id: seller.id,
                name: seller.name ? (0, encryption_1.decrypt)(seller.name) : '',
                address: seller.address ? (0, encryption_1.decrypt)(seller.address) : '',
                phoneNumber: seller.phone_number ? (0, encryption_1.decrypt)(seller.phone_number) : '',
                email: seller.email ? (0, encryption_1.decrypt)(seller.email) : undefined,
                ...
                exclusiveOtherDecisionFactors: seller.exclusive_other_decision_factor
                    ? seller.exclusive_other_decision_factor.split(', ').filter((f) => f.trim())
                    : [],
    ...
    async markAsUnreachable(sellerId) {
        ...
        // キャッシュを無効化
        const cacheKey = cache_1.CacheHelper.generateKey('seller', sellerId);
     ...
     */
    async confirmDuplicate(sellerId, employeeId) {
        const { data: seller, error } = await this.table('sellers')
            .update({
     ...
     */
    async getDuplicateHistory(sellerId) {
        return DuplicateDetectionService_1.duplicateDetectionService.instance.getDuplicateHistory(sellerId);
    }
    /**
     * Phase 1: Check for duplicate sellers
     */
    async checkDuplicates(phoneNumber, email, excludeId) {
        const encryptedPhone = (0, encryption_1.encrypt)(phoneNumber);
        const encryptedEmail = email ? (0, encryption_1.encrypt)(email) : undefined;
        return DuplicateDetectionService_1.duplicateDetectionService.instance.checkDuplicates(encryptedPhone, encryptedEmail, excludeId);
    }
    /**
     * 訪問統計を取得
     * @param month - 対象月（YYYY-MM形式）
     */
    async getVisitStats(month) {
      ...
      `)
        ...
        // 山本マネージャーの訪問率を計算
        // イニシャルがY（山本）のスタッフを探す
        const yamamoto = Object.values(statsByEmployee).find((stat) => stat.name.includes('山本') || stat.initials === 'Y');
     ...
     * 「外す」と空文字は除外する
     */
    async getUniqueAssigneeInitials() {
        const { data, error } = await this.table('sellers')
        ...
        // ユニークな値を抽出してソート
        const unique = [...new Set((data || []).map((row) => row.visit_assignee))]
            .filter(Boolean)
            .sort();
        return unique;
    }
    ...
    async getSidebarCounts() {
        try {
            ...
            // テーブルのデータを返却形式に変換
            const getCount = (category) => rows.find((r) => r.category === category && !r.label && !r.assignee)?.count ?? 0;
            const visitAssignedCounts = {};
            const todayCallAssignedCounts = {};
            const todayCallWithInfoLabelCounts = {};
            const fi_todayCallWithInfoLabelCounts = {};
            rows.forEach((r) => {
   ...
   */
    async getSidebarCountsFallback() {
        // JST今日の日付を計算
        const now = new Date();
        const jstTime = new Date(now.getTime() + (9 * 60 * 60 * 1000));
        const todayJST = `${jstTime.getUTCFullYear()}-${String(jstTime.getUTCMonth() + 1).padStart(2, '0')}-${String(jstTime.getUTCDate()).padStart(2, '0')}`;
        ...
        let visitAssigneeForDayBeforeAllData = [];
        {
        ...
        const [visitCompletedCountResult, todayCallAssignedResult, allAssignedResult, todayCallBaseResult1, todayCallBaseResult2, unvaluatedSellersResult, mailingPendingCountResult, exclusiveSellersResult, generalSellersResult, visitOtherDecisionSellersResult, unvisitedOtherDecisionSellersResult,] = await Promise.all([
            // 1. 訪問済みカウント
            this.table('sellers')
        ...
        let pinrichCandidatesAllFallback = [];
        {
            let pcPage = 0;
        ...
        todayCallAssignedSellers.forEach((s) => {
            ...
            if (a)
                todayCallAssignedCounts[a] = (todayCallAssignedCounts[a] || 0) + 1;
        });
        // 4. 担当(イニシャル)親カテゴリのイニシャル別集計
        const allAssignedSellers = allAssignedResult.data || [];
        const visitAssignedCounts = {};
        allAssignedSellers.forEach((s) => {
            const a = s.visit_assignee;
            if (a)
                visitAssignedCounts[a] = (visitAssignedCounts[a] || 0) + 1;
        });
        ...
        const filteredTodayCallSellers = todayCallBaseSellers.filter(s => {
            return !(0, sellerCategoryFilters_1.hasValidVisitAssignee)(s.visit_assignee);
        });
        // FI（福岡）売主と通常売主を分離（共通関数を使用）
        const isFiSeller = sellerCategoryFilters_1.isFiSeller;
        ...
        // ── 共通フィルタ関数を使用してカウント計算 ──────────────────────────────
        const todayCallWithInfoSellers = filteredTodayCallNormal.filter(s => (0, sellerCategoryFilters_1.isTodayCallWithInfo)(s, todayJST));
        const todayCallWithInfoCount = todayCallWithInfoSellers.length;
        const labelCountMap = {};
        todayCallWithInfoSellers.forEach(s => {
            const label = (0, sellerCategoryFilters_1.getTodayCallWithInfoLabel)(s);
            labelCountMap[label] = (labelCountMap[label] || 0) + 1;
        });
        const todayCallWithInfoLabels = Object.keys(labelCountMap);
        const todayCallNoInfoCount = filteredTodayCallNormal.filter(s => (0, sellerCategoryFilters_1.isTodayCallNoInfo)(s, todayJST)).length;
        // 6. 未査定のカウント（FI分離）
        const unvaluatedSellers = unvaluatedSellersResult.data || [];
        const unvaluatedCount = unvaluatedSellers.filter(s => !isFiSeller(s) && (0, sellerCategoryFilters_1.isUnvaluated)(s, todayJST)).length;
        const fi_unvaluatedCount = unvaluatedSellers.filter(s => isFiSeller(s) && (0, sellerCategoryFilters_1.isUnvaluated)(s, todayJST)).length;
        // 7. 査定（郵送）カウント
        const mailingPendingCount = mailingPendingCountResult.count || 0;
        // 8. 当日TEL_未着手
        const todayCallNotStartedCount = filteredTodayCallNormal.filter(s => (0, sellerCategoryFilters_1.isTodayCallNotStarted)(s, todayJST)).length;
        // FI（福岡）専用カウント計算（共通関数を使用）
        const fi_todayCallWithInfoSellers = filteredTodayCallFI.filter(s => (0, sellerCategoryFilters_1.isTodayCallWithInfo)(s, todayJST));
        const fi_todayCallWithInfoCount = fi_todayCallWithInfoSellers.length;
        const fi_labelCountMap = {};
        fi_todayCallWithInfoSellers.forEach(s => {
            const label = (0, sellerCategoryFilters_1.getTodayCallWithInfoLabel)(s);
            fi_labelCountMap[label] = (fi_labelCountMap[label] || 0) + 1;
        });
        const fi_todayCallNotStartedCount = filteredTodayCallFI.filter(s => (0, sellerCategoryFilters_1.isTodayCallNotStarted)(s, todayJST)).length;
        const fi_todayCallNoInfoCount = filteredTodayCallFI.filter(s => (0, sellerCategoryFilters_1.isTodayCallNoInfo)(s, todayJST)).length;
        // 9. Pinrich空欄: 次電日に関係なく「追客中 + Pinrich空欄 + 反響日2026/1/1以降 + 営担なし」
        // ※ filteredTodayCallSellers（次電日が今日以前）には依存しない
        let pinrichEmptyAllSellers = [];
        {
            let pePage = 0;
            ...
            while (true) {
                ...
                if (peError || !peData || peData.length === 0)
                    break;
                pinrichEmptyAllSellers = pinrichEmptyAllSellers.concat(peData);
                if (peData.length < pePageSize)
                    break;
                pePage++;
            }
        }
