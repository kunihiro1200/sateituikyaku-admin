...
exports.PriceReductionNotificationService = void 0;
 ...
 */
class PriceReductionNotificationService {
    constructor() {
    ...
    async sendNotifications(targets) {
        ...
        for (const target of targets) {
            try {
                ...
                // 送信成功をログに記録
                console.log(`[PriceReductionNotificationService] メール送信成功: 物件番号=${target.property_number}`);
                result.sent += 1;
                result.details.push({
                ...
                // 送信失敗をログに記録し、次の物件へ継続
                console.error(`[PriceReductionNotificationService] メール送信失敗: 物件番号=${target.property_number}`, { error: error.message });
                result.failed += 1;
                result.details.push({
                    property_number: target.property_number,
                    success: false,
...
exports.PriceReductionNotificationService = PriceReductionNotificationService;
