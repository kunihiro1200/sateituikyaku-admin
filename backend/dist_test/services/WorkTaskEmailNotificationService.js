...
exports.WorkTaskEmailNotificationService = exports.EMAIL_RULES = exports.TEMPLATE_VARIABLE_MAP = void 0;
...
const SITE_REGISTRATION_REQUEST_BODY = '<!DOCTYPE html><html><body style="margin:0;padding:0;font-family:Arial, Helvetica, \'Noto Sans JP\', sans-serif;font-size:14px;line-height:1.4;">' +
...
const SITE_REGISTRATION_OK_BODY = '<!DOCTYPE html><html><body style="margin:0;padding:0;font-family:Arial, Helvetica, \'Noto Sans JP\', sans-serif;font-size:14px;line-height:1.4;">' +
...
const FLOOR_PLAN_STORED_BODY = '<!DOCTYPE html><html><body style="margin:0;padding:0;font-family:Arial, Helvetica, \'Noto Sans JP\', sans-serif;font-size:14px;line-height:1.4;">' +
 ...
 * Requirements: 5.1, 5.2, 5.3
 */
class WorkTaskEmailNotificationService {
    constructor() {
    ...
    resolveTemplate(template, data) {
        let result = template;
        for (const [templateVar, columnName] of Object.entries(exports.TEMPLATE_VARIABLE_MAP)) {
            let value;
        ...
        // {メール配信コメント} を動的に解決
        const emailDist = data['email_distribution'] ?? '';
        ...
        else if (emailDist.includes('要') && emailDist.includes('期日通り')) {
            emailDistComment = '公開前配信・要、サイト公開は期日通りでおねがいします<br>';
        }
        result = result.split('{メール配信コメント}').join(emailDistComment);
        // {パノラマ行} を動的に解決（空なら非表示、値があれば「パノラマ：あり」）
        const panoramaValue = data['panorama'] ?? '';
        ...
        const roadDimValue = data['road_dimensions'] ?? '';
        const propertyTypeValue = data['property_type'] ?? '';
        ...
        if (result.includes('__manager_confirmation_body__')) {
            ...
            const assignee = data['sales_assignee'] ?? '';
            const bindingRaw = data['binding_scheduled_date'] ?? '';
            ...
            const revisionExists = data['contract_revision_exists'] ?? '';
            const revisionContent = data['contract_revision_content'] ?? '';
     ...
     */
    async processEmailNotifications(propertyNumber, beforeData, afterData) {
        for (const rule of exports.EMAIL_RULES) {
...
exports.WorkTaskEmailNotificationService = WorkTaskEmailNotificationService;
