...
exports.assigneeService = exports.AssigneeService = void 0;
...
class AssigneeService {
     ...
     */
    async updateValuationAssignee(sellerId, assigneeId) {
        try {
            const { data, error } = await supabase
                .from('sellers')
                .update({ valuation_assignee: assigneeId })
                .eq('id', sellerId)
                .select(`valuation_assignee, phone_assignee, first_call_initials, 
     ...
     * @returns Updated assignee information
     */
    async updatePhoneAssignee(sellerId, assigneeId) {
        try {
            const { data, error } = await supabase
                .from('sellers')
     ...
     * @returns Updated assignee information
     */
    async updateVisitAssignee(sellerId, assigneeId) {
        try {
            const { data, error } = await supabase
                .from('sellers')
     ...
     * @returns Updated assignee information
     */
    async recordVisitAcquiredBy(sellerId, acquiredBy) {
        try {
            const { data, error } = await supabase
                .from('sellers')
     ...
     */
    async getAssigneeInfo(sellerId) {
        try {
            const { data, error } = await supabase
                .from('sellers')
                .select(`valuation_assignee, phone_assignee, first_call_initials, 
     ...
     */
    async calculateVisitAcquisitionRatio(employeeId, startDate, endDate) {
        try {
     ...
     */
    async calculateMonthlyVisitRatio(employeeId, year, month) {
        try {
     ...
     */
    async bulkAssignValuationAssignee(sellerIds, assigneeId) {
        try {
            const { data, error } = await supabase
                .from('sellers')
                .update({ valuation_assignee: assigneeId })
...
exports.assigneeService = new AssigneeService();
