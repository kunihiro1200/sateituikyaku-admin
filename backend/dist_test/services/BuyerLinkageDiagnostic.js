...
exports.BuyerLinkageDiagnostic = void 0;
// 買主と物件の紐づけ診断サービス
const supabase_js_1 = require("@supabase/supabase-js");
class BuyerLinkageDiagnostic {
    constructor(supabaseClient) {
        this.supabase = supabaseClient || (0, supabase_js_1.createClient)(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_KEY);
    }
    /**
     * 買主と物件の紐づけ状況を分析
     */
    async analyzeLinkageStatus() {
     ...
     */
    async getBuyersWithPropertyCount() {
        const { count, error } = await this.supabase
            .from('buyers')
            .select('*', { count: 'exact', head: true })
     ...
     */
    async findBuyersWithoutProperty(limit = 10) {
     ...
     * 特定の物件番号に紐づく買主を取得
     */
    async getBuyersForProperty(propertyNumber) {
        const { data, error } = await this.supabase
            .from('buyers')
            .select('buyer_number, name, property_number, synced_at')
...
exports.BuyerLinkageDiagnostic = BuyerLinkageDiagnostic;
