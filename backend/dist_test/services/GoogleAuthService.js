...
class GoogleAuthService extends BaseRepository_1.BaseRepository {
    constructor() {
        super();
        this.SCOPES = [
        ...
        this.companyAccountId = null; // 会社アカウント用のemployee ID（動的に取得）
     ...
     */
    async initializeCompanyAccountId() {
        try {
    ...
    initializeOAuthClient() {
        ...
        if (!clientId || !clientSecret || !redirectUri) {
            console.warn('⚠️  Google Calendar API credentials not configured. Calendar features will be disabled.');
            return;
        }
    ...
    async exchangeCodeForTokens(code) {
        ...
        try {
            ...
            if (!tokens.refresh_token) {
                ...
                // 既存のトークンがあるか確認
                const { data: existingToken } = await this.table('google_calendar_tokens')
                    .select('*')
            ...
            // Webhook自動登録
            try {
                const { CalendarWebhookService } = await Promise.resolve().then(() => __importStar(require('./CalendarWebhookService')));
    ...
    async getAccessToken() {
        ...
        try {
            ...
            // リフレッシュトークンを復号化
            const refreshToken = (0, encryption_1.decrypt)(tokenData.encrypted_refresh_token);
     ...
     * @returns 認証済みOAuth2クライアント
     */
    async getAuthenticatedClientForEmployee(employeeId) {
        if (!this.oauth2Client) {
            throw new Error('Google Calendar API is not configured');
        }
        // 新しいクライアントインスタンスを作成
