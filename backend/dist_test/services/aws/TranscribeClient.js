...
exports.AmazonTranscribeClient = void 0;
exports.getTranscribeClient = getTranscribeClient;
...
class AmazonTranscribeClient {
    constructor() {
        ...
        if (this.useMock) {
            logger_1.default.info('Amazon Transcribe: Using mock implementation');
        }
        else {
            try {
                // 実際のAWS SDKクライアント初期化
                ...
                logger_1.default.info('Amazon Transcribe: Using real AWS SDK');
            }
            catch (error) {
                logger_1.default.error('Failed to initialize Amazon Transcribe client', { error: error.message });
                throw new phone_1.AWSConnectionError('Failed to initialize Amazon Transcribe', 'transcribe', error);
            }
        }
    }
    /**
    ...
    async startTranscriptionJob(params) {
        ...
        try {
            ...
            logger_1.default.error('Failed to start transcription job', { error, params });
    ...
    async getTranscriptionJob(jobName) {
        ...
        try {
            ...
            logger_1.default.error('Failed to get transcription job', { error, jobName });
            throw new phone_1.TranscriptionError('Failed to get transcription job', 'GET_FAILED', false, error);
        }
    }
    /**
     * 文字起こし結果を取得
    ...
    async getTranscriptionResult(transcriptFileUri) {
        ...
        try {
            ...
            logger_1.default.error('Failed to get transcription result', { error, transcriptFileUri });
            throw new phone_1.TranscriptionError('Failed to get transcription result', 'RESULT_FETCH_FAILED', false, error);
        }
    }
    /**
     * 文字起こしジョブを削除
     */
    async deleteTranscriptionJob(jobName) {
        if (this.useMock) {
        ...
        try {
            ...
            logger_1.default.error('Failed to delete transcription job', { error, jobName });
            throw new phone_1.TranscriptionError('Failed to delete transcription job', 'DELETE_FAILED', false, error);
        }
    }
    /**
     * 接続テスト
    ...
    async testConnection() {
        ...
        try {
            ...
            logger_1.default.error('Amazon Transcribe connection test failed', error);
            return false;
        }
    }
    // ============================================================================
    // モック実装（開発・テスト用）
    // ============================================================================
    mockStartTranscriptionJob(params) {
        logger_1.default.info('[MOCK] Starting transcription job', params);
    ...
    mockGetTranscriptionJob(jobName) {
        logger_1.default.info('[MOCK] Getting transcription job', { jobName });
    ...
    mockGetTranscriptionResult(transcriptFileUri) {
        logger_1.default.info('[MOCK] Getting transcription result', { transcriptFileUri });
        // モックの文字起こし結果
...
かしこまりました。詳しくお話を伺わせていただきます。
    ...
    mockDeleteTranscriptionJob(jobName) {
        logger_1.default.info('[MOCK] Deleting transcription job', { jobName });
        return Promise.resolve();
    }
}
exports.AmazonTranscribeClient = AmazonTranscribeClient;
// シングルトンインスタンス
let transcribeClientInstance = null;
/**
...
exports.default = AmazonTranscribeClient;
