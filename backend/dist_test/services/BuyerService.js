...
exports.isVendorBuyer = isVendorBuyer;
exports.invalidateBuyerStatusCache = invalidateBuyerStatusCache;
// 買主リストのCRUDサービス
const supabase_js_1 = require("@supabase/supabase-js");
const AuditLogService_1 = require("./AuditLogService");
const BuyerWriteService_1 = require("./BuyerWriteService");
...
const BuyerNumberSpreadsheetClient_1 = require("./BuyerNumberSpreadsheetClient");
const BuyerStatusCalculator_1 = require("./BuyerStatusCalculator");
...
let _moduleLevelStatusCache = null;
const _MODULE_STATUS_CACHE_TTL = 30 * 60 * 1000; // 30分
// 他社物件新着配信用のキャッシュ（TTL: 10分）
const distributionCache = new node_cache_1.default({ stdTTL: 600 });
// 買付率統計用のキャッシュ（TTL: 30分）
const purchaseRateStatisticsCache = new node_cache_1.default({ stdTTL: 1800 });
/**
 * 業者問合せ買主かどうかを判定する
 * broker_inquiry が null、undefined、空文字列、'0' の場合は通常買主（除外しない）
 * それ以外の値が入っている場合は業者問合せ買主（除外する）
 */
function isVendorBuyer(brokerInquiry) {
    if (brokerInquiry === null || brokerInquiry === undefined)
        return false;
    if (brokerInquiry === '')
        return false;
    if (brokerInquiry === '0')
 ...
 */
async function invalidateBuyerStatusCache() {
...
class BuyerService {
    ...
    async initSyncServices() {
        ...
        this.writeService = new BuyerWriteService_1.BuyerWriteService(sheetsClient, this.columnMapper);
        this.conflictResolver = new ConflictResolver_1.ConflictResolver(this.writeService, this.columnMapper);
        this.retryHandler = new RetryHandler_1.RetryHandler(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_KEY);
    }
    /**
     * 買主リストを取得（ページネーション、検索、フィルタ対応）
     */
    async getAll(options = {}) {
        const { page = 1, limit = 50, search, status, assignee, dateFrom, dateTo, sortBy = 'reception_date', sortOrder = 'desc', statusCategory, viewingMonth, followUpAssignee, } = options;
        ...
        if (search) {
            ...
            if (isBuyerNumber) {
                ...
                if (matchingPropertyNumbers.length > 0) {
                    query = query.or(`buyer_number.ilike.%${search}%,name.ilike.%${search}%,phone_number.ilike.%${search}%,email.ilike.%${search}%,property_number.ilike.%${search}%,property_number.in.(${matchingPropertyNumbers.join(',')})`);
                }
                else {
                    query = query.or(`buyer_number.ilike.%${search}%,name.ilike.%${search}%,phone_number.ilike.%${search}%,email.ilike.%${search}%,property_number.ilike.%${search}%`);
                }
            }
        }
        // フィルタリング
            ...
            switch (statusCategory) {
                ...
                default: {
                    ...
                    if (dynamicCategory.startsWith('assigned:')) {
                        ...
                        query = query.or(`follow_up_assignee.eq.${assigneeInitial},initial_assignee.eq.${assigneeInitial}`);
                    }
        ...
        const enrichedData = buyers.map((b) => {
            // calculated_statusを計算
            const statusResult = (0, BuyerStatusCalculator_1.calculateBuyerStatus)(b);
            return {
                ...b,
     ...
     * 買主番号で買主を取得
     */
    async getByBuyerNumber(buyerNumber, includeDeleted = false) {
    ...
    async search(query, limit = 20) {
        ...
        if (isNumericOnly) {
            ...
            const seen = new Set(exactData.map((b) => b.buyer_number));
            const combined = [
                ...exactData,
                ...partialData.filter((b) => !seen.has(b.buyer_number)),
            ];
     ...
     */
    async bulkUpdatePinrich500man(email, value) {
        const { data, error } = await this.supabase
            .from('buyers')
     ...
     * 買主に紐づく物件リストを取得
     */
    async getLinkedProperties(buyerId) {
        ...
        // 物件番号で物件リストを検索
        const propertyNumbers = buyer.property_number.split(',').map((n) => n.trim()).filter((n) => n);
        if (propertyNumbers.length === 0) {
            return [];
        }
        const { data, error } = await this.supabase
    ...
    async create(buyerData) {
        ...
        try {
            const { SidebarCountsUpdateService } = await Promise.resolve().then(() => __importStar(require('./SidebarCountsUpdateService')));
    ...
    async initBuyerNumberClient() {
        ...
        await sheetsClient.authenticate();
        return new BuyerNumberSpreadsheetClient_1.BuyerNumberSpreadsheetClient(sheetsClient, cell);
    }
    /**
     * 買主番号を自動生成（スプレッドシートの連番シートB2セル+1）
     */
    async generateBuyerNumber() {
        const client = await this.initBuyerNumberClient();
     ...
     */
    shouldUpdateBuyerSidebarCounts(updateData) {
     ...
     */
    async fetchCoordinatesFromPropertyNumber(propertyNumber) {
        try {
            ...
            const { data: property, error } = await this.supabase
                ...
                .maybeSingle();
            if (error || !property?.google_map_url)
                return null;
     ...
     * 買主情報を更新
     */
    async update(id, updateData, userId, userEmail) {
        ...
        if (userId && userEmail) {
            for (const key in finalAllowedData) {
                if (key !== 'db_updated_at' && existing[key] !== finalAllowedData[key]) {
                    try {
                        await AuditLogService_1.AuditLogService.logFieldUpdate('buyer', buyerNumber, key, existing[key], finalAllowedData[key], userId, userEmail);
                    }
                    catch (auditError) {
     ...
     */
    async updateWithSync(id, updateData, userId, userEmail, options) {
        // 存在確認（UUIDか買主番号かを判定）
        const isUuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(id);
        const existing = isUuid
            ? await this.getById(id)
        ...
        if (!options?.force && this.conflictResolver && existing.last_synced_at) {
            ...
            const conflictResult = await this.conflictResolver.checkConflict(buyerNumber, allowedData, expectedValues, new Date(existing.last_synced_at));
        ...
        if (userId && userEmail) {
            for (const key in finalAllowedData) {
                if (key !== 'db_updated_at' && existing[key] !== finalAllowedData[key]) {
                    try {
                        await AuditLogService_1.AuditLogService.logFieldUpdate('buyer', id, key, existing[key], finalAllowedData[key], userId, userEmail, 'pending');
                    }
                    catch (auditError) {
                        console.error('Failed to create audit log:', auditError);
                    }
        ...
        // syncResult の初期値
        let syncResult = { success: true, syncStatus: 'pending' };
        ...
        try {
            ...
            if (this.writeService && this.retryHandler) {
                // リトライ付きで同期を実行
                const retryResult = await this.retryHandler.executeWithRetry(async () => {
     ...
     * @returns バッチ更新結果
     */
    async updateBatch(buyers, userId, userEmail) {
    ...
    async getExportData(options = {}) {
        ...
        let query = this.supabase
            ...
            .select('*');
        if (search) {
            query = query.or(`buyer_number.ilike.%${search}%,name.ilike.%${search}%,phone_number.ilike.%${search}%`);
        }
        if (status) {
            query = query.ilike('latest_status', `%${status}%`);
        }
     ...
     */
    parsePastBuyerList(pastBuyerList) {
     ...
     */
    async getPastBuyerNumbers(buyerId) {
        ...
        const detailsMap = new Map((pastBuyerRecords || []).map(record => [
            record.buyer_number,
            {
     ...
     */
    async getInquiryHistoryByBuyerNumber(buyerNumber) {
        const { data, error } = await this.supabase
            .from('buyers')
     ...
     */
    async getCompleteInquiryHistory(buyerId) {
        const buyer = await this.getById(buyerId);
        if (!buyer) {
            throw new Error(`Buyer not found: ${buyerId}`);
        }
     ...
     */
    async getInquiryHistory(buyerId) {
        ...
        const allPropertyNumbers = [];
        const propertyToBuyerMap = new Map();
        // property_numberなしの関連買主も行として追加するためのリスト（自分自身も含む）
        const noPropertyRelatedBuyers = [];
        ...
        if (buyer.property_number) {
            const currentPropertyNumbers = buyer.property_number
                ...
                .map((n) => n.trim())
                .filter((n) => n);
            currentPropertyNumbers.forEach((propNum) => {
        ...
        if (buyer.email)
            relatedConditions.push(`email.eq.${buyer.email}`);
        if (buyer.phone_number)
            relatedConditions.push(`phone_number.eq.${buyer.phone_number}`);
                ...
                for (const rb of relatedBuyers) {
                    if (rb.property_number) {
                        const relatedPropertyNumbers = rb.property_number
                            ...
                            .map((n) => n.trim())
                            .filter((n) => n);
                        relatedPropertyNumbers.forEach((propNum) => {
        ...
        const pastBuyerNumbers = this.parsePastBuyerList(buyer.past_buyer_list);
        if (pastBuyerNumbers.length > 0) {
            const pastBuyerResults = await Promise.all(pastBuyerNumbers.map(pastBuyerNumber => this.supabase
                .from('buyers')
            ...
            for (const { data: pastBuyer, error: pastBuyerError } of pastBuyerResults) {
                if (!pastBuyerError && pastBuyer && pastBuyer.property_number) {
                    const pastPropertyNumbers = pastBuyer.property_number
                        ...
                        .map((n) => n.trim())
                        .filter((n) => n);
                    pastPropertyNumbers.forEach((propNum) => {
                        allPropertyNumbers.push(propNum);
                        propertyToBuyerMap.set(propNum, {
     ...
     */
    async getBuyersByAreas(areaNumbers, propertyType, salesPrice) {
      ...
      `)
        ...
        const propertyNumbers = [...new Set(sortedBuyers.map((b) => b.property_number).filter(Boolean))];
        let propertyAddressMap = {};
        if (propertyNumbers.length > 0) {
            const { data: properties } = await this.supabase
                .from('property_listings')
    ...
    parseDistributionAreas(distributionAreas) {
    ...
    filterBuyerCandidates(buyers, propertyAreaNumbers, propertyType, salesPrice) {
    ...
    sortBuyersByDateAndConfidence(buyers) {
        return buyers.sort((a, b) => {
            ...
            if (dateA !== dateB)
                return dateB - dateA;
            const confidenceOrder = { 'A': 1, 'B': 2, 'C': 3, 'D': 4, 'E': 5, 'F': 6 };
    ...
    shouldExcludeBuyer(buyer) {
        ...
        if (brokerInquiry !== '業者（両手）' && !this.hasMinimumCriteria(buyer))
            return true;
        return false;
    }
    isBusinessInquiry(buyer) {
        ...
        if (brokerInquiry === '業者（両手）')
            return false;
        if (inquirySource === '業者問い合わせ' || inquirySource.includes('業者'))
            return true;
        if (distributionType === '業者問い合わせ' || distributionType.includes('業者'))
            return true;
        if (brokerInquiry && brokerInquiry !== '' && brokerInquiry !== '0' && brokerInquiry.toLowerCase() !== 'false')
            return true;
        return false;
    }
    hasMinimumCriteria(buyer) {
    ...
    matchesPropertyTypeCriteria(buyer, propertyType) {
        ...
        const normalizedDesiredTypes = desiredType.split(/[,、\s]+/).map((t) => this.normalizePropertyType(t));
        return normalizedDesiredTypes.some((dt) => dt === normalizedPropertyType ||
            normalizedPropertyType.includes(dt) ||
            dt.includes(normalizedPropertyType));
    }
    matchesPriceCriteria(buyer, salesPrice, propertyType) {
        if (!salesPrice)
            return true;
        const propertyTypeMap = {
            'land': '土地',
    ...
    async fetchAllBuyers() {
        ...
        if (countError)
            throw new Error(`Failed to count buyers: ${countError.message}`);
        if (firstBatchResult.error)
            throw new Error(`Failed to fetch buyers: ${firstBatchResult.error.message}`);
        const totalCount = count || 0;
        if (totalCount === 0)
            return [];
        const allBuyers = [...(firstBatchResult.data || [])];
        ...
        const listingsBatchRanges = [];
        for (let i = 0; i < listingsBatchCount; i++) {
            listingsBatchRanges.push([i * PAGE_SIZE, (i + 1) * PAGE_SIZE - 1]);
        }
        const listingsBatchResults = await Promise.all(listingsBatchRanges.map(([from, to]) => this.supabase
            .from('property_listings')
        ...
        if (batchCount > 1) {
            ...
            const batchResults = await Promise.all(remainingRanges.map(([from, to]) => this.supabase
                .from('buyers')
                .select(BUYER_COLUMNS)
                .is('deleted_at', null)
                .range(from, to)));
     ...
     */
    async fetchAllBuyersWithStatus() {
        const now = Date.now();
        ...
        const buyers = allBuyers.map(buyer => {
            try {
                ...
                const statusResult = (0, BuyerStatusCalculator_1.calculateBuyerStatus)(buyer);
     ...
     */
    _buildSidebarCountsFromCache(cachedBuyers) {
        const result = {
            ...
            projectAssigneeOnlyTodayCallCounts: {},
        ...
        for (const buyer of cachedBuyers) {
            ...
            if (status === '内覧日前日')
                ...
                if (match)
                    result.assignedCounts[match[1]] = (result.assignedCounts[match[1]] || 0) + 1;
            }
        }
     ...
     */
    async fetchBuyersForSidebarCounts() {
        const PAGE_SIZE = 1000;
    ...
    async getSidebarCountsFallback() {
        ...
        try {
            ...
            const result = {
                ...
                assignedCounts: {}, // ✅ 買主用：担当別
                todayCallAssignedCounts: {}, // ✅ 買主用：当日TEL担当別（後続担当あり）
                projectAssigneeOnlyTodayCallCounts: {}, // 🆕 案件担当のみの当日TEL（当日TEL直下に表示）
                // 🆕 新カテゴリ（2026年4月）
            ...
            allBuyers.forEach((buyer) => {
                if (buyer.email && String(buyer.email).trim() &&
     ...
     */
    async fetchNormalStaffInitials() {
        try {
            ...
            if (!normalError && staffDataNormal && staffDataNormal.length > 0) {
                return staffDataNormal.map((s) => s.initials).filter((i) => i);
            }
            const { data: allStaffData } = await this.supabase
                .from('employees')
                .select('initials');
            return (allStaffData || []).map((s) => s.initials).filter((i) => i);
        }
        catch {
            return [];
        }
     ...
     */
    buildCategoriesFromCounts(counts) {
        const categories = [];
        // 基本カテゴリ
        categories.push({
            status: '内覧日前日',
     ...
     */
    async buildCategoriesFromBuyers(allBuyers) {
        ...
        const categories = [];
        buyer_status_definitions_1.STATUS_DEFINITIONS.forEach((definition) => {
            categories.push({
        ...
        const assigneePriorityMap = new Map();
        const knownAssigneePriorities = {
     ...
     * 特定のステータスに該当する買主を取得
     */
    async getBuyersByStatus(status, options = {}) {
        try {
            ...
            if (todayCallAssignedMatch) {
                ...
                filteredBuyers = allBuyers.filter((buyer) => {
                    return (buyer.email && String(buyer.email).trim() &&
                        buyer.inquiry_property_price !== null &&
                        buyer.inquiry_property_price !== undefined &&
                        Number(buyer.inquiry_property_price) <= 5000000 &&
                        (!buyer.pinrich_500man_registration || buyer.pinrich_500man_registration === '未') &&
                ...
                filteredBuyers = allBuyers.filter((buyer) => buyer.viewing_unconfirmed === '未確定');
                ...
                filteredBuyers = allBuyers.filter((buyer) => {
                    if (buyer.seller_viewing_date_contact !== '未')
                        return false;
                ...
                filteredBuyers = allBuyers.filter((buyer) => buyer.calculated_status === targetStatus);
                ...
                filteredBuyers = allBuyers.filter(buyer => {
                    ...
                    const matches = (buyer.follow_up_assignee === assignee ||
            ...
            filteredBuyers = filteredBuyers.filter(buyer => {
                ...
                return ((buyer.buyer_number || '').toLowerCase().includes(s) ||
     ...
     */
    async permanentDelete(buyerIdOrNumber) {
        ...
        if (error)
            throw new Error(`Failed to permanently delete buyer: ${error.message}`);
    }
    /**
     * 論理削除した買主を復元
     */
    async restore(buyerId) {
        const { data, error } = await this.supabase
            .from('buyers')
     ...
     * 近隣物件を取得（類似物件機能）
     */
    async getNearbyProperties(propertyNumber) {
        // 基準物件を取得
        ...
        const baseLat = baseProperty.latitude;
        const baseLng = baseProperty.longitude;
        ...
        const isValidStatus = (status) => {
            if (!status)
                return true;
            if (status.includes('非公開（配信メールのみ）'))
                return true;
            if (status.includes('非公開'))
                return false;
            return true;
        };
        // バウンディングボックスは正方形なので、実際の距離でフィルタリング＋ステータスフィルタ
        const nearbyProperties = (candidates || []).filter((p) => {
            if (!isValidStatus(p.atbb_status))
                return false;
    ...
    calcDistanceKm(lat1, lng1, lat2, lng2) {
        const R = 6371;
     ...
     * @returns 更新結果
     */
    async updateSidebarCountsTable() {
        const startTime = Date.now();
        ...
        try {
            ...
            for (const [assignee, count] of Object.entries(categoryCounts.assignedCounts || {})) {
                rows.push({ category: 'assigned', count: count, label: null, assignee, updated_at: now });
            }
            // 当日TEL担当別カテゴリ
            for (const [assignee, count] of Object.entries(categoryCounts.todayCallAssignedCounts || {})) {
                rows.push({ category: 'todayCallAssigned', count: count, label: null, assignee, updated_at: now });
            }
            // 次電日空欄担当別カテゴリ
            for (const [assignee, count] of Object.entries(categoryCounts.nextCallDateBlankCounts || {})) {
                rows.push({ category: 'nextCallDateBlank', count: count, label: null, assignee, updated_at: now });
            }
    ...
    async getOtherCompanyDistributionBuyers(params) {
        ...
        if (area) {
            ...
            if (!isCircledNumber) {
                ...
                try {
                    const { GeocodingService } = await Promise.resolve().then(() => __importStar(require('./GeocodingService')));
                    ...
                    if (coordinates) {
                        ...
                        while (true) {
                            ...
                            const withinRadius = page.filter(buyer => {
                                const distance = this.calcDistanceKm(coordinates.lat, coordinates.lng, buyer.desired_area_lat, buyer.desired_area_lng);
            ...
            if (propertyError) {
                ...
                if (nearbyError) {
                    ...
                    const buyersWithinRadius = nearbyBuyers.filter(buyer => {
                        const distance = this.calcDistanceKm(propertyLat, propertyLng, buyer.desired_area_lat, buyer.desired_area_lng);
                        return distance <= RADIUS_KM;
                    });
        ...
        if (allBuyers.length > 0) {
            console.log('[getOtherCompanyDistributionBuyers] Sample buyers (first 3):', allBuyers.slice(0, 3).map(b => ({
        ...
        const propertyTypeFiltered = allBuyers.filter(buyer => {
            ...
            const matches = dbTypes.some(dbType => buyer.desired_property_type.includes(dbType));
     ...
     * 指定なし: 全件
     */
    static filterByParking(buyers, parking) {
        if (parking === '指定なし')
            return buyers;
        if (parking === '不要') {
            return buyers.filter(b => {
     ...
     * どちらでも: 全件
     */
    static filterByOnsen(buyers, onsen) {
        if (onsen === 'どちらでも')
            return buyers;
        if (onsen === 'あり') {
     ...
     * どちらでも: 全件
     */
    static filterByFloor(buyers, floor) {
        if (floor === 'どちらでも')
            return buyers;
        if (floor === '高層階') {
            return buyers.filter(b => {
    ...
    filterByPriceRange(buyers, priceRange, propertyTypes) {
        if (priceRange === '指定なし') {
     ...
     * 価格帯マッチング
     */
    matchesPriceRange(buyerPrice, priceRange) {
    ...
    async getBuyersByRadiusSearch(params) {
        const { address, priceRange, propertyTypes, pet = 'どちらでも', parking = '指定なし', onsen = 'どちらでも', floor = 'どちらでも', } = params;
        ...
        const { GeocodingService: GeocodingServiceClass } = await Promise.resolve().then(() => __importStar(require('./GeocodingService')));
        ...
        const buyersWithinRadius = (allBuyers || []).filter(buyer => {
            const distance = geocodingService.calculateDistance(coordinates.lat, coordinates.lng, buyer.desired_area_lat, buyer.desired_area_lng);
            return distance <= RADIUS_KM;
        });
        ...
        // 買主ごとの問い合わせ物件番号を取得
        const propertyNumberMap = new Map();
        ...
        const sortedBuyers = filteredBuyers.map(buyer => {
            ...
            return {
                ...buyer,
                distance: geocodingService.calculateDistance(coordinates.lat, coordinates.lng, buyer.desired_area_lat, buyer.desired_area_lng),
     ...
     */
    groupByMonthAndAssignee(buyers) {
     ...
     * 月ごとの統計を計算
     */
    calculateMonthlyStatistics(groupedData) {
        const statistics = [];
        for (const [month, assigneeData] of groupedData.entries()) {
            ...
            for (const [assignee, buyers] of assigneeData.entries()) {
                ...
                const purchaseCount = buyers.filter(b => b.latest_status && b.latest_status.includes('買（')).length;
                // 内覧数を集計（重複排除）
     ...
     */
    getUniqueViewings(buyers) {
        const uniqueViewings = new Set();
        for (const buyer of buyers) {
    ...
    async validatePropertyNumber(propertyNumber) {
        const { data, error } = await this.supabase
            ...
            .maybeSingle();
        if (error)
            throw new Error(`Failed to validate property number: ${error.message}`);
        return data !== null;
    }
}
