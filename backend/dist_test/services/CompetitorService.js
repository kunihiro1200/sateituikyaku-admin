...
exports.competitorService = exports.CompetitorService = void 0;
...
class CompetitorService {
     ...
     */
    async recordCompetitor(sellerId, competitorData) {
        try {
            const updateData = {};
            ...
            const { data, error } = await supabase
                ...
                .update(updateData)
                .eq('id', sellerId)
                .select(`competitor_name_and_reason, competitor_name, exclusive_other_decision_factor,
     ...
     * @returns Updated competitor information
     */
    async updateContractYearMonth(sellerId, yearMonth) {
        try {
            const { data, error } = await supabase
                .from('sellers')
     ...
     * @returns Updated competitor information
     */
    async updateMeetingStatus(sellerId, status) {
        try {
     ...
     */
    async getCommonOtherDecisionFactors() {
        try {
            const { data, error } = await supabase
                .from('sellers')
     ...
     */
    async analyzeCompetitorPerformance(startDate, endDate) {
        try {
            let query = supabase
                .from('sellers')
     ...
     */
    getSuggestedCountermeasures(competitorName, factor) {
        const suggestions = [];
        ...
        if (competitorName) {
            ...
            const matched = Object.keys(competitorSuggestions).find((key) => competitorName.includes(key));
            if (matched) {
        ...
        if (suggestions.length === 0) {
            suggestions.push('顧客ニーズの再確認', '提案内容の見直し', '追加サービスの提案', '担当者との面談設定');
        }
        return suggestions;
    }
    /**
     ...
     */
    isCompetitorInfoRequired(status) {
     ...
     */
    validateCompetitorInfo(competitorData, status) {
        const errors = [];
        ...
        if (!competitorData.exclusiveOtherDecisionFactor ||
        ...
        if (!competitorData.otherDecisionCountermeasure ||
        ...
        if (!competitorData.exclusiveOtherDecisionMeeting ||
     ...
     */
    getExclusiveOtherDecisionFactorOptions() {
        return [
            '①知り合い',
            '②価格が高い',
            '③決定権者の把握',
...
exports.competitorService = new CompetitorService();
