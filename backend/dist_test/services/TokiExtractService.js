...
exports.TokiExtractService = void 0;
...
class TokiExtractService {
     ...
     */
    async findTokiPdf(storageFolderUrl) {
        try {
            // URLからフォルダIDを抽出
            ...
            const tokiFile = files.find((f) => (f.name.includes('全部事項') || f.name.includes('全部謄本')) &&
     ...
     */
    async findTokiPdfForKeiyaku(storageFolderUrl) {
        try {
            const folderId = this.extractFolderIdFromUrl(storageFolderUrl);
            if (!folderId) {
            ...
            let tokiFile = files.find((f) => f.name.includes('建物_全部事項') &&
                (f.mimeType === 'application/pdf' || f.name.toLowerCase().endsWith('.pdf')));
            if (!tokiFile) {
                // フォールバック：「全部事項」または「全部謄本」を含むPDFを検索
                tokiFile = files.find((f) => (f.name.includes('全部事項') || f.name.includes('全部謄本')) &&
                    (f.mimeType === 'application/pdf' || f.name.toLowerCase().endsWith('.pdf')));
            }
            if (!tokiFile) {
     ...
     */
    async extractFromPdf(pdfBase64) {
...

{
  ...
  "construction_date": null
     ...
     */
    async extractFromPdfForKodate(pdfBase64) {
        const apiKey = process.env.ANTHROPIC_API_KEY;
        if (!apiKey) {
            throw new Error('ANTHROPIC_API_KEY が設定されていません');
        }
...

{
  ...
  "renovation_date": null
     ...
     */
    async writeToSpreadsheetForKodate(req) {
        const { spreadsheetUrl, sheetName, extractResult } = req;
        const spreadsheetId = this.extractSpreadsheetId(spreadsheetUrl);
        if (!spreadsheetId) {
            throw new Error(`スプレッドシートIDの抽出に失敗しました: ${spreadsheetUrl}`);
     ...
     */
    async extractFromPdfForKeiyaku(pdfBase64) {
        const apiKey = process.env.ANTHROPIC_API_KEY;
        if (!apiKey) {
            throw new Error('ANTHROPIC_API_KEY が設定されていません');
        }
...

{
  ...
  "remaining_rights": []
        ...
        const remainingRights = Array.isArray(raw.remaining_rights) ? raw.remaining_rights : [];
        return {
            ...
            rightChecks: remainingRights.map((rightName) => ({
                cell: rightName,
                value: true,
            })),
        };
     ...
     */
    async writeToSpreadsheetForKeiyaku(req) {
        const { spreadsheetUrl, sheetName, extractResult } = req;
        const spreadsheetId = this.extractSpreadsheetId(spreadsheetUrl);
        if (!spreadsheetId) {
            throw new Error(`スプレッドシートIDの抽出に失敗しました: ${spreadsheetUrl}`);
     ...
     * ダウンロード・解析はしない
     */
    async listTokiPdfsForKodate(storageFolderUrl) {
        const folderId = this.extractFolderIdFromUrl(storageFolderUrl);
        if (!folderId)
            return [];
        const files = await this.driveService.listFiles(folderId);
        ...
        if (landFiles.length === 0 && buildingFile === null) {
            ...
            if (fallback)
                result.push({ fileId: fallback.id, fileName: fallback.name, pdfType: 'unknown' });
        }
        else {
            for (const f of landFiles)
                result.push({ fileId: f.id, fileName: f.name, pdfType: 'land' });
            if (buildingFile)
                result.push({ fileId: buildingFile.id, fileName: buildingFile.name, pdfType: 'building' });
        }
        return result;
    }
    /**
     ...
     */
    async extractSingleTokiPdfForKodate(fileId, fileName) {
        const fileData = await this.driveService.getFile(fileId);
        if (!fileData)
            throw new Error(`PDFの取得に失敗しました: ${fileName}`);
        const base64 = fileData.data.toString('base64');
     ...
     */
    async findTokiPdfsForKodate(storageFolderUrl) {
        try {
            const folderId = this.extractFolderIdFromUrl(storageFolderUrl);
            if (!folderId) {
            ...
            const fallbackFile = landFiles.length === 0 && buildingFile === null
                ? (pdfFiles.find((f) => f.name.includes('全部事項') || f.name.includes('全部謄本')) ?? null)
                : null;
            console.log(`[TokiKodate] 土地謄本: ${landFiles.length}件, 建物謄本: ${buildingFile ? 1 : 0}件, フォールバック: ${fallbackFile ? fallbackFile.name : 'なし'}`);
            // 土地PDFを取得
            const landPdfs = [];
            for (const f of landFiles) {
     ...
     */
    async extractFromPdfsForKodate(landPdfs, buildingPdf) {
        ...
        const [landResults, buildingResult] = await Promise.all([
            // 土地謄本：複数を並列処理
            Promise.all(landPdfs.map(async (pdf) => {
        ...
        allLands.sort((a, b) => {
            if (a.areaNumeric === null && b.areaNumeric === null)
                return 0;
            if (a.areaNumeric === null)
                return 1;
            if (b.areaNumeric === null)
                return -1;
    ...
    async findTokiPdfsForKodateKeiyaku(storageFolderUrl) {
        try {
            const folderId = this.extractFolderIdFromUrl(storageFolderUrl);
            if (!folderId) {
            ...
            const tokiFiles = files.filter((f) => (f.name.includes('全部事項') || f.name.includes('全部謄本')) &&
                (f.mimeType === 'application/pdf' || f.name.toLowerCase().endsWith('.pdf')));
     ...
     */
    async extractFromPdfForKodateKeiyaku(pdfBase64) {
        const apiKey = process.env.ANTHROPIC_API_KEY;
        if (!apiKey)
            throw new Error('ANTHROPIC_API_KEY が設定されていません');
        const client = new sdk_1.default({ apiKey });
...

{
  ...
  "remaining_rights": []
     ...
     */
    async listTokiPdfsForKodateKeiyaku(storageFolderUrl) {
        const folderId = this.extractFolderIdFromUrl(storageFolderUrl);
        if (!folderId)
            return [];
        const files = await this.driveService.listFiles(folderId);
     ...
     */
    async extractSingleTokiPdfForKodateKeiyaku(fileId, fileName) {
        const fileData = await this.driveService.getFile(fileId);
        if (!fileData)
            throw new Error(`PDFの取得に失敗しました: ${fileName}`);
        const base64 = fileData.data.toString('base64');
     ...
     */
    async extractFromPdfsForKodateKeiyaku(pdfs) {
        if (pdfs.length === 0)
            throw new Error('謄本PDFが見つかりませんでした');
     ...
     */
    async writeToSpreadsheetForKodateKeiyaku(req) {
        const { spreadsheetUrl, sheetName, extractResult } = req;
        const spreadsheetId = this.extractSpreadsheetId(spreadsheetUrl);
        if (!spreadsheetId)
            throw new Error(`スプレッドシートIDの抽出に失敗しました: ${spreadsheetUrl}`);
     ...
     */
    async listTokiPdfsForTochi(storageFolderUrl) {
        const folderId = this.extractFolderIdFromUrl(storageFolderUrl);
        if (!folderId)
            return [];
        const files = await this.driveService.listFiles(folderId);
     ...
     */
    async extractSingleTokiPdfForTochi(fileId, fileName) {
        const fileData = await this.driveService.getFile(fileId);
        if (!fileData)
            throw new Error(`PDFの取得に失敗しました: ${fileName}`);
        const base64 = fileData.data.toString('base64');
     ...
     */
    async extractFromPdfForTochi(pdfBase64) {
        const apiKey = process.env.ANTHROPIC_API_KEY;
        if (!apiKey) {
            throw new Error('ANTHROPIC_API_KEY が設定されていません');
        }
...

{
  ...
  "is_shared_ownership": false
     ...
     * シート：専任媒介契土地（売却)
     */
    async writeToSpreadsheetForTochi(req) {
        const { spreadsheetUrl, sheetName, extractResult } = req;
        const spreadsheetId = this.extractSpreadsheetId(spreadsheetUrl);
        if (!spreadsheetId) {
            throw new Error(`スプレッドシートIDの抽出に失敗しました: ${spreadsheetUrl}`);
...
exports.TokiExtractService = TokiExtractService;
