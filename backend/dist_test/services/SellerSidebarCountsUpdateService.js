 ...
 */
class SellerSidebarCountsUpdateService {
     ...
     * 3. 手動実行（デバッグ用）
     */
    async updateSellerSidebarCounts() {
        ...
        try {
            ...
            const hasValidVisitAssignee = sellerCategoryFilters_1.hasValidVisitAssignee;
            const isFiSeller = sellerCategoryFilters_1.isFiSeller;
            // 全データを並列取得
            // 🔧 訪問日前日用データはページネーション対応（1000件超の場合に対応）
            let visitAssigneeAllSellers = [];
            {
                let vdbPage = 0;
            ...
            const visitDayBeforeCount = visitAssigneeSellers.filter(s => {
                ...
                if (!visitDateStr || visitDateStr.trim() === '')
                    return false;
     ...
     */
    async updateAffectedCategories(updatedFields) {
        const startTime = Date.now();
        // 影響カテゴリを収集（重複排除）
        ...
        const todayJST = `${jstTime.getUTCFullYear()}-${String(jstTime.getUTCMonth() + 1).padStart(2, '0')}-${String(jstTime.getUTCDate()).padStart(2, '0')}`;
        const cutoffDate = '2025-12-08';
        const hasValidVisitAssignee = (v) => !!(v && v.trim() !== '' && v.trim() !== '外す');
        // 影響カテゴリに必要なクエリだけ並列実行
        const queries = {};
        ...
        if (needsTodayCallBase) {
            queries.todayCallBase1 = this.supabase
                .from('sellers')
                .select('id, seller_number, visit_assignee, phone_contact_person, preferred_contact_time, contact_method, unreachable_status, inquiry_date, status')
                .is('deleted_at', null)
                .ilike('status', '%追客中%')
                .not('next_call_date', 'is', null)
                .lte('next_call_date', todayJST);
        ...
        if (needsTodayCallBase) {
            ...
            const filtered = merged.filter(s => {
                if ((s.status || '').includes('追客不要'))
                    return false;
                return !hasValidVisitAssignee(s.visit_assignee);
            });
            // FI（福岡）売主と通常売主を分離
            const isFi = (s) => (s.seller_number || '').startsWith('FI');
            const filteredNormal = filtered.filter(s => !isFi(s));
            const filteredFI = filtered.filter(s => isFi(s));
            const withInfo = filteredNormal.filter(s => (0, sellerCategoryFilters_1.isTodayCallWithInfo)(s, todayJST));
            const notStarted = filteredNormal.filter(s => (0, sellerCategoryFilters_1.isTodayCallNotStarted)(s, todayJST));
            const noInfo = filteredNormal.filter(s => (0, sellerCategoryFilters_1.isTodayCallNoInfo)(s, todayJST));
            // todayCallWithInfo ラベル別
            const isValidValue = (v) => !!(v && v.trim() !== '' && v.trim().toLowerCase() !== 'null');
            const labelCountMap = {};
            withInfo.forEach(s => {
                const label = (0, sellerCategoryFilters_1.getTodayCallWithInfoLabel)(s);
                labelCountMap[label] = (labelCountMap[label] || 0) + 1;
            ...
            // FI専用カウント（共通関数を使用）
            const fi_withInfo = filteredFI.filter(s => (0, sellerCategoryFilters_1.isTodayCallWithInfo)(s, todayJST));
            const fi_notStarted = filteredFI.filter(s => (0, sellerCategoryFilters_1.isTodayCallNotStarted)(s, todayJST));
            const fi_noInfo = filteredFI.filter(s => (0, sellerCategoryFilters_1.isTodayCallNoInfo)(s, todayJST));
            const fi_labelCountMap = {};
            ...
            if (affected.has('todayCall')) {
                ...
                upsertRows.push({ category: 'fi_todayCall', count: fi_noInfo.length, label: null, assignee: null });
            }
            if (affected.has('todayCallWithInfo')) {
                ...
                upsertRows.push({ category: 'fi_todayCallWithInfo', count: fi_withInfo.length, label: null, assignee: null });
                Object.entries(fi_labelCountMap).forEach(([label, count]) => {
                    upsertRows.push({ category: 'fi_todayCallWithInfo', count, label, assignee: null });
                });
            }
            if (affected.has('todayCallNotStarted')) {
                ...
                upsertRows.push({ category: 'fi_todayCallNotStarted', count: fi_notStarted.length, label: null, assignee: null });
            }
        }
        // visitDayBefore
        if (needsVisitDayBefore) {
            ...
            const count = sellers.filter((s) => {
                ...
                if (!visitDateStr)
                    return false;
                if ((s.visit_reminder_assignee || '').trim() !== '')
                    return false;
                let visitDateOnly = visitDateStr;
                if (visitDateStr.includes(' '))
        ...
        if (needsTodayCallAssigned) {
            ...
            const counts = {};
            sellers.forEach((s) => { if (s.visit_assignee)
                counts[s.visit_assignee] = (counts[s.visit_assignee] || 0) + 1; });
        ...
        if (needsUnvaluated) {
            ...
            const isFiSeller = (s) => (s.seller_number || '').startsWith('FI');
            const unvaluatedFilterFn = (s) => {
                if (s.valuation_amount_1 || s.valuation_amount_2 || s.valuation_amount_3)
                    return false;
            ...
            const count = sellers.filter((s) => !isFiSeller(s) && unvaluatedFilterFn(s)).length;
            const fi_count = sellers.filter((s) => isFiSeller(s) && unvaluatedFilterFn(s)).length;
            upsertRows.push({ category: 'unvaluated', count, label: null, assignee: null });
            upsertRows.push({ category: 'fi_unvaluated', count: fi_count, label: null, assignee: null });
        }
        // mailingPending
        if (needsMailingPending) {
        ...
        // exclusive
        if (needsExclusive) {
            const count = (resultMap.exclusive?.data || []).filter((s) => {
                if (s.exclusive_other_decision_meeting === '完了')
                    return false;
        ...
        // general
        if (needsGeneral) {
            const count = (resultMap.general?.data || []).filter((s) => {
                if (s.exclusive_other_decision_meeting === '完了')
                    return false;
                return !s.next_call_date || s.next_call_date !== todayJST;
            }).length;
        ...
        // visitOtherDecision
        if (needsVisitOtherDecision) {
            const count = (resultMap.visitOtherDecision?.data || []).filter((s) => {
                if (s.exclusive_other_decision_meeting === '完了')
                    return false;
                return !s.next_call_date || s.next_call_date !== todayJST;
            }).length;
        ...
        if (needsUnvisitedOtherDecision) {
            const count = (resultMap.unvisitedOtherDecision?.data || []).filter((s) => {
                if (s.exclusive_other_decision_meeting === '完了')
                    return false;
        ...
        if (needsPinrichChange) {
            ...
            const count = (resultMap.pinrichChange?.data || []).filter((s) => {
                const pinrich = s.pinrich_status || '';
                const status = s.status || '';
                const assignee = s.visit_assignee || '';
                const confidence = s.confidence_level || '';
...
exports.SellerSidebarCountsUpdateService = SellerSidebarCountsUpdateService;
