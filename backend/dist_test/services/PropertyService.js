...
const propertyBySellerCache = new Map();
...
class PropertyService {
     ...
     */
    async getProperty(propertyId, includeDeleted = false) {
        try {
            let query = supabase
                .from('properties')
                .select('*')
     ...
     */
    async getPropertyBySellerId(sellerId, includeDeleted = false) {
        // キャッシュ確認（60秒TTL）
    ...
    invalidatePropertyBySellerCache(sellerId) {
    ...
    async updateProperty(propertyId, updates) {
        try {
            ...
            if (updates.address !== undefined)
                updateData.property_address = updates.address;
            if (updates.prefecture !== undefined)
                updateData.prefecture = updates.prefecture;
            ...
            if (updates.landArea !== undefined)
                updateData.land_area = updates.landArea;
            if (updates.buildingArea !== undefined)
                updateData.building_area = updates.buildingArea;
            ...
            if (data.seller_id && (updates.landAreaVerified !== undefined || updates.buildingAreaVerified !== undefined)) {
                ...
                if (updates.landAreaVerified !== undefined)
                    sellerUpdateData.land_area_verified = updates.landAreaVerified;
                if (updates.buildingAreaVerified !== undefined)
                    sellerUpdateData.building_area_verified = updates.buildingAreaVerified;
            ...
            if (data.seller_id) {
                await cache_1.CacheHelper.del(`seller:${data.seller_id}`);
                ...
                const { invalidateSellerCache, invalidateListSellersCache } = await Promise.resolve().then(() => __importStar(require('./SellerService.supabase')));
                ...
                try {
                    const { SellerService } = await Promise.resolve().then(() => __importStar(require('./SellerService.supabase')));
                    const syncQueue = SellerService.sharedSyncQueue;
                    if (syncQueue) {
     ...
     * 英語値・略語・日本語を全て日本語に統一
     */
    normalizePropertyTypeToJapanese(type) {
        if (!type)
            return null;
        const map = {
            // 英語値 → 日本語
    ...
    validatePropertyType(propertyType) {
        const validTypes = Object.values(types_1.PropertyType);
     ...
     * @returns true if valid
     */
    validateSellerSituation(situation) {
     ...
     */
    isIeulMansion(inquirySite, propertyType) {
        return inquirySite === 'ウ' && propertyType === types_1.PropertyType.APARTMENT;
    }
    /**
     * Map database record to PropertyInfo type
     */
    mapToPropertyInfo(data) {
        return {
            id: data.id,
            sellerId: data.seller_id,
     ...
     */
    async getSettlementDate(propertyNumber) {
        try {
            // キャッシュをチェック
            ...
            if (!rowIndex) {
                ...
                // nullもキャッシュ（5分間）
                await cache_1.CacheHelper.set(cacheKey, null, 300);
                return null;
            }
            // E列（決済日）を取得
            ...
            // キャッシュに保存（5分間）
            await cache_1.CacheHelper.set(cacheKey, settlementDate, 300);
     ...
     * @returns 説明文またはnull
     */
    async getPropertyAbout(propertyNumber) {
        try {
            // キャッシュをチェック
            ...
            // キャッシュに保存（5分間）
            await cache_1.CacheHelper.set(cacheKey, aboutValue, 300);
            return aboutValue;
        }
        catch (error) {
     ...
     * @returns PDFのURL
     */
    async generateEstimatePdf(propertyNumber) {
        ...
        try {
            ...
            try {
                await cache_1.CacheHelper.set(cacheKey, pdfUrl, 300);
            }
            catch (cacheError) {
                // Redisが利用できない場合はキャッシュをスキップ（Vercel環境など）
     ...
     */
    async waitForCalculationCompletion(sheets, spreadsheetId, sheetName) {
     ...
     * シート名からシートIDを取得
     */
    async getSheetId(sheetsClient, sheetName) {
     ...
     */
    exportSheetAsPdf(spreadsheetId, sheetId, propertyNumber) {
     ...
     */
    async retrieveStorageUrl(propertyNumber) {
        try {
            const { PropertyImageService } = await Promise.resolve().then(() => __importStar(require('./PropertyImageService')));
     ...
     * @returns 成功した場合はtrue
     */
    async updateStorageUrl(propertyNumber, storageUrl) {
        try {
            const { createClient } = await Promise.resolve().then(() => __importStar(require('@supabase/supabase-js')));
            const supabase = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_KEY);
            const { error } = await supabase
