...
exports.BuyerCandidateService = void 0;
// 買主候補抽出サービス
const supabase_js_1 = require("@supabase/supabase-js");
const BeppuAreaMappingService_1 = require("./BeppuAreaMappingService");
...
class BuyerCandidateService {
    constructor() {
        this.supabase = (0, supabase_js_1.createClient)(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_KEY);
        this.beppuAreaMappingService = new BeppuAreaMappingService_1.BeppuAreaMappingService();
     ...
     * 物件に対する買主候補を取得
     */
    async getCandidatesForProperty(propertyNumber, overridePropertyType) {
        ...
        const candidates = await this.filterCandidatesOptimized(buyers || [], effectivePropertyType, property.sales_price, propertyAreaNumbers, propertyCoords);
        ...
        filtered = BuyerService_1.BuyerService.filterByPet(filtered, pet);
        filtered = BuyerService_1.BuyerService.filterByParking(filtered, parking);
        filtered = BuyerService_1.BuyerService.filterByOnsen(filtered, onsen);
        filtered = BuyerService_1.BuyerService.filterByFloor(filtered, floor);
        ...
        limitedCandidates.forEach(b => {
            if (b.property_number) {
                const numbers = b.property_number.split(',').map((n) => n.trim()).filter((n) => n);
        ...
        const candidatesWithAddress = limitedCandidates.map(b => {
            let inquiryPropertyAddress = null;
            let inquiryPropertyPrice = null;
            if (b.property_number) {
     ...
     */
    async getPropertyAddressesInBatch(propertyNumbers) {
     ...
     */
    filterCandidates(buyers, propertyType, salesPrice, propertyAreaNumbers, propertyCoords) {
     ...
     */
    async filterCandidatesAsync(buyers, propertyType, salesPrice, propertyAreaNumbers, propertyCoords) {
        const candidates = [];
     ...
     */
    async filterCandidatesOptimized(buyers, propertyType, salesPrice, propertyAreaNumbers, propertyCoords) {
        const candidates = [];
        const candidatesWithoutDistance = [];
     ...
     */
    async matchesAreaCriteriaWithDistance(buyer, propertyAreaNumbers, propertyCoords) {
     ...
     */
    async matchesByInquiryDistance(buyer, propertyCoords, geocodingCache) {
        try {
    ...
    /**
       ...
       * 3. 市全体のデフォルト（大分市㊵、別府市㊶）
       */
    async getAreaNumbersForProperty(property) {
     ...
     * DBに保存済みの latitude/longitude カラムのみ使用（リアルタイムURL展開は行わない）
     */
    async getPropertyCoordsFromAddress(property) {
        try {
...
exports.BuyerCandidateService = BuyerCandidateService;
