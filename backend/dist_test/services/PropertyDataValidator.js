...
exports.PropertyDataValidator = void 0;
const database_1 = __importDefault(require("../config/database"));
class PropertyDataValidator {
    constructor(dbPool = database_1.default) {
        this.pool = dbPool;
    }
    /**
     ...
     */
    async validatePropertyListing(propertyId) {
        try {
            const result = await this.pool.query('SELECT id FROM property_listings WHERE property_id = $1 LIMIT 1', [propertyId]);
     ...
     */
    async generateReport(properties) {
        const reports = [];
        for (const property of properties) {
            ...
            if (hasPropertyListing) {
                try {
                    const result = await this.pool.query('SELECT distribution_areas FROM property_listings WHERE property_id = $1', [property.id]);
            ...
            const issues = [...validation.warnings, ...validation.errors];
            if (!hasPropertyListing) {
    ...
    async generateSummary() {
      ...
      `);
            ...
            for (const row of result.rows) {
                ...
                if (!hasGoogleMapUrl)
                    summary.missingGoogleMapUrl++;
                if (!hasCity)
                    summary.missingCity++;
                if (!hasListing)
                    summary.missingPropertyListing++;
     ...
     */
    async findPropertiesWithIssues() {
        try {
...
exports.PropertyDataValidator = PropertyDataValidator;
exports.default = PropertyDataValidator;
