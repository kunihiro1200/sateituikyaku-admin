...
class AuthService extends BaseRepository_1.BaseRepository {
     ...
     */
    async getOrCreateEmployee(userId, email, userMetadata) {
        const extractedName = (0, employeeUtils_1.extractDisplayName)(userMetadata, email);
        ...
        if (existingByEmail) {
            ...
            if (this.isInvalidName(existingByEmail.name))
                updates.name = extractedName;
