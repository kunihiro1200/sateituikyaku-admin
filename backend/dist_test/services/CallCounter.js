...
const TimestampParser_1 = require("./TimestampParser");
class CallCounter {
    constructor() {
        this.timestampParser = new TimestampParser_1.TimestampParser();
    }
    /**
     ...
     */
    countCallsFromHistory(activities) {
        const phoneCalls = activities.filter(activity => {
            ...
            return activityType === 'phone_call' || activityType === types_1.ActivityType.PHONE_CALL;
        });
     ...
     */
    countCallsFromComments(comments) {
        const allTimestamps = [];
     ...
     */
    countTotalCalls(activities, comments) {
        ...
        const deduplicatedCount = this.deduplicateTimestamps(historyResult.timestamps, commentsResult.timestamps);
        return {
     ...
     */
    deduplicateTimestamps(historyTimestamps, commentTimestamps) {
     ...
     */
    countSimple(activities, comments) {
     ...
     */
    getAllTimestamps(activities, comments) {
        const historyResult = this.countCallsFromHistory(activities);
        const commentsResult = this.countCallsFromComments(comments);
        ...
        const uniqueTimestamps = Array.from(new Set(allTimestamps.map(d => d.getTime()))).map(t => new Date(t));
        // Sort newest first
