...
exports.fetchPopulationData = fetchPopulationData;
exports.estimateAreaRatio = estimateAreaRatio;
 ...
 * @param year 年（ログ用）
 */
async function fetchCensusPopulation(cityCode, statsDataId, year, appId) {
    try {
        ...
        const totalEntry = values.find((v) => (v['@cat01'] === '0' || v['@cat01'] === '00010') &&
            (!v['@cat02'] || v['@cat02'] === '0000' || v['@cat02'] === '000') &&
            (!v['@cat03'] || v['@cat03'] === '0000' || v['@cat03'] === '000')) || values[0];
 ...
 */
async function fetchPopulationData(cityName, areaRatio = 0.05 // エリアが市全体に占める割合の推定（デフォルト5%）
) {
    ...
    result.forEach(r => {
        if (r.area === 0)
            r.area = Math.round(r.city * areaRatio);
    });
 ...
 * 町名の規模感から大まかな比率を返す
 */
function estimateAreaRatio(cityName, townName) {
    // 大都市ほど1エリアの比率は小さい
    const bigCities = ['大阪市', '名古屋市', '横浜市', '福岡市', '札幌市', '仙台市', '広島市', '京都市'];
    const midCities = ['大分市', '別府市', '熊本市', '長崎市', '鹿児島市', '宮崎市', '佐賀市'];
    if (bigCities.some(c => cityName.includes(c)))
        return 0.02;
    if (midCities.some(c => cityName.includes(c)))
        return 0.04;
    return 0.06; // 小都市
}
/**
 ...
 */
async function fetchReinfolibYear(cityCode, year, apiKey) {
    let totalCount = 0;
    const unitPrices = [];
    const tradePrices = [];
 ...
 */
async function fetchTransactionData(cityName, areaRatio = 0.05) {
    ...
    const results = await Promise.all(years.map(y => fetchReinfolibYear(cityCode, y, apiKey)));
 ...
 */
async function fetchPriceData(cityName, areaVariation = 1.08 // エリアが市平均より高い場合の係数
) {
    const apiKey = process.env.REINFOLIB_API_KEY;
    if (!apiKey) {
        console.warn('[EStatService] REINFOLIB_API_KEY not set');
