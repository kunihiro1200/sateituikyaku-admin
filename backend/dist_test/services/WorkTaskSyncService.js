...
exports.WorkTaskSyncService = void 0;
const supabase_js_1 = require("@supabase/supabase-js");
const googleapis_1 = require("googleapis");
const WorkTaskColumnMapper_1 = require("./WorkTaskColumnMapper");
...
class WorkTaskSyncService {
    ...
    constructor() {
        ...
        this.columnMapper = new WorkTaskColumnMapper_1.WorkTaskColumnMapper();
    }
    /**
    ...
    async initSheetsClient() {
        ...
        // 4. いずれの認証情報も存在しない場合
        const errorMessage = 'Google Sheets 認証情報が設定されていません。' +
     ...
     * 単一の物件番号でデータを同期
     */
    async syncByPropertyNumber(propertyNumber) {
        try {
            const sheets = await this.initSheetsClient();
            // ヘッダー行を取得
            const headerResponse = await sheets.spreadsheets.values.get({
     ...
     * DBの変更をスプレッドシートに書き戻す
     */
    async writeBackToSpreadsheet(propertyNumber, updates) {
        try {
            const sheets = await this.initSheetsClient();
            // ヘッダー行を取得
            const headerResponse = await sheets.spreadsheets.values.get({
...
exports.WorkTaskSyncService = WorkTaskSyncService;
