...
class CalendarService extends BaseRepository_1.BaseRepository {
     ...
     * @returns カレンダーイベントID
     */
    async createGoogleCalendarEventForEmployee(employeeId, employeeEmail, eventData) {
        try {
     ...
     * @returns カレンダーイベントID
     */
    async createGoogleCalendarEvent(employeeEmail, eventData) {
        try {
     ...
     * @param updates 更新データ
     */
    async updateGoogleCalendarEvent(employeeEmail, eventId, updates) {
        try {
            // 会社アカウントの認証済みクライアントを取得
            const auth = await this.googleAuthService.getAuthenticatedClient();
            const calendar = googleapis_1.google.calendar({ version: 'v3', auth });
     ...
     * @param eventId カレンダーイベントID
     */
    async deleteGoogleCalendarEvent(employeeEmail, eventId) {
        try {
            // 会社アカウントの認証済みクライアントを取得
            const auth = await this.googleAuthService.getAuthenticatedClient();
            const calendar = googleapis_1.google.calendar({ version: 'v3', auth });
     ...
     * @returns イベント説明文
     */
    formatEventDescription(sellerName, sellerPhone, propertyAddress, notes, sellerNumber) {
        let description = '';
        // 売主番号を一番上に表示（存在する場合）
        if (sellerNumber) {
     ...
     * @param propertyAddress 物件住所
     */
    async createAppointment(request, assignedEmployeeId, sellerName, sellerPhone, propertyAddress, sellerNumber) {
            ...
            try {
                ...
                if (isConnected) {
                    ...
                    const eventData = {
                        ...
                        description: this.formatEventDescription(sellerName, sellerPhone, propertyAddress, request.notes, sellerNumber),
                        startTime: request.startTime,
                        endTime: request.endTime,
                    };
                    console.log('[CalendarService] Creating calendar event in calendar:', employee.email);
                    calendarEventId = await this.createGoogleCalendarEvent(employee.email, eventData);
            ...
            if (appointment.calendar_event_id && appointment.assigned_employee_id) {
                try {
                    ...
                    if (employee && employee.email) {
                        ...
                        await this.deleteGoogleCalendarEvent(employee.email, appointment.calendar_event_id);
                    }
                    else {
                    ...
                    if (employee && employee.email) {
                        ...
                        if (updates.notes !== undefined) {
                            ...
                            const { data: property } = await this.table('properties')
                                ...
                                .single();
                            if (seller && property) {
                                eventUpdates.description = this.formatEventDescription(seller.name, seller.phone_number, property.address, updates.notes);
                            }
                        }
                        await this.updateGoogleCalendarEvent(employee.email, appointment.calendar_event_id, eventUpdates);
     ...
     * 社員の予約一覧を取得
     */
    async getEmployeeAppointments(employeeId, dateFrom, dateTo) {
