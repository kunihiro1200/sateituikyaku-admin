...
exports.getMaskedAWSConfig = getMaskedAWSConfig;
...
function validateAWSConfig(config) {
    ...
    // エラーがある場合は例外をスロー
    if (errors.length > 0) {
        throw new phone_1.PhoneServiceError(`AWS configuration validation failed:\n${errors.join('\n')}`, 'AWS_CONFIG_INVALID', 'validation', false, { errors });
    }
}
/**
 * 電話番号がE.164形式かチェック
 ...
 * AWS設定のマスク版を取得（ログ出力用）
 */
function getMaskedAWSConfig(config) {
    return {
        region: config.region,
