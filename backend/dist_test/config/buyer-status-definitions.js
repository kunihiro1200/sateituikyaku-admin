...
exports.STATUS_DEFINITIONS = [
    ...
    { priority: 5, status: '他社物件問合せ未', description: '他社物件の問い合わせメールへの対応が未完了', color: '#ff6f00' },
...
function getStatusColor(status) {
    const definition = exports.STATUS_DEFINITIONS.find(d => d.status === status);
    if (definition)
        return definition.color;
    // 当日TEL（Y）のような動的ステータスは 当日TEL の色を使用
    if (status.startsWith('当日TEL')) {
        const base = exports.STATUS_DEFINITIONS.find(d => d.status === '当日TEL');
        if (base)
            return base.color;
    }
    return '#cccccc';
