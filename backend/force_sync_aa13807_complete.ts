/**
 * AA13807 の全フィールド（電話番号・メアド含む）を強制同期するスクリプト
 */
...
async function forceSyncAA13807() {
  ...
  console.log('  住所:', rowData['依頼者住所(物件所在と異なる場合）']);
  ...
  if (address && address.trim() !== '') {
    ...
    console.log('✅ 住所を暗号化して更新します:', address.trim());
  } else {
    console.log('⚠️  住所が空欄です');
  }
  ...
  const { data, error: fetchError } = await supabase
    .from('sellers')
    .select('seller_number, name, address, phone_number, email, property_address, current_status, status, next_call_date, valuation_method, valuation_amount_1, valuation_amount_2, valuation_amount_3, phone_contact_person, preferred_contact_time, contact_method')
    .eq('seller_number', TARGET)
    .single();
  ...
  console.log('  address (暗号化済):', data?.address ? '設定済み' : 'null');
...
forceSyncAA13807().catch(console.error);
