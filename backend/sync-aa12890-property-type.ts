...
async function syncAA12890PropertyType() {
  console.log('=== AA12890の物件種別を同期 ===\n');
  ...
  try {
    ...
    const targetRow = rows.find((row: any) => row['売主番号'] === 'AA12890');

    if (!targetRow) {
      console.error('❌ スプレッドシートにAA12890が見つかりません');
      return;
    }
    ...
    if (propertyData.property_type) {
      ...
      console.log(`  更新前: ${property.property_type || '(空)'}`);
    ...
    console.log(`  物件種別: ${updatedProperty?.property_type || '(空)'}`);
