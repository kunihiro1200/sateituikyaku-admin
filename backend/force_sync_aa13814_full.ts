/**
 * AA13814 の氏名・査定額・査定方法を強制同期するスクリプト
 * スプレッドシートから直接データを取得してDBに反映する
 */
...
async function forceSyncAA13814Full() {
  ...
  const { data, error: fetchError } = await supabase
    .from('sellers')
    .select('seller_number, name, valuation_method, valuation_amount_1, valuation_amount_2, valuation_amount_3')
    .eq('seller_number', 'AA13814')
    .single();
...
forceSyncAA13814Full().catch(console.error);
