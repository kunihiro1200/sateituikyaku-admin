...
async function main() {
  console.log('=== マンション（マ）の不一致チェック ===\n');
  console.log('データベースで「マ」になっている物件を全て取得し、');
  ...
  try {
    // Step 1: データベースから property_type = 'マ' の全レコードを取得
    console.log('📊 データベースから property_type = "マ" のレコードを取得中...');
    ...
    if (!properties || properties.length === 0) {
      console.log('✅ データベースに property_type = "マ" のレコードはありません');
      return;
    }

    console.log(`✅ ${properties.length}件の「マ」レコードを取得しました\n`);
    ...
    for (const property of properties) {
      ...
      // データベースは「マ」、スプレッドシートが「マ」以外の場合は不一致
      if (sheetValue !== 'マ') {
        mismatches.push({
          sellerNumber,
    ...
    if (mismatches.length > 0) {
      ...
      for (const [type, items] of byType.entries()) {
        ...
        for (const item of items) {
          console.log(`  ${item.sellerNumber}: DB="マ" → Sheet="${item.spreadsheetValue}"`);
        }
      }
      ...
      console.log('   データベースの「マ」は全てスプレッドシートでも「マ」です。\n');
      process.exit(0);
    }
