/**
 * AA13807 の unreachable_status と comments をスプレッドシートから強制同期するスクリプト
 */
...
async function forceSyncAA13807Unreachable() {
  ...
  if (unreachableStatus !== undefined && unreachableStatus !== null) {
    ...
    console.log('\n✅ 不通ステータスを更新します:', updateData.unreachable_status ?? 'null');
  }
  ...
  if (comments !== undefined && comments !== null && String(comments).trim() !== '') {
    ...
    console.log('✅ コメントを更新します（先頭100文字）:', String(comments).trim().substring(0, 100));
  }
  ...
  if (propertyAddress && String(propertyAddress).trim() !== '') {
    ...
    if (!current?.property_address || current.property_address.trim() === '') {
      updateData.property_address = String(propertyAddress).trim();
      console.log('✅ 物件住所を更新します（DBが空欄のため）:', updateData.property_address);
    } else {
      console.log('ℹ️  物件住所はDBに既に設定済みのためスキップ:', current.property_address);
    }
  }
  ...
  const { data, error: fetchError } = await supabase
    .from('sellers')
    .select('seller_number, unreachable_status, comments, property_address')
    .eq('seller_number', TARGET)
    .single();
  ...
  console.log('  comments (先頭100文字):', data?.comments?.substring(0, 100));
  console.log('  property_address:', data?.property_address);
}

forceSyncAA13807Unreachable().catch(console.error);
