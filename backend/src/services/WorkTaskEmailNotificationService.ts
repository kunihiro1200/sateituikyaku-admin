...
const FLOOR_PLAN_REQUEST_BODY = [
  ...
  '',
  '間取り図作成後こちらのチェックリストにて確認をお願いいたします：',
  'https://docs.google.com/spreadsheets/d/19ihN7602pQVuyXNC-3gvTWalydb0sSvgfRoRlhZKbBo/edit?gid=0#gid=0',
  '',
  '㈱いふう',
  'TEL:097-533-2022',
  ...
  resolveTemplate(template: string, data: Record<string, any>): string {
    ...
    if (emailDist.includes('即') && emailDist.includes('不要')) {
      ...
      emailDistComment =
        ...
        'https://docs.google.com/document/d/145LKr_Q7ftxnRVvNalaKPO1NH_FqncOlOY5bqP5P48c/edit?usp=sharing<br>';
    } else if (emailDist.includes('不要') && emailDist.includes('期日通り')) {
      // 「不要」かつ「期日通り」→ 配信不要だがサイト公開は期日通り
      emailDistComment = '公開前配信・不要、サイト公開日は期日通りでおねがいします<br>';
    } else if (emailDist.includes('要') && !emailDist.includes('不要') && emailDist.includes('期日通り')) {
