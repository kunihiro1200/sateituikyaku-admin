...
/**
 * Unit Test: BuyerColumnMapper の property_number マッピング検証
 *
 * **目的**:
 * - `databaseToSpreadsheet` に `property_number: "物件番号"` が存在することを確認
 * - `mapDatabaseToSpreadsheet({ property_number: 'AA1234' })` が `{ '物件番号': 'AA1234' }` を返すことを検証
 *
 * **Validates: Requirements 2.1, 2.2**
 */
describe('Unit Test: BuyerColumnMapper の property_number マッピング', () => {
  let buyerColumnMapper: BuyerColumnMapper;
  ...
  it('mapDatabaseToSpreadsheet({ property_number: "AA1234" }) が { "物件番号": "AA1234" } を返す', () => {
    const result = buyerColumnMapper.mapDatabaseToSpreadsheet({
      property_number: 'AA1234',
    } as any);

    expect(result).toHaveProperty('物件番号', 'AA1234');
  });

  it('databaseToSpreadsheet マッピングに property_number -> 物件番号 が存在する', () => {
    const spreadsheetColumnName = buyerColumnMapper.getSpreadsheetColumnName('property_number');
    expect(spreadsheetColumnName).toBe('物件番号');
  });
});
