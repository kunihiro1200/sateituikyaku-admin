...
export class SellerSidebarCountsUpdateService {
  ...
  async updateSellerSidebarCounts(): Promise<void> {
    ...
    try {
      ...
      // visit_date はTIMESTAMP型のため、今日の23:59:59までを含めるよう todayJST + 1日 で lt を使う
  ...
  /**
   * 「売却サポートページ：対応が必要」カテゴリーだけを再計算してupsertする（軽量版）。
   * seller_portal_* テーブル（sellersとは別テーブル）の変更時に呼ぶ。
   * FIELD_TO_CATEGORIES（sellersのカラム変更用）とは対象が異なるため、専用メソッドとして分離する。
   */
  ...
  async updateAffectedCategories(updatedFields: string[]): Promise<void> {
    ...
    if (needsTodayCallBase) {
      queries.todayCallBase1 = this.supabase
        .from('sellers')
        .select('id, seller_number, visit_assignee, phone_contact_person, preferred_contact_time, contact_method, unreachable_status, inquiry_date, status, next_call_date')
        .is('deleted_at', null)
        .ilike('status', '%追客中%')
        .not('next_call_date', 'is', null)
        .lte('next_call_date', todayJST);
    ...
    if (needsTodayCallBase) {
      ...
      // FI（福岡）売主と通常売主を分離
      const isFi = (s: any) => ((s.seller_number || '') as string).startsWith('FI');
    ...
    if (needsUnvaluated) {
      ...
      const isFiSeller = (s: any) => ((s.seller_number || '') as string).startsWith('FI');
      ...
      const count = sellers.filter((s: any) => !isFiSeller(s) && unvaluatedFilterFn(s)).length;
      const fi_count = sellers.filter((s: any) => isFiSeller(s) && unvaluatedFilterFn(s)).length;
