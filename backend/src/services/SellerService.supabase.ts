...
export class SellerService extends BaseRepository {
  ...
  async updateSeller(sellerId: string, data: UpdateSellerRequest): Promise<Seller> {
    ...
    // 郵送済みにした日時フィールド
    if ((data as any).mailingDoneAt !== undefined) {
      updates.mailing_done_at = (data as any).mailingDoneAt;
    }
    // 郵送先住所フィールド
    ...
    if (this.shouldUpdateSellerSidebarCounts(updates)) {
      ...
      // 常に非同期・ノンブロッキングで実行（awaitしない）
      // サイドバーカウント更新は重い処理（DBクエリ10本以上）のため、
      // レスポンスをブロックするとステータス更新が1分以上かかる原因になる
      ...
      switch (statusCategory) {
        ...
        default: {
          ...
          if (dynamicCategory.startsWith('fi:')) {
            ...
            // 🚨 専任他決打合せが「完了」でも一覧からは消さない（フロントでグレー表示するため除外条件なし）
            ...
            if (visitedIds.length > 0) {
              const { data: actData } = await this.supabase
                ...
                .or('content.ilike.%訪問査定後御礼%,content.ilike.%訪問後御礼%,content.ilike.%初回訪問査定後のお礼%,content.ilike.%訪問査定後のお礼%');
    ...
    if (typeof statusCategory === 'string' && statusCategory.startsWith('fi:')) {
      ...
      if (fiSubCat === 'sellerPortalScheduleAttention' || 
        ...
        if (fiSubCat === 'todayCallNotStarted') {
          return sharedIsTodayCallNotStarted(normalized, todayJSTStr);
        } else if (fiSubCat === 'todayCall') {
          return sharedIsTodayCallNoInfo(normalized, todayJSTStr);
        } else if (fiSubCat === 'todayCallWithInfo' || fiSubCat.startsWith('todayCallWithInfo:')) {
          if (!sharedIsTodayCallWithInfo(normalized, todayJSTStr)) return false;
          if (fiSubCat.startsWith('todayCallWithInfo:')) {
            const targetLabel = fiSubCat.replace('todayCallWithInfo:', '');
            return sharedGetTodayCallWithInfoLabel(normalized) === targetLabel;
          }
          return true;
  ...
  private async decryptSeller(seller: any): Promise<Seller> {
    ...
    try {
      ...
      const decrypted = {
        ...
        mailingDoneAt: seller.mailing_done_at ? new Date(seller.mailing_done_at) : undefined,
  ...
  private async getSidebarCountsFallback(): Promise<{
    ...
    const [
      ...
      this.table('sellers')
        .select('id, seller_number, visit_assignee, phone_contact_person, preferred_contact_time, contact_method, unreachable_status, inquiry_date, pinrich_status, confidence_level, exclusion_date, status, next_call_date')
        .is('deleted_at', null)
        .ilike('status', '%追客中%')
        .not('next_call_date', 'is', null)
        .lte('next_call_date', todayJST),
    ...
    const filteredTodayCallSellers = todayCallBaseSellers.filter(s => {
      return !sharedHasValidVisitAssignee(s.visit_assignee);
    });
