...
router.post('/extract-and-register-property', async (req: Request, res: Response) => {
  try {
    ...
    // 6. 画像をStorageに保存（会社情報差し替えはフロントエンド印刷時にHTMLオーバーレイで対応）
    if (mediaType !== 'application/pdf') {
      try {
        const imgBuffer = Buffer.from(imageBase64, 'base64');
        await supabase.storage
          .from('tasha-property-images')
          .upload(`${propertyNumber}/original.jpg`, imgBuffer, { contentType: 'image/jpeg', upsert: true });
        console.log(`[AI Register] 画像保存成功: ${propertyNumber}/original.jpg`);
      } catch (imgErr) {
      ...
      try {
        ...
        await supabase.storage
          .from('tasha-property-images')
          .upload(`${propertyNumber}/original.pdf`, pdfBuffer, { contentType: 'application/pdf', upsert: true });
        console.log(`[AI Register] PDF保存成功: ${propertyNumber}/original.pdf`);
      } catch (imgErr) {
        console.warn('[AI Register] Storage保存失敗（登録自体は成功）:', imgErr);
      }
    }
