...
## デザイン要件
- 全体的に清潔感のある白背景
- セクションタイトルは濃い青（#1a237e）、左ボーダー付き
- 比較表はヘッダーを薄い青（#e3f2fd）、${detailArea || reportAreaName}列を薄い黄色（#fff9c4）で強調
- まとめセクションは薄い黄色背景（#fffde7）、オレンジボーダー
- フォントサイズは本文12px、見出し15px
- ページ上部に「エリア情勢レポート」タイトル・対象エリア・作成日を表示
- 末尾に「※本レポートの数値は市場動向に基づく概算値です」と注記`;
