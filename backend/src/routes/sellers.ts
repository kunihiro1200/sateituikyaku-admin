...
router.get('/unvisited-other-decision-monthly-summary', async (req: Request, res: Response) => {
  try {
    ...
    const { data, error } = await supabase
      .from('sellers')
      .select('id, seller_number, name, property_address, address, status, contract_year_month, comments, competitor_name, competitor_name_and_reason, next_call_date, other_decision_countermeasure, unvisited_other_decision_ai_analysis, inquiry_date')
 ...
 * ⚠️ /:id より前に定義必須
 */
router.get('/sumai-step-monthly-summary', async (req: Request, res: Response) => {
  try {
    const supabase = createClient(
      process.env.SUPABASE_URL!,
      process.env.SUPABASE_SERVICE_KEY!
    ...
    // すまいステップ（inquiry_site = 'す'）で2026年5月以降の反響日付を持つ売主を取得
    const { data, error } = await supabase
      .from('sellers')
    ...
    console.error('[sumai-step-monthly-summary] Error:', error);
    return res.status(500).json({ error: 'Failed to get sumai step monthly summary' });
  }
});
...
【重要な前提】
- 「1番電話」「初動の速さ」「電話対応」「追客電話」は事務スタッフの仕事であり、営業担当の強みではない。これらには一切触れないこと。
- 分析は「訪問査定時の対応」「提案内容」「売主との信頼関係構築」「競合との差別化」など、営業担当が訪問時に実際に行った行動に絞ること。
- Markdownの見出し（##）は使わず、番号付きの段落形式で書くこと。`;
