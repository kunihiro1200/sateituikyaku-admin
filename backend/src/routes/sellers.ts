    ...
    const response = await axiosLib.post(
      'https://api.openai.com/v1/chat/completions',
      {
        ...
        messages: [
          {
            role: 'system',
            content: '日本の住所から市区町村より後の部分（区・町名・丁目など）のひらがな読みだけを返すアシスタントです。都道府県・市は除外して、それ以降の部分のみをひらがなで返してください。番地（数字・ハイフン）は除外してください。余分な説明は不要です。',
          },
          {
            role: 'user',
            content: `次の住所の、市より後の部分（区・町名・丁目など）のひらがな読みを返してください。都道府県・市は不要です。番地（数字・ハイフン）も不要です。\n${address}`,
          },
        ],
        temperature: 0,
        max_tokens: 200,
