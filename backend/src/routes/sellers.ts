...
【左列ラベル】${cityLabel}全体（このラベルを一字一句変えずに使うこと）
【右列ラベル】${detailArea}エリア（このラベルを一字一句変えずに使うこと）
【物件種別】${propertyType || '不動産'}
【作成日】${today}（この日付をレポートのヘッダーに必ず使用すること。他の日付は絶対に使わない）
【現在年】2026年（データは2025年まで表示すること。2026年以降のデータは不要）

⚠️ 重要：表のヘッダーは必ず上記の【左列ラベル】【右列ラベル】をそのまま使うこと。住所から独自に解釈したラベルは絶対に使わないこと。
...
- 左列のヘッダーは必ず「${cityLabel}全体」のみ、右列のヘッダーは必ず「${detailArea}エリア」のみとすること
- 数値は概算で構わない
...
- **矢印の表示（必須・厳守）**: 2行目以降の全セルで、前の行より増加なら数値の直後に spanタグ(style="color:#c62828;font-weight:bold;") で「 ↑」、減少なら spanタグ(style="color:#e53935;") で「 ▼」を必ず付けること。矢印なしのセルは禁止。
- 矢印はUnicode文字をそのまま使う（↑ ▼）。HTMLエンティティ（&uarr;等）は禁止。
...
- 「※本レポートの数値は市場動向に基づく概算値です。実際の取引価格は個別物件の状況により異なります。」`;
    ...
    // 例: 「別府市石垣全体」→「別府市全体」
    const wrongCityLabel = new RegExp(cityLabel + '[^全体<]{1,10}全体', 'g');
    htmlContent = htmlContent.replace(wrongCityLabel, cityLabel + '全体');
