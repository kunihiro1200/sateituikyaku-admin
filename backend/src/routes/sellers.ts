...
【対象物件の住所】${address}
【左列ラベル（市全体）】${cityLabel}全体
【右列ラベル（町エリア）】${detailArea}エリア
【物件種別】${propertyType || '不動産'}
【作成日】${today}（この日付をレポートのヘッダーに必ず使用すること。他の日付は絶対に使わない）
【現在年】2026年（データは2025年まで表示すること。2026年以降のデータは不要）

以下の5つのセクションを含む、A4印刷用のHTMLレポートを作成してください。
レポートは日本語で、売主様が「今が売却のチャンス」と感じられるよう、説得力のある内容にしてください。

## 重要な方針
- 各セクションで「${cityLabel}全体」と「${detailArea}エリア」の**2列比較**を必ず行う
- 左列のヘッダーは必ず「${cityLabel}全体」、右列のヘッダーは必ず「${detailArea}エリア」とすること
- 数値は概算で構わない
...
### 1. 人口の推移
- 左列「${cityLabel}全体」vs 右列「${detailArea}エリア」の人口推移を比較表で表示
- 2015・2018・2021・2024・2025年の5行（2025年まで必ず含める）
- 傾向の分析コメント

### 2. 世帯種類の推移
- 左列「${cityLabel}全体」vs 右列「${detailArea}エリア」の世帯構成比較
- 単身・夫婦のみ・核家族・三世代の割合（2025年時点）
- エリアの特徴を強調

### 3. 物件種別の取引件数推移
- 左列「${cityLabel}全体」vs 右列「${detailArea}エリア」の取引件数比較
- 2020・2021・2022・2023・2024・2025年の6行（2025年まで必ず含める）
- エリアの取引活発度を強調

### 4. 不動産価格の推移
- 左列「${cityLabel}全体」vs 右列「${detailArea}エリア」の価格比較（坪単価・万円）
- 2020・2021・2022・2023・2024・2025年の6行（2025年まで必ず含める）
- ${detailArea}エリアの価格優位性・上昇率を強調

### 5. まとめ（今が売却のチャンス）
- 上記4つのデータを総合した分析
- 「${detailArea}エリアで今売るべき理由」を4〜5点で箇条書き
- 市全体と比較した優位性を必ず含める
- 強調スタイルで表示
...
- ${detailArea}エリアの列ヘッダー: background:#fff9c4
- ${detailArea}エリアのデータセル: background:#fffde7
- 列幅: 年列20%、市全体列40%、エリア列40%（3列の場合）
- 前回比・変化率の列: 上昇は color:#c62828 で「↑」、下降は color:#e53935 で「▼」を使う
- 最新年（2025年）の行は font-weight:bold
