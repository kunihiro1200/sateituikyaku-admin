 ...
 */
function generateSampleAreaReport(cityName: string, detailArea: string, propertyType: string): string {
  const today = new Date().toLocaleDateString('ja-JP', { year: 'numeric', month: 'long', day: 'numeric' });
  const area = detailArea || cityName;
  return `
...
router.post('/:id/area-report', async (req: Request, res: Response) => {
  try {
    ...
    if (!process.env.OPENAI_API_KEY) {
      // 住所からエリア名を抽出（デモ用）
      const prefMatchDemo = address.match(/^(.{2,5}[都道府県])/);
      const cityMatchDemo = address.match(/[都道府県](.{2,10}[市区町村])/);
      const townMatchDemo = address.match(/[市区町村](.{2,15}?)(?:\d|[一二三四五六七八九十]|$)/);
      const cityDemo = cityMatchDemo ? cityMatchDemo[1] : '';
      const townDemo = townMatchDemo ? townMatchDemo[1].replace(/[\s　]+/g, '') : '';
      const areaNameDemo = cityDemo || (prefMatchDemo ? prefMatchDemo[1] : address.substring(0, 10));
      const detailAreaDemo = townDemo ? `${cityDemo}${townDemo}` : cityDemo;
      const sampleHtml = generateSampleAreaReport(areaNameDemo, detailAreaDemo || areaNameDemo, propertyType);
      return res.json({ html: sampleHtml, areaName: detailAreaDemo || areaNameDemo, generatedAt: new Date().toISOString(), isDemo: true });
    }
    ...
    // 住所から市区町村・町丁目を抽出
    const prefMatch = address.match(/^(.{2,5}[都道府県])/);
    const cityMatch = address.match(/[都道府県](.{2,10}[市区町村])/);
    const townMatch = address.match(/[市区町村](.{2,15}?)(?:\d|[一二三四五六七八九十]|$)/);

    const prefecture = prefMatch ? prefMatch[1] : '';
    const city = cityMatch ? cityMatch[1] : '';
    const town = townMatch ? townMatch[1].replace(/[\s　]+/g, '') : '';
    const reportAreaName = city || prefecture || address.substring(0, 10);
    const detailArea = town ? `${city}${town}` : city;

    const prompt = `あなたは不動産売買仲介会社の営業担当者です。
以下のエリアについて、売主様に訪問査定時にお見せする「エリア情勢レポート」を作成してください。

【対象物件の住所】${address}
【市区町村】${city || reportAreaName}
【町丁目エリア】${detailArea || reportAreaName}
【物件種別】${propertyType || '不動産'}

以下の5つのセクションを含む、A4印刷用のHTMLレポートを作成してください。
レポートは日本語で、売主様が「今が売却のチャンス」と感じられるよう、説得力のある内容にしてください。

## 重要な方針
- 各セクションで「${city || reportAreaName}全体」と「${detailArea || reportAreaName}エリア」の**2列比較**を必ず行う
- ${detailArea || reportAreaName}エリアの特徴・優位性を具体的に説明する
- 数値は概算・推計で構わない（「※概算値」と注記する）
- 売主様が「このエリアで今売るべき」と納得できる内容にする

## 出力形式
HTMLのbodyタグ内のコンテンツのみを出力してください（htmlタグ、headタグ、bodyタグ自体は不要）。
インラインスタイルのみ使用し、印刷時に見やすいデザインにしてください。

## 必須セクション

### 1. 人口の推移
- ${city || reportAreaName}全体 vs ${detailArea || reportAreaName}エリアの人口推移を比較表で表示
- 過去10年の推移（2015・2018・2021・2024年）
- 傾向の分析コメント（エリアの特徴を強調）

### 2. 世帯種類の推移
- ${city || reportAreaName}全体 vs ${detailArea || reportAreaName}エリアの世帯構成比較
- 単身・夫婦のみ・核家族・三世代の割合
- エリアの特徴（例：ファミリー層が多い、利便性が高く単身需要も旺盛など）

### 3. 物件種別の取引件数推移
- ${city || reportAreaName}全体 vs ${detailArea || reportAreaName}エリアの取引件数比較
- 戸建て・マンション・土地の件数（2020〜2024年）
- エリアの取引活発度を強調

### 4. 不動産価格の推移
- ${city || reportAreaName}全体 vs ${detailArea || reportAreaName}エリアの価格比較（坪単価・㎡単価）
- 2020〜2024年の推移
- ${detailArea || reportAreaName}エリアの価格優位性・上昇率を強調

### 5. まとめ（今が売却のチャンス）
- 上記4つのデータを総合した分析
- 「${detailArea || reportAreaName}エリアで今売るべき理由」を4〜5点で箇条書き
- 市全体と比較した優位性を必ず含める
- 強調スタイルで表示
...
- font-size: 12px、max-width: 800px、padding: 20px
- 白背景

### ヘッダー
- 中央揃え、下線は border-bottom: 3px solid #1a237e
- タイトル「エリア情勢レポート」はfont-size:20px、color:#1a237e
- 対象エリア名・作成日・物件種別を表示

### セクションタイトル
- 必ず「① ② ③ ④ ⑤」の丸数字を使う（1. 2. は禁止）
- color:#1a237e、border-left:4px solid #1a237e、padding-left:8px

### 比較表
- width:100%、border-collapse:collapse
- ヘッダー行: background:#e3f2fd、border:1px solid #90caf9
- ${detailArea || reportAreaName}エリアの列ヘッダー: background:#fff9c4
- ${detailArea || reportAreaName}エリアのデータセル: background:#fffde7
- 列幅: 年列20%、市全体列40%、エリア列40%（3列の場合）
- 前回比・変化率の列: 上昇は color:#c62828 で「↑」、下降は color:#e53935 で「▼」を使う
- 最新年（2024年）の行は font-weight:bold

### 分析コメント
- 各表の下に必ず分析コメントを入れる
- スタイル: font-size:11px、background:#f5f5f5、padding:6px 10px、border-radius:4px
- 先頭に「📊 **分析：**」を付ける
- エリアの優位性を強調する内容にする

### まとめセクション
- background:#fffde7、border:2px solid #f9a825、border-radius:8px、padding:16px
- タイトルは color:#e65100、border-left:4px solid #e65100
- 箇条書きの各項目: <strong style="color:#c62828">見出し</strong>：説明文 の形式
- 末尾に background:#e65100、color:white のまとめバナーを入れる

### フッター
- border-top:1px solid #ccc、font-size:10px、color:#999、text-align:center
- 「※本レポートの数値は市場動向に基づく概算値です。実際の取引価格は個別物件の状況により異なります。」`;
    ...
    const completion = await axios.post(
      'https://api.openai.com/v1/chat/completions',
      {
        model: 'gpt-4o-mini',
        messages: [
          { role: 'system', content: 'あなたは不動産売買仲介会社の優秀な営業担当者です。売主様向けの説得力あるエリア情勢レポートをHTML形式で作成します。' },
