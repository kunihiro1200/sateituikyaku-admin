...
【最重要ルール：質問の作り方】
- 気づきやヒアリングに**既に答えが書かれていること**をわざわざ質問しない。書いてあることを再度書かせるのは無意味。
- 質問すべきは「気づきに書かれているが、まだ深掘りされていない重要ポイント」。
  例：FK41の気づきに「ちょっとここはないかな？と言われて切り返した」と書いてある。切り返し方は既に書いてあるので聞かない。
  しかし同じ気づきに「柱のヒビを指摘された」「クロスを張り替えれば気にならなくなる」とも書かれている。
  → 質問すべきは「柱のヒビを指摘された時、クロス張り替えの話をどのタイミングで切り出しましたか？お客様の反応は？」のように、**書かれてはいるが詳細が不明な対応**を聞く。
- つまり：気づきの中で「お客様の懸念→担当者の対応」のセットがある場合、対応の**具体的な言い回し・タイミング・お客様の反応の変化**を聞く。
- 気づきに結果まで全部書かれている場合（完結している話）は質問にしない。まだ深掘りできる余地があるポイントを選ぶ。
- **絶対禁止：気づきデータに書かれていない言葉・話題を質問に含めないこと。原文に出てくる言葉だけを使う。**

【質問の作り方】
- 質問文は必ず「買主○○○○（物件住所）の内覧で、」から始めること。これは絶対ルール。
  フォーマット：「買主{番号}（{物件住所}）の内覧で、〜」
  例：「買主7691（大分市岩田町2-9-17）の内覧で、〜」
  例：「買主8197（大分市尼ヶ瀬）の内覧で、〜」
  このフォーマットで始まっていない質問は無効。
- 質問文の中に「何についての質問か」を必ず具体的に書くこと。「どう伝えましたか？」だけでは何を伝えたか不明。
  NG例：「買主8185（大分市王子山の手町）の内覧で、具体的にどんな言葉で伝えましたか？」←何を伝えたのか不明
  OK例：「買主8185（大分市王子山の手町）の内覧で、お客様が『庭がない』と懸念されていたとのことですが、庭がない代わりの利点（高台でハザードクリアなど）をどう伝えて購入意欲を引き出しましたか？」←具体的に何の話かわかる
- 失敗・反省系（「〜できなかった」「〜わからなかった」）→ 「今後どう準備・対応しますか？」
  例：「買主8197（大分市尼ヶ瀬）の内覧で、お客様に近くのバス停を聞かれて答えられなかったとのことですが、今後内覧前に周辺交通情報をどう準備しますか？」
- 成功だが詳細が不明な対応 → そのポイントの具体的なタイミング・言い回し・お客様の反応を聞く
  例：「買主FK41の内覧で、柱のヒビを指摘された際に『クロスを張り替えれば気にならない』と伝えたとのことですが、このタイミングはいつ（ヒビを見た直後？帰り際？）で、お客様の表情や反応はどう変わりましたか？」
...
router.post('/:buyer_number/send-offer-chat', async (req: Request, res: Response) => {
  try {
    ...
    const { propertyNumber, offerComment, campaignHandedOver, pdfUrl } = req.body;
