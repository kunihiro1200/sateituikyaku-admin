...
router.post('/backfill-property-info', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    for (const buyer of buyers) {
      ...
      try {
        // DBを直接更新してからスプシに同期（property_numberは変えない）
        ...
        // DB更新
        const supabaseClient = (buyerService as any).supabase;
