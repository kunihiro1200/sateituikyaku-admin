 ...
 * 「外す」は担当なし扱い
 */
export const hasValidVisitAssignee = (visitAssignee: string | null | undefined): boolean => {
  if (!visitAssignee) return false;
 ...
 * "null" 文字列も空扱い
 */
export const hasContactInfo = (s: SellerRow): boolean => {
  const isValid = (v: string | null | undefined): boolean =>
 ...
 */
export const isTodayCallNoInfo = (s: SellerRow, todayJST: string): boolean => {
  if (!isTodayCallBase(s, todayJST)) return false;
  ...
  if (isTodayCallNotStarted(s, todayJST)) return false;
  return true;
};
 ...
 */
export const isTodayCallWithInfo = (s: SellerRow, todayJST: string): boolean => {
  if (!isTodayCallBase(s, todayJST)) return false;
  if (hasValidVisitAssignee(s.visit_assignee)) return false;
 ...
 * 当日TEL（内容）のラベルを生成
 */
export const getTodayCallWithInfoLabel = (s: SellerRow): string => {
  const isValid = (v: string | null | undefined): boolean =>
    !!(v && v.trim() !== '' && v.trim().toLowerCase() !== 'null');
  const parts: string[] = [];
  if (isValid(s.phone_contact_person)) parts.push(s.phone_contact_person!.trim());
  if (isValid(s.preferred_contact_time)) parts.push(s.preferred_contact_time!.trim());
  if (isValid(s.contact_method)) parts.push(s.contact_method!.trim());
 ...
 */
export const isTodayCallNotStarted = (s: SellerRow, todayJST: string): boolean => {
 ...
 */
export const isUnvaluated = (s: SellerRow, todayJST: string): boolean => {
  const status = s.status || '';
  ...
  if ((s.inquiry_date || '') < '2025-12-08') return false;
