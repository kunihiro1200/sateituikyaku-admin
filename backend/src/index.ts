...
import aiImagePropertyRegisterRoutes from './routes/aiImagePropertyRegister';
...
app.get('/api/cron/work-task-upcoming-deadline-notification', async (req, res) => {
  try {
    ...
    const { WorkTaskUpcomingDeadlineNotificationService } = await import('./services/WorkTaskUpcomingDeadlineNotificationService');
...
app.get('/api/cron/sales-meeting-notification', async (req, res) => {
  try {
    console.log('[Cron SalesMeetingNotification] 営業会議事前通知ジョブ開始');

    const authHeader = req.headers.authorization;
    if (authHeader !== `Bearer ${process.env.CRON_SECRET}`) {
      console.error('[Cron SalesMeetingNotification] 認証失敗: 不正なアクセス');
      return res.status(401).json({ error: 'Unauthorized' });
    }
    ...
    const result = await service.sendIfScheduledDay();
    console.log('[Cron SalesMeetingNotification] 完了:', result);

    return res.status(200).json({ success: true, ...result });
  } catch (error: any) {
    console.error('[Cron SalesMeetingNotification] 予期しないエラーが発生:', error.message);
    return res.status(500).json({ success: false, error: error.message });
  }
});
...
app.use('/api/scheduled-notifications', scheduledNotificationRoutes); // 定期メール通知（CRON_SECRET認証・認証不要/list）
...
app.use('/api/sellers', sumaiTransferRoutes);
...
app.post('/api/cron/check-property-prices', async (req, res) => {
  try {
    ...
    const { PropertyPriceMonitorService } = await import('./services/PropertyPriceMonitorService');
