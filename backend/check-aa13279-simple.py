...
print(f'営担の列番号: {visit_assignee_index}')
...
if visit_assignee_index != -1 and len(row_data) > visit_assignee_index:
    visit_assignee = row_data[visit_assignee_index] if row_data[visit_assignee_index] else "（空）"
    print(f'営担（列{visit_assignee_index}）: {visit_assignee}')
else:
    print('営担: （列が見つからない）')

# 訪問日
visit_date_index = visit_date_candidates[0] if visit_date_candidates else -1
if visit_date_index != -1 and len(row_data) > visit_date_index:
    visit_date_str = row_data[visit_date_index] if row_data[visit_date_index] else "（空）"
    print(f'訪問日（列{visit_date_index}）: {visit_date_str}')
    
    # 今日の日付
    today = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0)
    print(f'\n今日: {today.strftime("%Y-%m-%d")} ({["月","火","水","木","金","土","日"][today.weekday()]}曜)')
    
    if visit_date_str and visit_date_str != "（空）":
        # 日付をパース
        try:
