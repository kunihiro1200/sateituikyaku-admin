 ...
 * 
 * メール転記GASが失敗したため、手動で売主リストに追加する
 */
import * as dotenv from 'dotenv';
import * as path from 'path';

// .env.vercel.check から環境変数を読み込む
...
async function main() {
  ...
  for (let i = 0; i < headers.length; i++) {
    ...
    for (const target of targetColumns) {
      if (header === target || header.includes(target.replace('\n', ''))) {
        columnIndexes[target] = i;
      }
    }
    // 特殊なマッチング
    if (header.includes('売主番号')) columnIndexes['売主番号'] = i;
    if (header.includes('名前') && header.includes('漢字')) columnIndexes['名前(漢字のみ）'] = i;
    if (header.includes('電話番号')) columnIndexes['電話番号\nハイフン不要'] = i;
  ...
  for (const [key, idx] of Object.entries(columnIndexes)) {
    console.log(`    ${key}: 列${idx} (${headers[idx]})`);
  }
  ...
  if (columnIndexes['名前(漢字のみ）'] !== undefined) newRow[columnIndexes['名前(漢字のみ）']] = '恩田 透';
  if (columnIndexes['電話番号\nハイフン不要'] !== undefined) newRow[columnIndexes['電話番号\nハイフン不要']] = '08072072768';
  if (columnIndexes['物件所在地'] !== undefined) newRow[columnIndexes['物件所在地']] = '大阪府堺市東区日置荘西町14-11-19';
  ...
  console.log(`  物件所在地: 大阪府堺市東区日置荘西町14-11-19`);
  ...
  try {
    ...
    if (result.success) {
      ...
      console.log('  10分以内に売主リストページに表示されます。');
    } else {
      console.log(`\n⚠️ DB同期でエラーが発生しました。10分後の自動同期で反映される可能性があります。`);
    }
  } catch (error: any) {
    console.log(`  ⚠️ DB同期呼び出しエラー: ${error.message}`);
