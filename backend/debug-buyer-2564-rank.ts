...
function shouldExclude(buyer: any): { excluded: boolean; reason: string } {
  ...
  if (inquirySource.includes('業者') || distributionType.includes('業者') ||
      (brokerInquiry && brokerInquiry !== '0' && brokerInquiry.toLowerCase() !== 'false')) {
    return { excluded: true, reason: '業者問合せ' };
  }
...
async function debug() {
  ...
  console.log('除外理由の内訳:');
  for (const [reason, count] of Object.entries(failedReasons)) {
  ...
  if (rank === -1) {
    ...
    if (buyer2564) {
      ...
      console.log(`  除外チェック: ${excluded ? `除外(${reason})` : '通過'}`);
      console.log(`  エリアマッチ: ${matchesArea(buyer2564, propertyAreaNumbers)}`);
      console.log(`  種別マッチ: ${matchesType(buyer2564, property.property_type)}`);
