...
async function testBatchRetrieveStorageUrlsAA() {
  console.log('=== 格納先URL一括取得（テスト：AA番号の物件5件） ===\n');
  ...
  try {
    ...
    } else if (notFoundCount === properties.length) {
      console.log('\n⚠️  すべての物件でフォルダが見つかりませんでした。');
      console.log('これらの物件はGoogle Driveにフォルダが存在しない可能性があります。');
    }
...
testBatchRetrieveStorageUrlsAA();
