# -*- coding: utf-8 -*-
"""
スクレイピングサーバーにSUUMO対応を追加するスクリプト
- scrape_athome() は一切変更しない
- scrape_suumo() という新しい関数を追加する
- do_POST() でURLに応じて分岐するだけ（既存ロジックは変更しない）
"""
...
async def scrape_suumo(url: str) -> dict:
    ...
    async with Stealth().use_async(async_playwright()) as p:
        ...
        # --- アピールコメント ---
        appeal_texts = []
        for el in soup.select('[class*="comment"], [class*="appeal"]'):
            t = el.get_text(strip=True)
            if t and len(t) > 20 and t not in appeal_texts:
                appeal_texts.append(t)
        if appeal_texts:
            result['appeal_comment'] = '\\n\\n'.join(appeal_texts[:2])
...
# 既存の「data = asyncio.run(scrape_athome(url))」を分岐に変更
