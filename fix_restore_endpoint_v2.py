...
# -*- coding: utf-8 -*-
"""
restore-to-sheetエンドポイントを買主番号範囲指定方式に修正する。
DBから全件取得せず、指定範囲の買主のみ処理することでタイムアウトを回避。
"""
...
# 古いエンドポイントを新しいものに置き換え
old = '''// DBにあってスプレッドシートにない買主を検出してスプシに書き戻す
...
new = '''// DBにあってスプレッドシートにない買主を検出してスプシに書き戻す
// POST /api/buyers/restore-to-sheet
...
if old in text:
    ...
    print('SUCCESS: エンドポイントを番号範囲指定方式に修正しました')
else:
