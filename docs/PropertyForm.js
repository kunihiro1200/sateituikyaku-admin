...
class PropertyForm {
    ...
    displayContentWithLinks(content) {
        ...
        // Convert URLs to clickable links (stop at <br> tags or whitespace)
        const urlRegex = /(https?:\/\/[^\s<]+)/g;
        const linkedContent = contentWithBreaks.replace(urlRegex, '<a href="$1" target="_blank" rel="noopener noreferrer">$1</a>');
    ...
    handleError(error) {
        ...
        } else if (error.code === 'CONNECTION_ERROR' || error.code === 'NETWORK_ERROR') {
            this.showError('ネットワークエラーが発生しました。インターネット接続を確認してください');
        } else if (error.code === 'AUTH_ERROR') {
            this.showError('認証に失敗しました。管理者にお問い合わせください');
        } else if (error.code === 'TIMEOUT') {
            this.showError('リクエストがタイムアウトしました。もう一度お試しください');
        } else {
