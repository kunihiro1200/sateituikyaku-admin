...
# -*- coding: utf-8 -*-
"""
1. GmailDistributionButton.tsx を修正（重複削除 + props追加 + 新プレースホルダー対応）
2. gmailDistributionTemplates.ts を更新（新着物件テンプレート変更 + 公開前配信テンプレート追加）
"""

# ============================================================
# 1. GmailDistributionButton.tsx を正しい内容で書き直す
# ============================================================
...
export default function GmailDistributionButton({
  ...
  const [selectedBuyerNames, setSelectedBuyerNames] = useState<Record<string, string>>({});
  ...
  // テンプレートのプレースホルダーを置換（買主名なし版 - プレビュー用）
  ...
  const handleFilterSummaryConfirm = (emails: string[], buyerNames?: Record<string, string>) => {
    if (!selectedTemplate || emails.length === 0) {
      return;
    }
    setSelectedEmails(emails);
    setSelectedBuyerNames(buyerNames || {});
...
with open(button_path, 'wb') as f:
    f.write(new_button_content.encode('utf-8'))
print('✅ GmailDistributionButton.tsx を書き直しました')

# ============================================================
# 2. gmailDistributionTemplates.ts を更新
# ============================================================
templates_path = 'frontend/frontend/src/utils/gmailDistributionTemplates.ts'

new_templates_content = """\
...
${SIGNATURE}`,
    placeholders: ['address', 'propertyNumber', 'publicUrl', 'buyerName', 'propertyType', 'price']
  },
  {
    id: 'pre-listing',
    name: '公開前配信',
...
with open(templates_path, 'wb') as f:
    f.write(new_templates_content.encode('utf-8'))
