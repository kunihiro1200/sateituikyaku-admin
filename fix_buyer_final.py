...
# 現在のセクションを確認（Unicodeエスケープが混在している）
si = text.find('{/* \u554f\u5408\u305b\u60c5\u5831 */}')
ei = text.find('{/* \u5e0c\u671b\u6761\u4ef6 */}')
print(f'si={si} ei={ei}')
...
new_section = """                {/* \u554f\u5408\u305b\u60c5\u5831 */}
                ...
                {/* three_calls_confirmed: inquiryEmailPhone\u306b\u5024\u304c\u306a\u3044\u5834\u5408\u306f\u975e\u8868\u793a */}
                {inquiryEmailPhone && (
                  <Grid item xs={12}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                ...
                {/* owned_home_hearing_result: ownedHomeHearingInquiry\u306b\u5024\u304c\u306a\u3044\u5834\u5408\u306f\u975e\u8868\u793a */}
...
if si >= 0 and ei >= 0:
    ...
    print(f'Replacing {ei-si} chars with {len(new_section)} chars')
    print(f'New file: {len(new_text)} chars')
    with open(FILE_PATH, 'wb') as f:
        f.write(new_text.encode('utf-8'))
    print('Done!')
else:
    # Unicodeエスケープのまま書き込まれているので、それを探して置換
    # 現在のファイルの問合せ情報セクション開始を探す
    # Unicodeエスケープ版のマーカーを探す
    esc_start = '{/* \\u554f\\u5408\\u305b\\u60c5\\u5831 */}'
    esc_end = '{/* \\u5e0c\\u671b\\u6761\\u4ef6 */}'
    si2 = text.find(esc_start)
    ei2 = text.find(esc_end)
    print(f'Escaped markers: si2={si2} ei2={ei2}')
    
    # 別のアプローチ: 問合せ情報セクションの前後を探す
    # 「問合せ情報」の前のコンテキスト（基本情報セクション終わり）
    before_marker = '                {/* \\u554f\\u5408\\u305b\\u60c5\\u5831 */}'
    print(f'before_marker found: {before_marker in text}')
    
    # 実際のファイルの27790付近を確認
    print('Around 27790:')
    print(repr(text[27780:27850]))
