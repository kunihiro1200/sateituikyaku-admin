 ...
 * 2. 新しいプロジェクトを作成
 * 3. このコードを貼り付け
 * 4. トリガーを設定: setupTrigger() を1回実行
 *    → 毎日午前9時に checkAndSendNotifications が実行される
 */
...
function checkAndSendNotifications() {
  var today = new Date();
  var notifications = getAllNotifications_();
  
  for (var i = 0; i < notifications.length; i++) {
    var n = notifications[i];
    if (shouldSendToday_(n, today)) {
      sendNotification_(n.subject, n.body);
      Logger.log('送信完了: ' + n.subject);
    }
  }
}
 ...
 * 今日送信すべきかを判定
 */
function shouldSendToday_(notification, today) {
  var type = notification.type;
  var day = today.getDate();
  var month = today.getMonth() + 1; // 1-12
  var year = today.getFullYear();
  var dow = today.getDay(); // 0=日, 1=月, ..., 6=土
  ...
  switch (type) {
    ...
    case 'monthly_last_sunday':
      ...
      for (var d = lastDayOfMonth; d >= 1; d--) {
    ...
    case 'yearly_dates':
      // 年に複数回（特定の月日）
      for (var i = 0; i < notification.dates.length; i++) {
        if (month === notification.dates[i].month && day === notification.dates[i].day) {
          return true;
        }
      }
      return false;
    ...
    case 'one_time':
      // 1回きり（特定の年月日）
      return year === notification.year && month === notification.month && day === notification.day;
...
function getAllNotifications_() {
  return [
    ...
    // ===== 毎年（複数回） =====
    {
      type: 'yearly_dates', dates: [{month: 5, day: 1}, {month: 10, day: 1}],
    ...
    {
      type: 'yearly_dates', dates: [{month: 1, day: 20}, {month: 5, day: 20}, {month: 9, day: 20}],
      subject: '人事考課提出',
      body: 'お疲れ様です。\n人事考課は今月２５日までに各上長に提出願います。'
    },
    {
      type: 'yearly_dates', dates: [{month: 8, day: 1}, {month: 12, day: 1}],
      subject: '【重要・訪問予約スケジュールの日程確認】',
      body: 'お疲れ様です。\n売主リストの訪問予約カレンダーの休日の調整お願いいたします。\n\n８月　お盆休み\n\n１２月　正月休み\n\nよろしくお願いいたします。'
    },
    {
      type: 'yearly_dates', dates: [{month: 6, day: 1}, {month: 10, day: 1}],
 ...
 * 毎日午前9時に checkAndSendNotifications を実行
 */
function setupTrigger() {
  // 既存のトリガーを削除
  var triggers = ScriptApp.getProjectTriggers();
  for (var i = 0; i < triggers.length; i++) {
    if (triggers[i].getHandlerFunction() === 'checkAndSendNotifications') {
...
/**
 * テスト用: 今日送信されるメールを確認（実際には送信しない）
 */
function testTodayNotifications() {
  var today = new Date();
  var notifications = getAllNotifications_();
  var count = 0;

  Logger.log('=== 本日(' + Utilities.formatDate(today, 'Asia/Tokyo', 'yyyy/MM/dd (E)') + ')の送信予定 ===');

  for (var i = 0; i < notifications.length; i++) {
    if (shouldSendToday_(notifications[i], today)) {
      count++;
      Logger.log(count + '. ' + notifications[i].subject);
    }
  }

  if (count === 0) {
    Logger.log('本日の送信予定はありません。');
  } else {
...
/**
 * テスト用: 特定の日付の送信予定を確認
 * 使い方: testSpecificDate() を実行して日付を変更して確認
 */
function testSpecificDate() {
  var testDate = new Date(2026, 9, 1); // 2026年10月1日（月は0始まり）
  var notifications = getAllNotifications_();
  var count = 0;

  Logger.log('=== ' + Utilities.formatDate(testDate, 'Asia/Tokyo', 'yyyy/MM/dd (E)') + 'の送信予定 ===');

  for (var i = 0; i < notifications.length; i++) {
    if (shouldSendToday_(notifications[i], testDate)) {
      count++;
      Logger.log(count + '. ' + notifications[i].subject);
    }
  }
