/**
 * 物件リスト同期スクリプト（GAS → Supabase直接書き込み）
 *
 * 概要:
 * - スプレッドシートの「物件」シートを読み込み
 * - Supabase の property_listings テーブルに upsert（property_number をキーに）
 * - 10分ごとのトリガーで自動実行
 *
 * セットアップ:
...
// ============================================================
// カラムマッピング（property-listing-column-mapping.json と同期）
// ============================================================
var COLUMN_MAPPING = {
  ...
  '管理の勤務形態': 'management_work_type',
  '管理会社': 'management_company',
  '売出価格': 'listing_price',
  '●駐車場': 'parking',
  '駐車場代': 'parking_fee',
  'バイク': 'bike_parking',
  'バイク置場代': 'bike_parking_fee',
  '駐輪場': 'bicycle_parking',
  '駐輪代': 'bicycle_parking_fee',
  '管理費': 'management_fee',
  '積立金': 'reserve_fund',
  '●特記': 'special_notes',
  'ランニングコスト': 'running_cost',
  'ランニングコスト名目１': 'running_cost_item1',
  'ランニングコスト名目２': 'running_cost_item2',
  'ランニングコスト名目３': 'running_cost_item3',
  'ランニングコスト名目１　価格': 'running_cost_price1',
  'ランニングコスト名目２　価格': 'running_cost_price2',
  'ランニングコスト名目３　価格': 'running_cost_price3',
  '●ペット相談': 'pet_consultation',
  '温泉/引込状況': 'hot_spring_status',
  '温泉利用形態': 'hot_spring_usage_type',
  '温泉費用等': 'hot_spring_cost',
  '間取り': 'floor_plan',
  '専有面積': 'exclusive_area',
  '主要採光面': 'main_lighting',
  '●現況': 'current_status',
  ...
  '固定資産税': 'property_tax',
  '●業者への対応': 'broker_response',
  '売却理由': 'sale_reason',
  '●内覧前伝達事項': 'pre_viewing_notes',
  '内覧の時の伝達事項': 'viewing_notes',
  '価格': 'price',
  '●内覧可能日': 'viewing_available_date',
  '●建物内覧': 'building_viewing',
  '●控除利用': 'deduction_usage',
  '値下げ履歴': 'price_reduction_history',
  '片手理由': 'one_sided_reason',
  '買付日': 'offer_date',
  '買付': 'offer_status',
  '会社名\n（連絡先も）': 'company_name',
  ...
  '値下げ配信メール日１': 'price_reduction_email_date1',
  '■地積測量図': 'cadastral_map',
  '●売主内覧時連絡': 'seller_viewing_contact',
  'ATHOME入力担当': 'athome_input_assignee',
  '看板': 'signboard',
  'GoogleMap': 'google_map_url',
  'Suumo登録': 'suumo_registered',
  'Suumo URL': 'suumo_url',
  '●引渡方法': 'delivery_method',
  '温泉': 'hot_spring',
  '売主メールアドレス': 'seller_email',
  '買付あり': 'offer_exists',
  ...
  '販売計画': 'sales_plan',
  '備忘録': 'memo',
  '売買契約完了': 'sales_contract_completed',
  '値下げ配信メール日３': 'price_reduction_email_date3',
  '買付外れコメント': 'offer_lost_comment',
  '買付外れチャット': 'offer_lost_chat',
  '値下げ配信メール日４': 'price_reduction_email_date4',
  '値下げ配信mail': 'price_reduction_email',
  'データ更新': 'data_updated',
  '看板撤去': 'signboard_removal',
  '確認': 'confirmation',
  '担当をCCにいれる': 'cc_assignee',
  '報告日': 'report_date',
  '報告完了': 'report_completed',
  '報告担当': 'report_assignee',
...
var TYPE_CONVERSIONS = {
  ...
  'property_tax': 'number',
  'price': 'number',
  'post_visit_valuation1': 'number'
};

// ============================================================
// メイン同期関数（トリガーから呼び出される）
// ============================================================
function syncPropertyListings() {
  var startTime = new Date();
  Logger.log('=== 物件リスト同期開始: ' + startTime.toISOString() + ' ===');
  ...
  try {
    ...
    for (var i = 0; i < rawValues.length; i++) {
      ...
      // 注: calculateSidebarStatusGas関数が未定義のため一時的にコメントアウト
...
function convertValue(column, value) {
  ...
  var strCheck = String(value).trim();
...
function upsertToSupabase(records) {
  // on_conflict=property_number を指定してupsert
...
function setupTrigger() {
  ...
  for (var i = 0; i < triggers.length; i++) {
    if (triggers[i].getHandlerFunction() === 'syncPropertyListings') {
  ...
  // 10分ごとのトリガーを作成
  ScriptApp.newTrigger('syncPropertyListings')
    .timeBased()
    .everyMinutes(CONFIG.SYNC_INTERVAL_MINUTES)
    .create();

  Logger.log('トリガーを設定しました: ' + CONFIG.SYNC_INTERVAL_MINUTES + '分ごとに syncPropertyListings() を実行');
}
...
/**
 * 文字列が長すぎるカラムを検出（デバッグ用）
 * 実行するとログに「カラム名: XX文字」が出力される
 */
function findLongValues() {
  var ss = SpreadsheetApp.openById(CONFIG.SPREADSHEET_ID);
  var sheet = ss.getSheetByName(CONFIG.SHEET_NAME);
  if (!sheet) {
  ...
  var allData = sheet.getRange(2, 1, Math.min(lastRow - 1, 50), lastCol).getValues();
  ...
  for (var i = 0; i < allData.length; i++) {
    ...
    for (var col in record) {
      if (record[col] && typeof record[col] === 'string') {
        var len = record[col].length;
        if (!maxLengths[col] || len > maxLengths[col]) {
          maxLengths[col] = len;
        }
      }
    }
  ...
  // 50文字超のカラムを表示
  Logger.log('=== 50文字超のカラム ===');
  for (var key in maxLengths) {
    if (maxLengths[key] > 50) {
      Logger.log(key + ': 最大' + maxLengths[key] + '文字');
    }
  }
  Logger.log('=== 全カラムの最大文字数 ===');
  for (var key2 in maxLengths) {
    Logger.log(key2 + ': ' + maxLengths[key2]);
  }
}
 ...
 * 特定の物件番号だけ同期（デバッグ用）
 */
function syncSingleProperty(propertyNumber) {
  var ss = SpreadsheetApp.openById(CONFIG.SPREADSHEET_ID);
  var sheet = ss.getSheetByName(CONFIG.SHEET_NAME);
  if (!sheet) {
    Logger.log('シートが見つかりません');
