...
# -*- coding: utf-8 -*-
"""
NewBuyerPage.tsx の修正:
1. 「業者向けアンケート」を削除
2. 「持家ヒアリング」関連を削除（owned_home_hearing_inquiry, owned_home_hearing_result, valuation_required）
3. 「担当への確認事項」を削除
4. 「担当への伝言/質問事項」に入力があれば「担当へCHAT送信」ボタンを表示
"""
...
# 1. 「業者向けアンケート」セクションを削除
# vendor_survey: 常に表示、「未」のときはオレンジ から次のセクションまで
old_vendor_survey = '''                {/* vendor_survey: 常に表示、「未」のときはオレンジ */}
                <Grid item xs={12}>
...
new_vendor_survey = '''                {/* inquiry_hearing: multiline */}'''

text = text.replace(old_vendor_survey, new_vendor_survey)

# 2. 「持家ヒアリング」関連を削除
# owned_home_hearing_inquiry セクション
old_owned_home = '''                {/* owned_home_hearing_inquiry: normalInitials からのボックス選択 */}
                <Grid item xs={12}>
...
text = text.replace(old_owned_home, new_owned_home)
...
# 4. 「担当への伝言/質問事項」フィールドにチャット送信ボタンを追加
...
# 5. chatSending, chatSent の state を追加
old_state = '''  // その他
...
print("Done!")
print("BOM check:", repr(text[:3].encode('utf-8')[:3]))
